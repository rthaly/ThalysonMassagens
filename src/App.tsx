...
// ==================================================================================
// 1. ESTILOS GLOBAIS (Mantidos e Melhorados)
// ==================================================================================
...
.toast-exit { animation: toastSlideOut 0.4s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
@keyframes toastSlideIn { from { opacity: 0; transform: translateY(-20px) translateX(-50%); } to { opacity: 1; transform: translateY(0) translateX(-50%); } }
@keyframes toastSlideOut { from { opacity: 1; transform: translateY(0) translateX(-50%); } to { opacity: 0; transform: translateY(-10px) translateX(-50%); } }
...
.animate-scale { animation: scaleIn 0.4s cubic-bezier(0.34, 1.56, 0.64, 1); }
...
// 🔥 NOVO COMPONENTE: TOAST NOTIFICATION (Substitui Alerts)
const Toast = ({ show, message, type }) => {
  if (!show) return null;
  const isSuccess = type === 'success';
  return (
    <div className="fixed top-6 left-1/2 transform -translate-x-1/2 z-[300] flex items-center gap-3 px-6 py-3 rounded-full shadow-2xl backdrop-blur-md border toast-enter" 
         style={{ backgroundColor: isSuccess ? 'rgba(50, 215, 75, 0.9)' : 'rgba(255, 69, 58, 0.9)', borderColor: 'rgba(255,255,255,0.2)' }}>
       {isSuccess ? <CheckCircle2 className="w-5 h-5 text-white" /> : <AlertCircle className="w-5 h-5 text-white" />}
...
const InlineDateSelector = ({ selectedDate, selectedTime, onSelect }) => {
  ...
  const isTimeBlocked = (t, d) => {
    ...
    // Melhoria: Garante que só mostre horários futuros com pelo menos 1h de antecedência
...
const CouponInventory = ({ inventory, appliedCoupon, onApply, onRemove, onAddManual, showToast }) => {
  ...
  const handleManualAdd = () => {
      ...
      if(codeUpper && SYSTEM_COUPONS[codeUpper]) {
          if (inventory.includes(codeUpper)) {
              showToast('Você já tem este cupom!', 'error');
          } else {
  ...
  return (
    <div className="space-y-4 mt-8">
      ...
      <div className="flex gap-2 mb-3">
          <input 
            value={manualCode}
            onChange={(e) => setManualCode(e.target.value.toUpperCase())} // Melhoria: Auto Uppercase
...
export default function App() {
  ...
  // Melhoria: Scroll Center garantido
  const scrollTo = (ref) => {
    setTimeout(() => {
  ...
  const [isProcessing, setIsProcessing] = useState(false); // Melhoria: Estado de processamento
  ...
  const handleCopyPix = () => { navigator.clipboard.writeText("62922530000144"); showToast("CNPJ Pix Copiado!", "success"); }; 
  ...
  const handleAddManualCoupon = (code) => {
      if (!loyalty.inventory.includes(code)) {
          ...
          showToast(`Cupom ${code} adicionado!`, 'success');
          triggerHaptic();
      } else {
          showToast('Este cupom já está na sua carteira!', 'error');
      }
  };
  ...
  const handleWhatsApp = () => {
    ...
    setIsProcessing(true); // Bloqueia UI
    
    // Simula processamento para dar feedback visual
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen flex justify-center p-0 sm:p-6 font-sans text-gray-200 bg-black" onClick={() => { if(showMenu) setShowMenu(false); }}>
      ...
      <div className="w-full max-w-[440px] bg-[#000] sm:rounded-[40px] shadow-2xl flex flex-col relative overflow-hidden sm:border border-white/10 h-screen sm:h-[92vh] aurora-bg">
        ...
        {/* --- SERVICES --- */}
        {step === 'services' && (
          <div className="flex-1 p-6 pt-36 overflow-y-auto pb-32 animate-fade-in scrollbar-hide">
        ...
        {step === 'configure' && selection.service && (
          <div className="flex-1 p-6 pt-36 overflow-y-auto pb-64 animate-fade-in scrollbar-hide"> 
            ...
            <div className="space-y-10">
              ...
              <CouponInventory inventory={loyalty.inventory} appliedCoupon={selection.coupon} onApply={(code) => { setSelection({...selection, coupon: SYSTEM_COUPONS[code]}); scrollTo(paymentRef); }} onRemove={() => setSelection({...selection, coupon: null})} onAddManual={handleAddManualCoupon} showToast={showToast}/>
          ...
          <div className="absolute bottom-0 w-full p-0 z-40">
            ...
            <div className="bg-[#1C1C1E] rounded-t-[32px] p-5 border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.8)]">
              ...
              <button 
                  ...
                  className={`w-full bg-[#0A84FF] hover:bg-[#007AFF] active:scale-[0.98] transition-all text-white font-bold py-4 rounded-[18px] shadow-[0_4px_20px_rgba(10,132,255,0.4)] flex justify-center items-center gap-2 text-[16px] disabled:opacity-50 disabled:shadow-none ${isProcessing ? 'cursor-wait opacity-80' : ''}`}
              >
                {isProcessing ? (
                   <>
