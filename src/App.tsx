...
/**
 * ==================================================================================
 * THALYSON APP OS v16.1 - STABILITY FIX & FULL I18N
 * ==================================================================================
 * CORREÇÕES REALIZADAS PELO PARCEIRO DE PROGRAMAÇÃO:
 * 1. [CRASH FIX] Proteção contra dados corrompidos/antigos no localStorage.
 * 2. [LOGIC] Proteção no cálculo financeiro para evitar erro de .find() indefinido.
 * 3. [SYNTAX] Correção na interpolação da URL do Google Maps (0{...} -> ${...}).
 * 4. [UI] Formatação e limpeza de imports.
 */

const CONFIG = {
  PHONE: "5517991360413", 
  INSTAGRAM_URL: "https://instagram.com/thalyson.massagens", 
  STORAGE_KEY: '@thaly_app_v16_i18n_fix', // Chave alterada para garantir reset limpo
  LOCALE_PT: 'pt-BR',
  LOCALE_EN: 'en-US'
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
        ...
        if (s) {
            ...
            // PROTEÇÃO CONTRA TELA BRANCA: Merge seguro com valores default
            setUser(prev => ({
                ...prev,
                ...parsed,
                // Garante que arrays não sejam undefined se o localStorage for antigo
        ...
        // Se der erro no parse, reseta para evitar crash
  ...
  const financials = useMemo(() => {
    ...
    // Proteção se booking.extras for undefined
    const safeExtras = booking.extras || {};
    Object.keys(safeExtras).forEach(k => { 
        if(safeExtras[k]) {
            const extraItem = DATA.extras.find(e=>e.id===k);
            // Proteção contra crash se o ID do extra não existir mais
            const exPrice = extraItem ? extraItem.price : 0;
            sub += exPrice; 
        }
    });
  ...
  const generateWhatsAppLink = () => {
    ...
    // CORREÇÃO DE SINTAXE AQUI: Mudança de 0{...} para ${...}
    const msg = `
${T.zap.intro}
...
${locTxt}
${mapQuery ? `\n${T.zap.map_link} http://googleusercontent.com/maps.google.com/?q=${encodeURIComponent(mapQuery)}` : ''}
