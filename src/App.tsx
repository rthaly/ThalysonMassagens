...
const CONFIG = {
  ...
  STORAGE_KEY: '@thaly_app_v22_final', 
...
// Ícones Outline 100% Funcionais e Refeitos (Estilo Minimalista 1.5px)
...
// ==================================================================================
// 2. DESIGN SYSTEM & ESTILOS GLOBAIS (Respiro e UX refinada)
// ==================================================================================
...
// ==================================================================================
// 3. COMPONENTES DE UI (LIMPOS E RESPIRÁVEIS)
// ==================================================================================
...
const Button = memo(({ children, onClick, variant = 'primary', size = 'md', disabled = false, full = false, icon, className = '', loading = false, ariaLabel }: any) => {
  const baseStyle = "inline-flex items-center justify-center font-semibold tracking-wide transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed rounded-2xl select-none active:scale-[0.98] font-poppins gap-2";
  const variants = {
...
const Card = memo(({ children, className = '', onClick, active = false, isDark = true, popular = false }: any) => (
  <div onClick={onClick} className={`relative p-6 md:p-8 rounded-3xl transition-all duration-300 flex flex-col h-full font-poppins ${onClick ? 'cursor-pointer active:scale-[0.99] hover:-translate-y-1' : ''} ${active ? 'bg-blue-500/5 border border-blue-500/50 shadow-md' : isDark ? 'bg-zinc-900/40 border border-zinc-800 hover:border-zinc-700' : 'bg-white border border-slate-200 shadow-sm hover:border-slate-300'} ${className}`}>
    {popular && (
      <div className="absolute -top-3 left-6 md:left-8 bg-zinc-800 text-zinc-100 text-[10px] font-bold uppercase tracking-widest px-4 py-1.5 rounded-full shadow-sm border border-zinc-700">
...
const ReviewCard = memo(({ review, isDark }: { review: Review; isDark: boolean }) => (
  <article className={`w-full h-full p-5 md:p-6 rounded-2xl transition-all duration-300 border ${isDark ? 'bg-zinc-900/30 border-zinc-800' : 'bg-white border-slate-100 shadow-sm'}`}>
...
// Avaliações blindadas: Removido o "Cliente" genérico, Gustavo na Bela Vista sendo o primeiro.
const generateReviews = (isPT: boolean): Review[] => {
  const realReviews = [
    ...
    { n: "Bruno", loc: "Bela Vista - SP", t: isPT ? "Thalyson, quero dizer que sua massagem foi muito bem executada. Recomendo muito." : "Thalyson, I want to say that your massage was very well executed. Highly recommend." },
    { n: "Alan", loc: "Bela Vista - SP", t: isPT ? "Gostei bastante, saí mais leve. Da pra ver que ele manda bem no que faz." : "Liked it a lot, left feeling lighter. You can tell he knows what he's doing." },
    { n: "Tiago", loc: "Bela Vista - SP", t: isPT ? "O Thalyson tem uma energia surreal. A massagem foi perfeita, melhor da minha vida." : "Thalyson has surreal energy. The massage was perfect, best of my life." }
  ];

  const seriousReviews = [
    { n: "Roberto", loc: isPT ? "Jardins - SP" : "Jardins - SP", t: isPT ? "A sensação de vazio e paz que senti após a sessão foi indescritível. A finalização foi extremamente potente, liberando uma carga de tensão que eu carregava há meses. Profissionalismo impecável." : "The feeling of emptiness and peace I felt after the session was indescribable. The finish was extremely powerful, releasing a load of tension I had been carrying for months. Impeccable professionalism." },
    ...
    { n: "Marcelo", loc: isPT ? "Centro - SP" : "Downtown - SP", t: isPT ? "Fui sem expectativa e saí surpreendido. A massagem lingam foi executada com uma técnica precisa e respeitosa. O prazer foi intenso e genuíno." : "I went without expectation and left surprised. The lingam massage was executed with precise and respectful technique. The pleasure was intense and genuine." },
    ...
    { n: "Caio", loc: isPT ? "Consolação - SP" : "Consolação - SP", t: isPT ? "Atendimento impecável no meu hotel. Pontual, discreto e com uma mão que sabe exatamente onde tocar para aliviar a tensão." : "Impeccable service at my hotel. Punctual, discreet, and with a hand that knows exactly where to touch to relieve tension." },
...
const getData = (lang: string) => {
  ...
  // Preços Gatilho / Numerações precisas
  const p = {
    relax: getPrice(157),
    sens: getPrice(177),
    titan: getPrice(207),
    nuru: getPrice(317),
    depil: getPrice(107), 
    // Matematica atraente (Valor percebido alto com desconto real)
    packRelax: { v: getPrice(527), full: getPrice(628), save: getPrice(101) }, // 4x 157
    packTri: { v: getPrice(517), full: getPrice(621), save: getPrice(104) }, // 3x 207
    packMix: { v: getPrice(637), full: getPrice(768), save: getPrice(131) }, // 2x 177 + 2x 207
    packSupreme: { v: getPrice(567), full: getPrice(681), save: getPrice(114) }, // 1x 157 + 1x 207 + 1x 317
    extras: {
  ...
  return {
    ...
    // Descrições clínicas EXATAS como foram solicitadas
    services: [
      {
        id: 'depilacao',
        min: 45,
    ...
    // Textos de interface atualizados para serem mais humanos e acolhedores (Conforme pedido)
    text: {
...
export default function App() {
  ...
  useEffect(() => {
    if (isClient) {
        document.title = step === 0 ? "Thalyson Massagens - Cuidado e Bem-estar" : "Thalyson - Confirmação do Cuidado";
  ...
  // =======================================================
  // RECUPERAÇÃO SEGURA DO CACHE (PREVINE TELA BRANCA)
  // =======================================================
  useEffect(() => {
    if (!isClient) return;
    ...
    try {
      ...
      console.error('Falha de Cache - Resetando', e);
      loadedStep = 0;
    }
  ...
  const generateWhatsAppMsg = () => {
    ...
    let serviceTitle = booking.item?.title || ''; if (booking.type !== 'single' && booking.item?.desc) serviceTitle += ` ${lang === 'pt' ? '(Pacote)' : '(Journey)'}`;
...
${locTxt}
${mapQuery ? `🔗 Rota Fixada: http://googleusercontent.com/maps.google.com/?q=${encodeURIComponent(mapQuery)}` : ''}
...
*Sessão condicionada à resposta da equipe de atendimento.*
  ...
  return (
    <>
      ...
      <main className={`min-h-screen relative z-10 pb-36 px-4 md:px-12 max-w-5xl mx-auto`}>
        {step !== 4 && (
        ...
        <div className="space-y-12">
              ...
              <div className="py-12 relative">
                ...
                <div id="reviews-slider" className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide scroll-smooth pb-8 -mx-4 px-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                  {DATA.reviews.map((r, i) => (
                    <div key={i} className="snap-center flex-shrink-0 w-[85vw] md:w-[350px] mr-4 last:mr-0">
          ...
          {step === 2 && (
            <section className="space-y-10 animate-fade-in max-w-3xl mx-auto">
              ...
              <div className={`p-6 md:p-8 rounded-[2rem] border ${isDark ? 'bg-zinc-900/20 border-zinc-800/60' : 'bg-white border-slate-100 shadow-sm'} space-y-6`}>
                    ...
                    return (
                      <div key={ex.id} onClick={() => setBooking(b => ({ ...b, extras: { ...b.extras, [ex.id]: !b.extras[ex.id] } }))} className={`flex items-center justify-between p-5 md:p-6 rounded-[2rem] border cursor-pointer transition-all ${isActive ? isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-slate-50 border-slate-300 shadow-sm' : isDark ? 'bg-zinc-900/20 border-zinc-800/50 hover:border-zinc-700' : 'bg-white border-slate-100 hover:border-slate-200 shadow-sm'}`} role="checkbox" aria-checked={isActive}>
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className={`text-sm md:text-base font-medium ${isActive ? isDark ? 'text-white' : 'text-slate-900' : isDark ? 'text-zinc-300' : 'text-slate-700'}`}>{ex.label}</p>
                            <p className={`text-xs font-light mt-0.5 ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>{ex.desc}</p>
                          </div>
                        </div>
                        <div className="text-right shrink-0">
                          <span className={`text-[10px] md:text-[11px] font-bold tracking-widest px-3 md:px-4 py-1.5 md:py-2 rounded-full ${isActive ? isDark ? 'bg-zinc-700 text-zinc-100' : 'bg-slate-200 text-slate-800' : isDark ? 'bg-zinc-900 text-zinc-600' : 'bg-slate-50 text-slate-400'}`}>
