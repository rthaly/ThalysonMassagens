...
import {
  ...
  LogOut, Copy, RefreshCw, Zap, Crown, Music, Trash2, CreditCard, Banknote, QrCode, AlertTriangle, Edit3, Plus, Info, Receipt, CheckCircle2, Siren, Send, ThumbsUp, Car, Menu, Smartphone
} from 'lucide-react';

// ==================================================================================
// 1. INFRAESTRUTURA & UTILITÁRIOS (SEGURANÇA E ARMAZENAMENTO)
// ==================================================================================

// Sistema simples de ofuscação para proteger dados locais sem custo
const SecureStorage = {
  SECRET: 'THALY_SECURE_V1_',
  encrypt: (data) => {
    try {
      const json = JSON.stringify(data);
      return btoa(encodeURIComponent(json)); // Base64 simples para evitar leitura direta
    } catch (e) { return null; }
  },
  decrypt: (cipher) => {
    try {
      const json = decodeURIComponent(atob(cipher));
...
/* --- TOAST NOTIFICATIONS --- */
.toast-container { position: fixed; top: 20px; left: 50%; transform: translateX(-50%); z-index: 9999; display: flex; flex-col-reverse; gap: 10px; width: 90%; max-width: 350px; pointer-events: none; }
.toast { pointer-events: auto; background: rgba(28, 28, 30, 0.95); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.1); padding: 12px 16px; border-radius: 16px; display: flex; items-center; gap: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); animation: toastIn 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
...
const ToastNotification = ({ message, type, onClose }) => {
  ...
  const icons = {
    ...
    success: <CheckCircle2 className="w-5 h-5 text-[#32D74B]" />,
    info: <Info className="w-5 h-5 text-[#0A84FF]" />
  };
  ...
  const handleManualAdd = () => {
      ...
      if(codeUpper && SYSTEM_COUPONS[codeUpper]) {
          if (inventory.includes(codeUpper)) {
              ...
              setManualCode('');
              triggerHaptic();
              addToast('Cupom adicionado com sucesso!', 'success');
          }
      } else {
...
export default function App() {
  ...
  const [loyalty, setLoyalty] = useState(() => {
    // Tenta recuperar do SecureStorage
    const saved = SecureStorage.get('DATA'); 
    // Se não existir, tenta o antigo localStorage inseguro para migrar
    if (!saved) {
    ...
    return saved ? saved : { savedName: '', avatar: '😎', totalSpent: 0, totalSaved: 0, inventory: ['BEMVINDO'], notifications: [], history: [] };
  });
  ...
  useEffect(() => {
    // Salva usando criptografia simples
    SecureStorage.set('DATA', loyalty);
  ...
  const handleWhatsApp = () => {
    ...
    // Validação Robusta
    if (!selection.date || !selection.time) { addToast("Selecione data e hora.", "error"); return; }
    if (!selection.location) { addToast("Selecione o local.", "error"); return; }
    if (selection.location.id === 'outras-cidades' && !selection.city) { addToast("Digite o nome da cidade.", "error"); return; }
    if (!selection.paymentMethod) { addToast("Escolha como pagar.", "error"); return; }
  ...
  return (
    <div className="min-h-screen flex justify-center p-0 sm:p-6 font-sans text-gray-200 bg-black" onClick={() => { if(showMenu) setShowMenu(false); }}>
      ...
      <div className="toast-container">
        {toasts.map(t => (
          <ToastNotification key={t.id} message={t.msg} type={t.type} onClose={() => setToasts(p => p.filter(x => x.id !== t.id))} />
        ))}
      </div>
