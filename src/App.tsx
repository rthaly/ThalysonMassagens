...
// ==================================================================================
// 1. DADOS E CONFIGURAÇÕES GLOBAIS (STATIC DATA)
// ==================================================================================

const CONFIG = {
  PHONE: "5517991360413", 
  INSTAGRAM_URL: "https://instagram.com/seumssagista", 
  STORAGE_KEY: '@thaly_app_v16_ultimate', 
  XP_TARGET: 500, 
};

// --- ALGORITMO DE HORÁRIOS ESGOTADOS ---
// Gera horários "ocupados" baseados na data (determinístico)
// Regra: Apenas nos próximos 3 dias. 2 horários por dia.
const getSoldOutSlots = (dateObj) => {
  ...
  // Se for daqui a 4 dias ou mais, mostra tudo livre (para não parecer fake)
  if (diffDays > 3) return [];

  // Cria um "seed" baseado no dia do mês
  const day = target.getDate();
  
  // Lógica arbitrária para bloquear horários fixos baseados no dia (par/impar)
  if (day % 2 === 0) return ['18:00', '20:00'];
  return ['10:00', '19:00'];
};
...
const TEXTS = {
  pt: {
    ...
    services: {
      ...
      mista: { title: "Massagem Completa", desc: "Massagem no corpo inteiro para relaxar, depois corpo a corpo sensitiva. Finalização com Lingam inclusa (opcional)." }
    },
    ...
    terms_body: [
      ...
      "2. Higiene: Prezo pela máxima higiene e exijo o mesmo do cliente. Todos os materiais são higienizados e esterilizados.",
  ...
  en: {
    ...
    viewing_now: "people viewing this slot now",
    sold_out: "Sold Out",
    today: "Today",
    tomorrow: "Tomorrow",
    ...
    services: {
      ...
      mista: { title: "Complete Massage", desc: "Full body relaxation massage, followed by sensitive body-to-body. Lingam finishing included (optional)." }
    },
    ...
    terms_body: [
      ...
      "2. Hygiene: I value maximum hygiene. All materials are sanitized and sterilized.",
...
// MODAL DE TERMOS (COM SCROLL INDEPENDENTE)
...
// MODAL DE AVALIAÇÕES (COM SCROLL INDEPENDENTE)
             ...
             {reviews.map((r, i) => (
                <div key={i} className={`p-5 rounded-2xl border ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-blue-100 shadow-sm'}`}>
                   <div className="flex justify-between items-start mb-2">
                      <span className="font-bold text-sm text-blue-500">{r.a}</span>
                      <div className="flex text-amber-400 gap-0.5">{[...Array(r.s)].map((_,k)=><Star key={k} size={12} fill="currentColor"/>)}</div>
                   </div>
                   <p className="text-sm opacity-70 leading-relaxed italic">"{r.t}"</p>
                </div>
             ))}
...
export default function App() {
  ...
  // Reset Scroll quando muda o passo
  useEffect(() => {
                ...
                <div className="space-y-3 mb-6">
                    {DB.reviews.slice(0, 3).map((r, i) => (
                        <div key={i} className={`p-4 rounded-2xl border ${isDark ? 'bg-zinc-900/50 border-zinc-800' : 'bg-white border-blue-50'}`}>
                             <p className="text-xs italic opacity-70 mb-2 line-clamp-2">"{r.t}"</p>
                             <div className="flex justify-between items-center">
                                 <span className="text-[10px] font-bold uppercase text-blue-500">{r.a}</span>
                                 <div className="flex text-amber-400 gap-0.5">{[...Array(r.s)].map((_,k)=><Star key={k} size={8} fill="currentColor"/>)}</div>
                             </div>
                        </div>
                    ))}
                </div>
             ...
             <div className="space-y-4">
                <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide -mx-6 px-6">
                  {[0, 1, 2, 3, 4, 5, 6].map((offset) => {
                    ...
                    // Lógica para mostrar Hoje/Amanhã
                    let dayLabel = d.toLocaleDateString(lang === 'pt' ? 'pt-BR' : 'en-US', { weekday: 'short' }).slice(0,3);
                    if (offset === 0) dayLabel = T.today;
                    if (offset === 1) dayLabel = T.tomorrow;
             ...
             <div className={`transition-all duration-500 ${booking.date ? 'opacity-100 translate-y-0' : 'opacity-30 translate-y-4 pointer-events-none'}`}>
                  ...
                  {['09:00', '10:00', '11:00', '13:00', '15:00', '18:00', '19:00', '20:00'].map((time) => {
                     ...
                     if (booking.date) {
                        ...
                        if (selectedDate.toDateString() === now.toDateString() && parseInt(h) <= now.getHours()) {
                            ...
                            // Lógica de Esgotado
                            const soldOutSlots = getSoldOutSlots(booking.date);
                            soldOut = soldOutSlots.includes(time);
                        }
                     }
                     ...
                     return (
                        <button key={time} disabled={disabled || soldOut} 
                          ...
                          className={`relative py-4 rounded-2xl text-xs font-bold border transition-all overflow-hidden
                            ${soldOut 
                                ? 'opacity-40 cursor-not-allowed bg-red-500/5 border-red-500/10 text-red-500 decoration-red-500/50 line-through' 
                                : booking.time === time 
                        ...
                        >
                          {time}
                          {soldOut && <div className="absolute inset-0 flex items-center justify-center bg-zinc-950/80 text-[8px] uppercase tracking-widest text-red-500 font-black rotate-12">{T.sold_out}</div>}
                        </button>
                     );
                  })}
                </div>
                ...
                {booking.time && (
                    <div className="mt-6 flex justify-center animate-fade-in">
                        <div className="bg-amber-500/10 border border-amber-500/20 text-amber-500 px-4 py-2 rounded-full text-xs font-bold flex items-center gap-2 shadow-lg shadow-amber-500/5">
