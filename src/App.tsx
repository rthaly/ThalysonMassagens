...
// --- ESTRUTURA DE IDIOMAS (TRADUÇÃO COMPLETA) ---
const TEXTS = {
  pt: {
    welcome: "Olá,",
    subtitle: "Escolha como deseja relaxar hoje.",
    reviews_btn: "Ver 50+ avaliações",
...
// --- DADOS NÚMERICOS (PREÇOS ATUALIZADOS) ---
const DB = {
  services: [
...
const Toast = ({ msg, show }) => (
  <div className={`fixed top-24 left-1/2 -translate-x-1/2 z-[130] transition-all duration-500 pointer-events-none ${show ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center p-4">
      ...
      <div className="relative w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-3xl p-6 pb-8 animate-slide-up shadow-2xl max-h-[85vh] flex flex-col">
        <div className="flex justify-between items-center mb-6 flex-shrink-0">
            {title && <h3 className="text-2xl font-bold text-white tracking-tight">{title}</h3>}
            ...
            <div className="relative bg-gradient-to-br from-blue-900 to-zinc-900 border border-blue-500/50 p-8 rounded-[2rem] text-center max-w-sm w-full animate-scale-in shadow-[0_0_50px_rgba(37,99,235,0.3)]">
                ...
                <div className="space-y-3">
                    <button onClick={onAllowNotif} className="w-full py-4 bg-white text-blue-900 font-bold rounded-xl text-lg hover:bg-blue-50 transition-colors flex items-center justify-center gap-2">
                        <Bell size={20} /> Receber Notificação
                    </button>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (s) {
        ...
        // Novo Usuário: Ganha cupom e vê popup
        setUser(prev => ({
            ...prev,
            coupons: [{ id: 'welcome15', val: 15, title: 'Cupom Boas Vindas' }]
        }));
      ...
      if ('Notification' in window) {
          Notification.requestPermission().then(permission => {
              if (permission === 'granted') {
                  new Notification(T.notification_title, { body: T.notification_msg });
  ...
  const finishBooking = () => {
    ...
    if(leveledUp) {
        ...
        // Tenta notificar se tiver permissão
        if (Notification.permission === 'granted') {
             new Notification("Novo Cupom!", { body: "Você ganhou R$ 25,00 por subir de nível!" });
        }
    }
    setStep(4);
  };
  ...
  const reset = () => {
    setStep(0);
    // Limpa booking mas mantém endereço salvo
    setBooking({ 
...
`.trim();
  ...
  return (
    <div className={`h-[100dvh] w-full ${isDark ? 'bg-zinc-950 text-zinc-100' : 'bg-slate-50 text-slate-900'} font-sans flex flex-col overflow-hidden selection:bg-blue-500/30 transition-colors duration-500`}>
      ...
      {/* HEADER (NAVBAR) */}
      <header className={`h-24 px-8 flex items-center justify-between border-b backdrop-blur-md z-20 shrink-0 ${isDark ? 'border-zinc-800 bg-zinc-950/90' : 'border-slate-200 bg-white/90'}`}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className={`font-bold text-lg tracking-wide ${isDark ? 'text-white' : 'text-slate-900'}`}>Thalyson</span>
            <span className="text-xs opacity-60 font-medium">Massoterapeuta</span>
          </div>
        </div>
        <div className="flex gap-4">
            <button onClick={() => setLang(l => l==='pt'?'en':'pt')} className={`p-3 rounded-full transition-colors ${isDark ? 'bg-white/5 hover:bg-white/10' : 'bg-slate-100 hover:bg-slate-200'}`}>
                <Globe size={24} className={isDark ? 'text-zinc-400' : 'text-slate-600'}/>
            </button>
            <button onClick={() => setIsDark(!isDark)} className={`p-3 rounded-full transition-colors ${isDark ? 'bg-white/5 hover:bg-white/10' : 'bg-slate-100 hover:bg-slate-200'}`}>
                {isDark ? <Sun size={24} className="text-amber-400"/> : <Moon size={24} className="text-blue-600"/>}
            </button>
            <a href={CONFIG.INSTAGRAM_URL} target="_blank" rel="noreferrer" className={`p-3 rounded-full transition-colors ${isDark ? 'bg-white/5 hover:bg-white/10' : 'bg-slate-100 hover:bg-slate-200'}`}>
      ...
      {/* PROGRESSO */}
      {step < 4 && (
        <div className={`w-full h-1.5 flex-shrink-0 ${isDark ? 'bg-zinc-900' : 'bg-slate-200'}`}>
      ...
      <main ref={scrollRef} className="flex-1 overflow-y-auto overflow-x-hidden p-8 pb-40 scroll-smooth">
        <div className="max-w-md mx-auto space-y-12 animate-fade-in">
          ...
          {step === 0 && (
            <>
              <div className="space-y-4">
                <h1 className={`text-4xl font-bold leading-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.welcome} <br/><span className="text-blue-600">{user.name.split(' ')[0] || ''}</span></h1>
                <p className={`text-xl font-light leading-relaxed ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>{T.subtitle}</p>
                <button onClick={() => setReviewsOpen(true)} className={`flex items-center gap-3 border px-5 py-3 rounded-2xl w-full transition-colors ${isDark ? 'bg-zinc-900 border-zinc-800 hover:bg-zinc-800' : 'bg-white border-slate-200 hover:bg-slate-50'}`}>
                  ...
                  <span className={`text-sm font-bold ${isDark ? 'text-zinc-300' : 'text-slate-600'}`}>{T.reviews_count}</span>
                  <ArrowRight size={16} className="ml-auto opacity-50"/>
                </button>
              </div>

              <div className="space-y-6">
                <h2 className={`text-sm font-bold uppercase tracking-widest pl-1 ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>{T.choose_service}</h2>
          ...
          {step === 1 && (
            <>
              <div className="text-center">
                <h2 className={`text-3xl font-bold mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.select_time_title}</h2>
                <p className={`text-lg font-light ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>{T.date_sub}</p>
              </div>
              ...
              <div className="space-y-8">
                    ...
                    return (
                      <button key={i} onClick={() => setBooking(b => ({ ...b, date: d, time: null }))} 
                        className={`min-w-[90px] h-28 rounded-[28px] flex flex-col items-center justify-center gap-1 border-2 transition-all flex-shrink-0
                          ${isSel ? 'bg-blue-600 border-blue-500 text-white shadow-xl scale-105' : (isDark ? 'bg-zinc-900 border-zinc-800 text-zinc-500' : 'bg-white border-slate-200 text-slate-400')}`}
                      >
                ...
                <div className={`transition-all duration-500 ${booking.date ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
                  <h3 className={`text-sm font-bold uppercase tracking-widest text-center mb-6 ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>{T.time_title}</h3>
                            ...
                            className={`py-5 rounded-2xl text-base font-bold border-2 transition-all
                              ${booking.time === time 
                                  ...
                                  : disabled 
                                    ...
                                    : (isDark ? 'bg-zinc-900 border-zinc-800 text-zinc-300 hover:border-zinc-600' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300')}`}
                          >
                            {time}
                          </button>
                       );
          ...
          {step === 2 && (
            <>
              <div className="text-center mb-8">
                <h2 className={`text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.location_title}</h2>
              </div>

              <div className={`flex p-2 rounded-[24px] border ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}>
                {[{ id: 'home', label: 'Home', icon: Home }, { id: 'motel', label: 'Motel', icon: BedDouble }, { id: 'hotel', label: 'Hotel', icon: Building }].map((type) => (
                  <button key={type.id} onClick={() => setBooking(b => ({ ...b, locationType: type.id }))}
                    className={`flex-1 py-5 rounded-[20px] text-sm font-bold flex flex-col items-center justify-center gap-2 transition-all duration-300 ${booking.locationType === type.id ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500'}`}
                  >
              ...
              <div className="space-y-6">
                <div>
                   <label className="text-sm font-bold text-zinc-500 ml-2 mb-2 block">{T.input_name}</label>
                   <input value={user.name} onChange={(e) => setUser(u => ({...u, name: e.target.value}))} placeholder={T.input_name_placeholder} className={`w-full p-6 rounded-[24px] border text-lg outline-none focus:border-blue-500 ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`}/>
                </div>
                ...
                {booking.locationType === 'home' && (
                  <div className="space-y-6 animate-fade-in">
                    <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl flex gap-4 items-center">
                        ...
                        <p className={`text-sm font-medium ${isDark ? 'text-blue-200' : 'text-blue-800'}`}>{T.address_warn}</p>
                    </div>
                    
                    <div>
                        <label className="text-sm font-bold text-zinc-500 ml-2 mb-2 block">{T.input_addr}</label>
                        <input className={`w-full p-6 rounded-[24px] border text-lg outline-none focus:border-blue-500 ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} value={booking.address.street} onChange={(e) => setBooking(b => ({...b, address: {...b.address, street: e.target.value}}))} />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                       <div>
                           <label className="text-sm font-bold text-zinc-500 ml-2 mb-2 block">{T.input_num}</label>
                           <input type="tel" className={`w-full p-6 rounded-[24px] border text-lg outline-none focus:border-blue-500 ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} value={booking.address.number} onChange={(e) => setBooking(b => ({...b, address: {...b.address, number: e.target.value}}))} />
                       </div>
                       <div>
                           <label className="text-sm font-bold text-zinc-500 ml-2 mb-2 block">{T.input_bairro}</label>
                           <input className={`w-full p-6 rounded-[24px] border text-lg outline-none focus:border-blue-500 ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} value={booking.address.district} onChange={(e) => setBooking(b => ({...b, address: {...b.address, district: e.target.value}}))} />
                       </div>
                    </div>
                    
                    <div>
                        <label className="text-sm font-bold text-zinc-500 ml-2 mb-2 block">{T.input_city}</label>
                        <input className={`w-full p-6 rounded-[24px] border text-lg outline-none focus:border-blue-500 ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} value={booking.address.city} onChange={(e) => setBooking(b => ({...b, address: {...b.address, city: e.target.value}}))} />
                    </div>

                    <div>
                        <label className="text-sm font-bold text-zinc-500 ml-2 mb-2 block">{T.input_comp}</label>
                        <input className={`w-full p-6 rounded-[24px] border text-lg outline-none focus:border-blue-500 ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} value={booking.address.comp} onChange={(e) => setBooking(b => ({...b, address: {...b.address, comp: e.target.value}}))} />
                    </div>
                  </div>
                )}

                {booking.locationType === 'motel' && (
                    <div className="p-8 rounded-[24px] bg-blue-500/10 border border-blue-500/20 text-blue-500 animate-fade-in text-center">
                ...
                {booking.locationType === 'hotel' && (
                   <div className="space-y-6 animate-fade-in">
                      <div>
                          <label className="text-sm font-bold text-zinc-500 ml-2 mb-2 block">{T.input_hotel}</label>
                          <input className={`w-full p-6 rounded-[24px] border text-lg outline-none focus:border-blue-500 ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} value={booking.address.placeName} onChange={(e) => setBooking(b => ({...b, address: {...b.address, placeName: e.target.value}}))} />
                      </div>
                       <div className="grid grid-cols-2 gap-4">
                          <div>
                              <label className="text-sm font-bold text-zinc-500 ml-2 mb-2 block">{T.input_city}</label>
                          ...
                          <div>
                              <label className="text-sm font-bold text-zinc-500 ml-2 mb-2 block">{T.input_room}</label>
                              <input className={`w-full p-6 rounded-[24px] border text-lg outline-none focus:border-blue-500 ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} value={booking.address.comp} onChange={(e) => setBooking(b => ({...b, address: {...b.address, comp: e.target.value}}))} />
                          </div>
                       </div>
                   </div>
               ...
               <div className="space-y-4">
                 {DB.extras.map(extra => (
                   <div key={extra.id} onClick={() => setBooking(b => ({ ...b, extras: { ...b.extras, [extra.id]: !b.extras[extra.id] } }))}
                    className={`flex items-center justify-between p-6 rounded-[24px] border-2 cursor-pointer transition-all ${booking.extras[extra.id] ? 'bg-blue-600/10 border-blue-500' : (isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200')}`}
                   >
                     <div className="flex items-center gap-5">
                       ...
                       <div><p className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.extras_list[extra.id].label}</p><p className="text-sm text-zinc-400">{T.extras_list[extra.id].sub}</p></div>
                     </div>
                     <span className={`text-base font-bold ${booking.extras[extra.id] ? 'text-blue-500' : 'text-zinc-500'}`}>+ R$ {extra.price}</span>
                   </div>
                 ))}
               </div>
             </div>
        ...
        {step === 3 && (
          <div className="space-y-12 animate-slide-in pb-10">
             <button onClick={()=>setStep(2)} className="flex items-center gap-2 text-base font-bold text-zinc-500 hover:text-blue-500 transition-colors"><ChevronLeft size={24}/> Voltar</button>
             
             <div className="text-center">
                <h2 className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.resume_title}</h2>
             </div>

             <div className={`rounded-[32px] border p-8 relative overflow-hidden shadow-2xl ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}>
                <div className="space-y-6 relative z-10">
                  <div className="flex justify-between items-center pb-6 border-b border-zinc-800">
                    <span className={`font-bold text-2xl ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.services[booking.service.id].title}</span>
                    <span className={`font-bold text-2xl ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.currency} {booking.service.price}</span>
                  </div>
                  
                  {Object.keys(booking.extras).filter(k => booking.extras[k]).map(k => (
                    <div key={k} className="flex justify-between text-lg text-zinc-500">
                  ...
                  {/* CUPOM */}
                  <div className={`p-6 rounded-2xl border ${isDark ? 'bg-zinc-950 border-zinc-800' : 'bg-slate-50 border-slate-200'}`}>
                    <div className="flex items-center gap-3 text-base font-bold text-zinc-400 mb-4">
                        <Ticket size={24} className="text-blue-500"/> {T.coupon_title}
                    </div>
                    {user.coupons.length > 0 ? (
                      booking.appliedCoupon ? (
                        <div className="flex justify-between items-center bg-green-500/10 p-4 rounded-xl border border-green-500/30">
                            <span className="text-green-600 font-bold">{booking.appliedCoupon.title}</span>
                            <button onClick={() => setBooking(b => ({...b, appliedCoupon: null}))} className="text-sm text-red-500 font-bold underline">{T.remove}</button>
                        </div>
                      ) : (
                        <select 
                            onChange={(e) => {
                            ...
                            className={`w-full p-4 rounded-xl outline-none text-base ${isDark ? 'bg-zinc-800 text-white' : 'bg-white text-slate-900'}`}
                        >
                           <option value="">{T.coupon_select}</option>
                           {user.coupons.map(c => <option key={c.id} value={c.id}>R$ {c.val} OFF - {c.title}</option>)}
                        </select>
                      )
                    ) : <p className="text-zinc-500 text-sm">{T.coupon_none}</p>}
                  </div>

                  {booking.appliedCoupon && (
                    <div className="flex justify-between text-lg text-green-500 font-bold">
                      <span>Desconto</span>
                  ...
                  <div className="pt-6 border-t border-zinc-800 flex flex-col gap-2">
                    <div className="flex justify-between items-end">
                      ...
                      <span className={`text-5xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.currency} {getFinancials.total}</span>
                    </div>
             ...
             <div className="space-y-6">
                <h3 className="text-lg font-bold text-zinc-500 ml-2">{T.pay_title}</h3>
                <div className="grid grid-cols-3 gap-4">
                  {[{id:'pix', l:T.pay_pix, i:QrCode}, {id:'card', l:T.pay_card, i:CreditCard}, {id:'money', l:T.pay_cash, i:Banknote}].map(p => (
                    <button key={p.id} onClick={() => setBooking(b => ({ ...b, payment: p.id }))}
                      className={`flex flex-col items-center justify-center gap-3 py-8 rounded-[24px] border-2 transition-all duration-200 ${booking.payment === p.id ? 'bg-blue-600 border-blue-500 text-white shadow-xl' : (isDark ? 'bg-zinc-900 border-zinc-800 text-zinc-400' : 'bg-white border-slate-200 text-slate-500')}`}
                    >
             ...
             <div className={`flex items-start gap-5 p-6 rounded-2xl border cursor-pointer ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`} onClick={() => setBooking(b => ({...b, termsAccepted: !b.termsAccepted}))}>
                <div className={`w-8 h-8 rounded-xl border-2 flex items-center justify-center transition-all shrink-0 mt-1 ${booking.termsAccepted ? 'bg-blue-600 border-blue-600 text-white' : 'border-zinc-400'}`}>
                   {booking.termsAccepted && <Check size={20} strokeWidth={4} />}
                </div>
                <p className="text-base text-zinc-500 leading-relaxed">
                   {T.terms_agree} <span onClick={(e)=>{e.stopPropagation(); setTermsOpen(true);}} className="font-bold underline text-blue-500 cursor-pointer">{T.terms_link}</span>.
                </p>
             </div>
          </div>
        )}
        ...
        {step === 4 && (
            <div className="flex flex-col items-center justify-center pt-24 animate-scale-in text-center h-full">
                ...
                <h1 className={`text-5xl font-black mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.success_title}</h1>
                <p className="text-xl text-zinc-500 max-w-[320px] mx-auto leading-relaxed mb-16">{T.success_sub}</p>
                ...
                <button onClick={reset} className="mt-12 text-sm font-bold text-zinc-500 hover:text-blue-500 uppercase tracking-widest p-4">
                    {T.back_home}
                </button>
            </div>
        )}
      ...
      {/* --- FOOTER FIXO (AÇÃO) --- */}
      {step < 4 && (
          <div className={`h-32 flex-shrink-0 flex items-center justify-center px-8 border-t backdrop-blur-xl pb-6 ${isDark ? 'border-zinc-800 bg-zinc-950/90' : 'border-slate-200 bg-white/90'}`}>
             <div className="w-full max-w-md flex items-center gap-6">
                {step < 3 && booking.service && (
                   <div className="flex-1 animate-fade-in">
                      ...
                      <span className={`block text-4xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>R$ {getFinancials.total}</span>
                   </div>
                )}
