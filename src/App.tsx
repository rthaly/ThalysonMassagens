...
import React, { useState, useEffect, useMemo, useRef, useCallback, useId, memo } from 'react';
...
const CONFIG = {
  ...
  PROFILE_IMAGE_URL: "", // cole aqui a URL da sua foto profissional quando subir a imagem
...
const GlobalStyles = memo(({ isDark }: { isDark: boolean }) => (
  <style dangerouslySetInnerHTML={{ __html: `
    ...
    *, *::before, *::after { box-sizing: border-box; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; }
    ...
    body, #root { width: 100%; min-width: 0; overflow-x: hidden; }
    img, svg, video, canvas { max-width: 100%; }
    p, h1, h2, h3, h4, h5, h6, span, strong, small, label, button, a, li {
    ...
    .safe-card { min-width: 0; max-width: 100%; overflow: hidden; }
    .safe-text { overflow-wrap: anywhere; word-break: normal; hyphens: auto; }
    .line-clamp-2 safe-text {
      display: -webkit-box;
      -webkit-line-clamp: 2;
...
// ICON COMPONENT
// ==================================================================================
const Icon = memo(({ name, size = 20, className = '', style, title }: { name: string; size?: number; className?: string; style?: React.CSSProperties; title?: string }) => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={`shrink-0 ${className}`} style={style} aria-hidden={title ? undefined : true} role={title ? "img" : undefined}>
...
const Button = memo(({ children, onClick, variant = 'primary', size = 'md', disabled = false, full = false, icon, className = '', loading = false, ariaLabel }: any) => {
  const base = "relative inline-flex items-center justify-center font-semibold tracking-wide transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed select-none active:scale-[0.97] gap-2 min-w-0 max-w-full overflow-hidden";
...
// Refined Input
const InputField = memo(({ label, value, onChange, placeholder, icon, type = 'text', isDark = true, hasError = false, disabled = false, maxLength, autoComplete }: any) => {
  const generatedId = useId();
  const inputId = `field-${generatedId.replace(/:/g, '')}`;

  return (
    <div className={`space-y-2 w-full min-w-0 ${hasError ? 'animate-shake' : ''}`}>
      {label && (
        <label htmlFor={inputId} className={`block text-xs font-semibold uppercase tracking-widest pl-1 safe-text ${hasError ? 'text-red-400' : isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
          {label}
        </label>
      )}
      <div className="relative group min-w-0">
        ...
        <input
          ...
          className={`input-field w-full min-w-0 h-14 rounded-2xl text-base font-medium transition-all border outline-none disabled:opacity-50 disabled:cursor-not-allowed ${icon ? 'pl-12 pr-4' : 'px-5'} ${hasError
...
const SideMenu = memo(({ isOpen, onClose, isDark, toggleTheme, user, T }: any) => {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-[60] animate-fade-in" onClick={onClose} aria-hidden="true" />
      <aside role="dialog" aria-modal="true" aria-label={T.menu_title} className={`fixed top-0 right-0 h-full w-80 max-w-[88vw] z-[70] p-6 sm:p-8 shadow-2xl animate-slide-right flex flex-col overflow-y-auto ${isDark ? 'bg-[#11141a] border-l border-white/6' : 'bg-[#f9f8f6] border-l border-black/6'}`}>
        <div className="flex justify-between items-center mb-8">
          <h2 className="font-display text-2xl">{T.menu_title}</h2>
          <button onClick={onClose} aria-label="Fechar menu" className={`w-10 h-10 flex items-center justify-center rounded-xl transition-colors ${isDark ? 'hover:bg-white/8 text-zinc-400' : 'hover:bg-black/5 text-slate-500'}`}>
            <Icon name="x" size={22} />
          </button>
        </div>
...
const ReviewCard = memo(({ review, isDark }: { review: Review; isDark: boolean }) => (
  <article className={`h-full flex flex-col p-5 sm:p-7 rounded-[1.5rem] sm:rounded-[2rem] border transition-all duration-300 safe-card ${isDark ? 'bg-white/4 border-white/8 hover:bg-white/6 hover:border-white/14' : 'bg-white border-slate-200 shadow-sm hover:shadow-md'}`}>
    ...
    <p className={`text-sm leading-relaxed font-medium italic flex-1 safe-text ${isDark ? 'text-zinc-300' : 'text-slate-600'}`}>"{review.t}"</p>
  </article>
));
...
const FAQItem = memo(({ q, a, isDark }: { q: string; a: string; isDark: boolean }) => {
  ...
  return (
    <div className={`border-b last:border-b-0 ${isDark ? 'border-white/8' : 'border-slate-200'}`}>
      <button type="button" aria-expanded={open} onClick={() => setOpen(!open)} className="w-full py-5 sm:py-6 flex items-center justify-between text-left gap-4 group">
        <span className={`text-sm sm:text-base font-medium leading-snug safe-text ${isDark ? 'text-white/90 group-hover:text-white' : 'text-slate-800 group-hover:text-slate-900'}`}>{q}</span>
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${open ? 'max-h-96 pb-6 opacity-100' : 'max-h-0 opacity-0'}`}>
        <p className={`text-sm leading-relaxed safe-text ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>{a}</p>
      </div>
    </div>
  );
});
...
const SmartTimer = memo(({ isDark, text }: any) => {
  ...
  return (
    <div className={`flex items-center gap-4 p-4 sm:p-5 rounded-[1.5rem] sm:rounded-[2rem] border safe-card ${isDark ? 'bg-blue-950/30 border-blue-500/20' : 'bg-blue-50 border-blue-200'}`}>
...
const RuleItem = memo(({ rule, isDark }: { rule: Rule; isDark: boolean }) => (
  <div className={`flex gap-4 p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border border-transparent transition-colors safe-card ${isDark ? 'hover:bg-white/5 hover:border-white/8' : 'hover:bg-slate-50 hover:border-slate-200'}`}>
    ...
    <div>
      ...
      <p className={`text-sm leading-relaxed safe-text ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>{rule.description}</p>
    </div>
  </div>
));
...
const ServiceModal = memo(({ service, isOpen, onClose, onSelect, isInCart, isDark, T, lang, isPremium }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="service-modal-title">
      <div className={`relative w-full max-w-md max-h-[92vh] flex flex-col rounded-[2rem] sm:rounded-[2.5rem] border shadow-2xl animate-scale-in overflow-hidden safe-card ${isDark ? 'bg-[#11141a] border-white/10' : 'bg-[#ffffff] border-slate-200'}`}>
        ...
        <div className={`relative p-5 sm:p-8 pb-5 sm:pb-6 flex-shrink-0 ${isPremium ? (isDark ? 'bg-amber-950/20' : 'bg-amber-50/50') : (isDark ? 'bg-blue-950/20' : 'bg-blue-50/50')}`}>
          ...
          <button type="button" onClick={onClose} aria-label="Fechar detalhes" className={`absolute top-5 sm:top-6 right-5 sm:right-6 w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isDark ? 'bg-black/20 text-white hover:bg-black/40' : 'bg-black/5 text-slate-700 hover:bg-black/10'}`}>
            <Icon name="x" size={20} />
          </button>
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <div className={`inline-flex max-w-full text-[9px] sm:text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full border safe-text ${isPremium ? isDark ? 'bg-amber-500/15 border-amber-500/30 text-amber-400' : 'bg-amber-100 border-amber-300 text-amber-800' : isDark ? 'bg-white/10 border-white/20 text-zinc-300' : 'bg-slate-100 border-slate-300 text-slate-600'}`}>
              {service.tag}
            </div>
            {service.popular && (
          ...
          <h3 id="service-modal-title" className={`font-display text-xl sm:text-2xl leading-tight mb-2 safe-text ${isDark ? 'text-white' : 'text-slate-900'}`}>{service.title}</h3>
        ...
        <div className={`flex-1 overflow-y-auto p-5 sm:p-8 pt-5 sm:pt-6 space-y-6 scrollbar-hide ${isDark ? 'text-zinc-300' : 'text-slate-700'}`}>
          <p className="text-sm leading-relaxed font-medium safe-text">{service.desc}</p>
            ...
            <div className="space-y-4">
              {service.details.split('\n').map((line: string, i: number) => (
                <div key={i} className="flex items-start gap-3">
                  ...
                  <span className={`text-sm leading-relaxed safe-text ${isDark ? 'text-zinc-300' : 'text-slate-600'}`}>{line}</span>
                </div>
              ))}
            </div>
          </div>
...
const ServiceCard = memo(({ service, isInCart, onToggle, isDark, T, lang, isPremium = false, onOpenModal }: any) => {
  return (
    <button
      ...
      aria-label={`${service.title} - ${formatMoney(service.price, lang)}`}
      className={`relative w-full text-left rounded-[1.5rem] sm:rounded-[2rem] border transition-all duration-300 overflow-hidden card-hover cursor-pointer flex flex-col mobile-compact-card safe-card ${isInCart
        ? isPremium
    ...
    >
      ...
      <div className="p-4 sm:p-6 lg:p-7 flex-1 flex flex-col min-w-0">
        <div className="flex items-start gap-3 sm:gap-4 mb-4 sm:mb-5 min-w-0">
          <div className={`w-11 h-11 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center border shrink-0 ${isPremium ? isDark ? 'bg-amber-500/12 border-amber-500/25 text-amber-400' : 'bg-amber-50 border-amber-200 text-amber-600' : isDark ? 'bg-white/8 border-white/10 text-zinc-200' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
            <Icon name={service.icon} size={26} />
          </div>
          <div className="flex-1 min-w-0 pr-4">
            <h3 className={`text-base sm:text-lg font-display leading-tight mb-1.5 safe-text ${isDark ? 'text-white' : 'text-slate-900'}`}>{service.title}</h3>
            <p className={`text-[12px] sm:text-[13px] leading-relaxed line-clamp-2 safe-text ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>{service.desc}</p>
          </div>
        </div>

        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mt-auto pt-4 sm:pt-6 min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            <div className={`inline-flex max-w-full text-[9px] sm:text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full border safe-text ${isPremium ? isDark ? 'bg-amber-500/10 border-amber-500/25 text-amber-400' : 'bg-amber-50 border-amber-200 text-amber-700' : isDark ? 'bg-white/6 border-white/10 text-zinc-400' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
              {service.tag}
            </div>
          </div>
          ...
          <div className="text-left sm:text-right shrink-0 max-w-full">
            ...
            <p className={`font-display text-lg sm:text-xl leading-none safe-text ${isDark ? 'text-white' : 'text-slate-900'}`}>{formatMoney(service.price, lang)}</p>
          </div>
        </div>
      </div>
    </button>
...
export default function App() {
  ...
  return (
    <>
      ...
      <main className={`min-h-screen relative z-10 pb-40 sm:pb-44 px-4 sm:px-5 md:px-8 max-w-5xl mx-auto overflow-x-hidden`}>

        {step !== 4 && (
          <header className="pt-10 pb-8 md:pt-14 md:pb-12">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-5 min-w-0">
              <button type="button" onClick={() => setStep(0)} className="group text-left min-w-0 max-w-full" aria-label="Voltar para o início">
                <h1 className={`font-display text-2xl sm:text-3xl md:text-4xl leading-tight safe-text mb-2 transition-opacity group-hover:opacity-80 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  Thalyson Massagens
                </h1>
              ...
              <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                <button type="button" aria-label={lang === 'pt' ? 'Trocar idioma para inglês' : 'Switch language to Portuguese'} onClick={() => setLang(l => l === 'pt' ? 'en' : 'pt')} className={`relative h-11 w-11 flex items-center justify-center rounded-2xl border transition-all ${isDark ? 'border-white/10 bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white' : 'border-black/8 bg-black/4 text-slate-500 hover:text-slate-800'}`}>
                ...
                <button type="button" aria-label="Abrir Instagram" onClick={() => openExternal('instagram')} className={`h-11 w-11 flex items-center justify-center rounded-2xl border transition-all ${isDark ? 'border-white/10 bg-white/5 text-pink-400 hover:bg-white/10' : 'border-black/8 bg-black/4 text-pink-600 hover:text-pink-700'}`}>
                  <Icon name="instagram" size={20} />
                </button>
                <button type="button" aria-label="Abrir configurações" onClick={() => setMenuOpen(true)} className={`h-11 w-11 flex items-center justify-center rounded-2xl border transition-all ${isDark ? 'border-white/10 bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white' : 'border-black/8 bg-black/4 text-slate-500 hover:text-slate-800'}`}>
        ...
        <div>
          ...
          {step === 0 && (
            <section className="animate-fade-up space-y-16">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 items-center min-w-0">
                <div>
                  <h2 className={`font-display text-3xl sm:text-4xl md:text-5xl leading-[1.15] mb-5 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                ...
                <div className={`rounded-[2rem] border relative overflow-hidden safe-card ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                  <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 via-transparent to-indigo-500/10 pointer-events-none" />
                  <div className="relative grid grid-cols-1 sm:grid-cols-[160px_1fr] gap-5 p-5 sm:p-6 items-center">
                    <div className={`relative aspect-[4/5] w-full max-w-[180px] mx-auto sm:mx-0 rounded-[1.5rem] overflow-hidden border ${isDark ? 'border-white/10 bg-white/6' : 'border-slate-200 bg-slate-100'}`}>
                      {CONFIG.PROFILE_IMAGE_URL ? (
                        <img
                          src={CONFIG.PROFILE_IMAGE_URL}
                          alt="Foto profissional de Thalyson"
                          className="w-full h-full object-cover"
                          loading="lazy"
                        />
                      ) : (
                        <div className="w-full h-full flex flex-col items-center justify-center text-center p-5">
                          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 ${isDark ? 'bg-blue-500/15 text-blue-300' : 'bg-blue-50 text-blue-600'}`}>
                            <Icon name="camera" size={26} />
                          </div>
                          <p className={`text-xs font-semibold uppercase tracking-widest leading-relaxed ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
                            Sua foto aqui
                          </p>
                        </div>
                      )}
                    ...
                    <div className="min-w-0 text-center sm:text-left">
                      <p className={`text-[10px] uppercase font-semibold tracking-widest mb-2 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                        Atendimento com presença e cuidado
                      </p>
                      <h3 className={`font-display text-xl sm:text-2xl leading-tight mb-3 safe-text ${isDark ? 'text-white' : 'text-slate-900'}`}>
                        Um espaço visual para gerar confiança antes do agendamento.
                      </h3>
                      <p className={`text-sm leading-relaxed safe-text ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>
                        Troque o valor de <strong>CONFIG.PROFILE_IMAGE_URL</strong> pela URL da sua foto. O card já está responsivo, com proporção segura e sem cortar texto no celular.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className={`grid grid-cols-1 sm:flex p-2 rounded-2xl border w-full sm:w-fit mx-auto shadow-sm gap-2 sm:gap-0 ${isDark ? 'bg-white/4 border-white/8' : 'bg-slate-50 border-slate-200'}`}>
                {[
                  { id: 'single', label: T.tab_single, icon: 'user' },
                  { id: 'packs', label: T.tab_packs, icon: 'package' }
                ].map(tab => (
                  <button type="button" key={tab.id} aria-pressed={activeTab === tab.id} onClick={() => setActiveTab(tab.id)}
                    className={`flex items-center justify-center gap-3 px-4 sm:px-6 py-3 rounded-xl text-xs font-semibold uppercase tracking-wider transition-all duration-300 ${activeTab === tab.id
                ...
                {activeTab === 'single' ? (
                  <div className="space-y-8 sm:space-y-14">
                    {categoryConfig.map(cat => {
                      ...
                      return (
                        <div key={cat.id} className="rounded-[1.75rem] sm:rounded-[2.5rem] overflow-hidden border safe-card" style={{ borderColor: cfg.borderColor, background: isDark ? cfg.bg : cfg.lightBg }}>
                          <div className="px-5 sm:px-8 py-5 sm:py-6 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-5 border-b text-center sm:text-left" style={{ borderColor: cfg.borderColor }}>
                            ...
                            <div>
                              <h3 className={`font-display text-xl sm:text-2xl leading-tight mb-1.5 safe-text ${isDark ? 'text-white' : 'text-slate-900'}`}>{cat.title}</h3>
                              <p className={`text-sm safe-text ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>{cat.desc}</p>
                            </div>
              ...
              <div className="py-12 border-t border-b" style={{ borderColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)' }}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
                  <h3 className={`font-display text-2xl sm:text-3xl safe-text ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.reviews_title}</h3>
                  <div className="hidden md:flex gap-3">
                    {['chevron-left', 'chevron-right'].map((dir, i) => (
                      <button type="button" aria-label={i === 0 ? "Ver avaliações anteriores" : "Ver próximas avaliações"} key={dir} onClick={() => reviewScrollRef.current?.scrollBy({ left: i === 0 ? -360 : 360, behavior: 'smooth' })}
                ...
                <div ref={reviewScrollRef} className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide gap-5 pb-5 -mx-5 px-5" style={{ scrollbarWidth: 'none' }}>
                  {DATA.reviews.map((r: Review, i: number) => (
                    <div key={i} className="snap-center shrink-0 w-[82vw] sm:w-80 md:w-96 h-auto">
              ...
              <div className="max-w-3xl mx-auto pb-8 min-w-0">
                ...
                <div className={`rounded-[1.5rem] sm:rounded-[2rem] border overflow-hidden safe-card ${isDark ? 'bg-white/3 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
          ...
          ═══════════════════════════════════════════════════════ */}
          {step === 1 && (
            <section className="animate-fade-up max-w-xl mx-auto space-y-8 sm:space-y-10 min-w-0">
              <div className="text-center">
                <h2 className={`font-display text-3xl sm:text-4xl md:text-5xl mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.location_title}</h2>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                {[
                  ...
                  <button type="button" key={x.id} aria-pressed={booking.locationType === x.id} onClick={() => setBooking(b => ({ ...b, locationType: x.id as any }))}
                    className={`py-5 sm:py-6 px-4 sm:px-3 rounded-2xl sm:rounded-3xl flex flex-row sm:flex-col items-center justify-start sm:justify-center gap-3 transition-all duration-300 border ${booking.locationType === x.id
              ...
              <div className={`p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border space-y-5 sm:space-y-6 safe-card ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                {booking.locationType === 'motel' && (
                  <div className={`p-5 sm:p-6 rounded-2xl border flex items-start gap-4 sm:gap-5 animate-fade-up safe-card ${isDark ? 'bg-white/4 border-white/8' : 'bg-slate-50 border-slate-200'}`}>
          ...
          ═══════════════════════════════════════════════════════ */}
          {step === 2 && (
            <section className="animate-fade-up max-w-3xl mx-auto space-y-8 sm:space-y-10 min-w-0">
              <div className="text-center">
                <h2 className={`font-display text-3xl sm:text-4xl md:text-5xl mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.select_time_title}</h2>
              </div>

              <div className={`p-4 sm:p-5 rounded-2xl sm:rounded-3xl border safe-card ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <div className="flex flex-wrap gap-2.5 min-w-0">
                  {booking.cart.map(item => (
                    <span key={item.id} className={`inline-flex items-center gap-2 text-xs sm:text-sm font-medium px-3 sm:px-4 py-2 rounded-xl border safe-text ${isDark ? 'bg-blue-500/10 border-blue-500/25 text-blue-300' : 'bg-blue-50 border-blue-200 text-blue-700'}`}>
              ...
              <div className="relative">
                ...
                <div ref={dateScrollRef} className="flex gap-3 overflow-x-auto snap-x px-2 py-4 scrollbar-hide">
                  {daysArray.map((d, idx) => {
                    ...
                    return (
                      <button type="button" key={idx} aria-pressed={!!isSel} onClick={() => { setBooking(b => ({ ...b, date: d.toISOString(), time: null })); vibrate(30); }}
                ...
                <button type="button" aria-label="Próximas datas" onClick={() => scrollDates('right')} className={`hidden md:flex absolute -right-12 top-1/2 -translate-y-1/2 z-20 w-10 h-10 items-center justify-center rounded-xl border transition-all ${isDark ? 'bg-white/5 border-white/10 text-zinc-400 hover:text-white hover:bg-white/10' : 'bg-white border-slate-200 text-slate-500 hover:text-slate-800 shadow-sm'} shadow-lg`}><Icon name="chevron-right" size={20} /></button>
              </div>
                    ...
                    <div key={group.key}>
                      ...
                      <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-6 gap-2.5 sm:gap-3">
                        {group.slots.map(t => {
                          ...
                          return (
                            <button type="button" key={t} aria-pressed={!!isSel} onClick={() => { setBooking(b => ({ ...b, time: t })); vibrate(30); }}
                              className={`relative flex flex-col items-center justify-center py-3 rounded-xl border text-sm sm:text-base font-semibold transition-all duration-200 ${isSel
          ...
          ═══════════════════════════════════════════════════════ */}
          {step === 3 && (
            <section className="animate-fade-up space-y-6 sm:space-y-8 max-w-5xl mx-auto min-w-0">
              <SmartTimer isDark={isDark} text={T.timer_text} />

              <div className={`p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border safe-card ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {DATA.extras.map((ex: any) => {
                    ...
                    return (
                      <button type="button" key={ex.id} onClick={() => { setBooking(b => ({ ...b, extras: { ...b.extras, [ex.id]: !b.extras[ex.id] } })); vibrate(30); }}
                        aria-pressed={!!isActive}
                        className={`flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 p-4 sm:p-5 rounded-2xl border cursor-pointer transition-all duration-200 ${isActive ? 'bg-blue-600/12 border-blue-500/50 shadow-sm' : isDark ? 'bg-white/3 border-white/8 hover:bg-white/6 hover:border-white/14' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.5fr)_minmax(320px,1fr)] gap-6 sm:gap-8">
                <div className={`p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border safe-card ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="space-y-3">
                        {booking.cart.map((item, i) => (
                          <div key={i} className={`flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4 text-sm sm:text-base font-medium border-b pb-3 last:border-0 last:pb-0 ${isDark ? 'border-white/6 text-white' : 'border-slate-100 text-slate-900'}`}>
                      ...
                      <div className={`border-t pt-6 ${isDark ? 'border-white/6' : 'border-slate-100'}`}>
                        ...
                        <div className="space-y-2">
                          {Object.keys(booking.extras || {}).filter(k => booking.extras[k]).map(k => {
                            ...
                            const price = booking.cart.some(i => i.type === 'pack') ? Math.floor(ex.price * 0.8) : ex.price;
                            return (
                              <div key={k} className={`flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-4 text-sm sm:text-base font-medium mb-1.5 ${isDark ? 'text-zinc-300' : 'text-slate-700'}`}>
                    ...
                    <div className={`border-t pt-6 space-y-3 ${isDark ? 'border-white/8' : 'border-slate-200'}`}>
                      <div className={`flex justify-between gap-4 text-sm sm:text-base font-medium ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>
                        <span>{T.subtotal}</span>
                        <span className="shrink-0 whitespace-nowrap">{formatMoney(financials.sub, lang)}</span>
                      </div>
                      {booking.appliedCoupon && (
                        <div className={`flex justify-between gap-4 text-sm sm:text-base font-medium ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>
                          <span className="flex items-center gap-2 min-w-0 break-words"><Icon name="gift" size={16} className="shrink-0" />{booking.appliedCoupon.title}</span>
                          <span className="shrink-0 whitespace-nowrap">-{formatMoney(financials.disc, lang)}</span>
                        </div>
                      )}
                      {financials.mediaDisc > 0 && (
                        <div className={`flex justify-between gap-4 text-sm sm:text-base font-medium ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                      ...
                      {financials.pixDisc > 0 && (
                        <div className={`flex justify-between gap-4 text-sm sm:text-base font-medium ${isDark ? 'text-zinc-300' : 'text-slate-700'}`}>
                      ...
                      {financials.rushFee > 0 && (
                        <div className={`flex justify-between gap-4 text-sm sm:text-base font-medium ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>
                          <span className="flex items-center gap-2 min-w-0 break-words"><Icon name="car" size={16} className="shrink-0" />{T.msg_rush_fee}</span>
                          <span className="shrink-0 whitespace-nowrap">+{formatMoney(financials.rushFee, lang)}</span>
                        </div>
                      )}

                      <div className={`flex flex-col sm:flex-row sm:justify-between gap-3 sm:gap-4 sm:items-end pt-5 mt-2 border-t ${isDark ? 'border-white/8' : 'border-slate-100'}`}>
                        ...
                        <div className="text-right min-w-0">
                          <p className="font-display text-3xl sm:text-4xl text-gradient-blue safe-text">{formatMoney(financials.total, lang)}</p>
                ...
                <div className="space-y-6">
                  <div className={`p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border safe-card ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                  ...
                  <div className={`p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border safe-card ${hasErrorGlobal && !booking.payment ? 'animate-shake' : ''} ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        <button key={p.id} onClick={() => {
                          ...
                          className={`w-full flex items-center gap-3 sm:gap-4 p-4 min-h-16 rounded-2xl border transition-all duration-200 ${booking.payment === p.id ? 'bg-blue-600 border-blue-400 text-white shadow-lg shadow-blue-900/20' : isDark ? 'bg-white/4 border-white/8 text-zinc-300 hover:bg-white/8 hover:border-white/14' : 'bg-slate-50 border-slate-200 text-slate-700 hover:border-slate-300'}`}>
                  ...
                  <div className={hasErrorGlobal && !booking.termsAccepted ? 'animate-shake' : ''}>
                    <button onClick={() => setTermsOpen(true)}
                      className={`w-full flex items-center justify-between gap-4 p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border safe-card cursor-pointer transition-all duration-300 ${booking.termsAccepted ? isDark ? 'bg-emerald-600/15 border-emerald-500/50' : 'bg-emerald-50 border-emerald-300' : isDark ? 'bg-white/4 border-white/8 hover:bg-white/8 hover:border-white/14' : 'bg-white border-slate-200 hover:border-slate-300 shadow-sm'}`}>
                      <div className="flex items-center gap-4 min-w-0">
          ...
          ═══════════════════════════════════════════════════════ */}
          {step === 4 && (
            <section className="min-h-[80vh] flex flex-col items-center justify-center text-center animate-scale-in max-w-md mx-auto px-4 sm:px-5 pt-10 sm:pt-12 min-w-0">
              ...
              <div className={`w-full p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border mb-8 sm:mb-10 text-left space-y-3 safe-card ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
        ...
        <nav className="fixed bottom-0 inset-x-0 px-4 sm:px-5 pb-5 pt-3 z-40 animate-slide-up pointer-events-none">
          <div className={`max-w-5xl mx-auto pointer-events-auto rounded-[2rem] overflow-hidden border shadow-[0_-10px_40px_rgba(0,0,0,0.25)] ${isDark ? 'bg-[#181c25] border-zinc-700' : 'bg-white border-slate-300'}`}>
            <div className="flex items-center justify-between gap-3 px-4 py-3 sm:px-6 sm:py-4">
              ...
              {step > 0 && (
                <button type="button" aria-label="Voltar etapa" onClick={() => { setStep(s => s - 1); vibrate(30); }}
              ...
              <div className="flex-1 min-w-0 pl-1">
                <p className={`text-[10px] sm:text-[11px] uppercase font-bold tracking-widest mb-0.5 break-words ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
              ...
              <button type="button" aria-label={step === 3 ? T.finish_btn : T.next_btn} onClick={handleNextStep}
      ...
      {/* ── TERMS MODAL ── */}
      {termsOpen && (
        <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center p-3 sm:p-4 bg-black/90 backdrop-blur-md animate-fade-in" role="dialog" aria-modal="true" aria-labelledby="terms-title">
          <div className={`relative w-full max-w-xl max-h-[88vh] rounded-[2rem] sm:rounded-[2.5rem] flex flex-col border shadow-2xl animate-slide-up safe-card ${isDark ? 'bg-[#11141a] border-zinc-700' : 'bg-white border-slate-300'}`}>
            <div className={`flex items-center justify-between p-5 sm:p-8 border-b shrink-0 ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
              <h3 id="terms-title" className={`font-display text-xl sm:text-2xl safe-text ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.rules_complete}</h3>
              <button type="button" aria-label="Fechar regras" onClick={() => setTermsOpen(false)} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${isDark ? 'text-zinc-400 hover:text-white hover:bg-white/10' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100'}`}>
                <Icon name="x" size={22} />
              </button>
            </div>
            <div className="overflow-y-auto flex-1 p-4 sm:p-8 space-y-3 sm:space-y-4">
      ...
      {/* ── WELCOME POPUP ── */}
      {welcomePopup && (
        <div className="fixed inset-0 z-[90] flex items-end sm:items-center justify-center p-3 sm:p-5 bg-black/95 backdrop-blur-md animate-fade-in">
          <div className={`relative w-full max-w-md rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 border shadow-2xl safe-card animate-scale-in overflow-hidden ${isDark ? 'bg-[#11141a] border-zinc-700' : 'bg-white border-slate-300'}`}>
      ...
      {/* ── LEVEL UP POPUP ── */}
      {levelUpPopup && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center p-3 sm:p-5 bg-black/95 backdrop-blur-md animate-fade-in">
          <div className={`relative w-full max-w-md rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 text-center border shadow-2xl safe-card animate-scale-in overflow-hidden ${isDark ? 'bg-[#11141a] border-amber-700/50' : 'bg-white border-amber-300'}`}>
