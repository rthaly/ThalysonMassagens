...
const CONFIG = {
  ...
  STORAGE_KEY: '@thaly_app_v20_optimized', // Mantida para não perder XP
...
const generateReviews = (isPT: boolean): Review[] => {
  ...
  const seriousReviews = [
    ...
    { n: "Marcelo", loc: isPT ? "SP - Centro" : "SP - Downtown", t: isPT ? "Fui sem expectativa e saí surpreendido. A massagem lingam foi executada com uma técnica precisa e respeitosa. O prazer foi intense e genuíno." : "I went without expectation and left surprised. The lingam massage was executed with precise and respectful technique. The pleasure was intense and genuine." },
...
export default function App() {
  ...
  const [user, setUser] = useState<UserData>({
    ...
    ordersCount: 0, // Inicia limpo, UI mostra 69 + este valor
  ...
  // ==========================================
  // BLINDAGEM DE CACHE - PREVINE TELA BRANCA
  // ==========================================
  useEffect(() => {
    if (!isClient) return;
    ...
    try {
      ...
      if (stored) {
        ...
        // Verifica se o item do rascunho ainda existe nos dados de hoje
        ...
        // Se o item existe e a data é válida, carrega o rascunho
        if (draftItemExists && parsed.bookingDraft) {
          ...
          if (draftDate > new Date()) {
           ...
           // CACHE INVÁLIDO OU ANTIGO: Limpa apenas o rascunho para não travar
           setStep(0);
        }
      }
    } catch (e) {
      console.error('Falha ao ler cache - Resetando apenas sessão atual', e);
    }
...
*${lang === 'pt' ? 'Aguardando confirmação e valor do Uber...' : 'Awaiting confirmation and Uber fee...'}*
  ...
  const isStepValid = useCallback(() => {
    if (step === 0 && !booking.item) return false;
    if (step === 1 && (!booking.date || !booking.time)) return false;
    if (step === 2) {
      if (!user.name || user.name.trim().length < 3) return false;
      if (booking.locationType === 'home' && !validateAddress(booking.address)) return false;
      if (booking.locationType === 'hotel' && (!booking.address.placeName || !booking.address.city)) return false;
    }
    if (step === 3 && (!booking.payment || !booking.termsAccepted)) return false;
    return true;
  ...
  const validateStep = useCallback(() => {
    ...
    if (step === 2) {
      ...
      if (booking.locationType === 'home' && !validateAddress(booking.address)) {
        addToast(T.toast_fill_addr, "error");
        return false;
      }
      if (booking.locationType === 'hotel' && (!booking.address.placeName || !booking.address.city)) {
  ...
  const finishBooking = () => {
    ...
    const newOrdersCount = user.ordersCount + 1; // Soma +1 na base
      ...
      <main className={`min-h-screen relative z-10 pb-32 px-6 md:px-12 max-w-6xl mx-auto ${isDark ? 'text-white' : 'text-black'}`}>
        {step !== 4 && (
          <header className="pt-12 pb-6 flex items-start justify-between">
            <div className="flex flex-col cursor-pointer" onClick={() => setStep(0)} title="Voltar ao Início">
        ...
        <nav className="fixed bottom-0 left-0 right-0 p-4 md:p-6 z-40 pointer-events-none" aria-label="Navegação da Reserva">
          ...
          `}>
            <button
              ...
              className={`w-12 h-12 flex items-center justify-center rounded-full transition-colors ${isDark ? 'hover:bg-zinc-800 text-zinc-400' : 'hover:bg-slate-100 text-slate-600'}`}
