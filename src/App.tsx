...
// Cupons podem vir do localStorage/cache. Esta camada deixa o fluxo imune a cupom antigo,
// objeto corrompido, código salvo como string, duplicidade e re-render desnecessário.
const makeCouponKey = (coupon) => {
  if (!coupon) return '';
  if (typeof coupon === 'string' || typeof coupon === 'number') return String(coupon).trim().toUpperCase();
  if (typeof coupon !== 'object') return '';
  return String(coupon.code ?? coupon.id ?? coupon.title ?? '').trim().toUpperCase();
};
...
const normalizeCoupon = (coupon) => {
  ...
  const source = typeof coupon === 'object'
    ? coupon
    : { id: couponKey, code: couponKey, title: couponKey, val: couponKey === 'BEMVINDO10' ? 10 : 0 };

  const code = makeCouponKey(source);
  const rawValue = Number(source.val ?? source.value ?? source.discount ?? 0);
  const fallbackValue = code === 'BEMVINDO10' ? 10 : 0;
  const safeValue = Number.isFinite(rawValue) && rawValue > 0 ? rawValue : fallbackValue;

  if (!code || !Number.isFinite(safeValue) || safeValue <= 0) return null;

  const val = Math.min(999, Math.max(0, Math.round(safeValue)));

  return {
    id: String(source.id || code),
    code,
    val,
    title: sanitizeInput(String(source.title || code)) || code
  };
};
...
// Resolve somente cupons realmente disponíveis na lista informada.
// Isso evita que BEMVINDO10 antigo, usado ou corrompido volte a aplicar desconto sozinho.
const resolveCoupon = (couponOrCode, couponList = []) => {
  const key = makeCouponKey(couponOrCode);
  if (!key) return null;

  return normalizeCouponList(couponList).find(item => makeCouponKey(item) === key) || null;
};

const normalizeUsedCouponKeys = (usedCoupons = []) => {
  const list = Array.isArray(usedCoupons) ? usedCoupons : [usedCoupons];
  return Array.from(new Set(list.map(makeCouponKey).filter(Boolean)));
};

const getAvailableCoupons = (sourceUser = {}) => {
  const used = new Set(normalizeUsedCouponKeys(sourceUser.usedCoupons));
  return normalizeCouponList(sourceUser.coupons).filter(coupon => !used.has(makeCouponKey(coupon)));
};
...
function ThalysonBookingApp() {
  ...
  useEffect(() => {
    ...
    const safeLoadedCoupon = resolveCoupon(loadedBooking.appliedCoupon, getAvailableCoupons(loadedUser));
    loadedBooking = {
      ...loadedBooking,
      appliedCoupon: safeLoadedCoupon ? makeCouponKey(safeLoadedCoupon) : null
    };
    ...
    if (isClient && dataLoaded) {
      ...
      try {
        ...
        const save = {
          ...
          user: {
            ...user,
            coupons: normalizeCouponList(user.coupons),
            usedCoupons: normalizeUsedCouponKeys(user.usedCoupons),
  ...
  const availableCoupons = useMemo(() => getAvailableCoupons(user), [user.coupons, user.usedCoupons]);
  ...
  const financials = useMemo(() => {
    ...
    const selectedCoupon = resolveCoupon(booking.appliedCoupon, availableCoupons);
  ...
  }, [booking.cart, booking.extras, booking.appliedCoupon, DATA.extras, booking.payment, booking.mediaAllowed, booking.time, booking.locationType, availableCoupons]);
  ...
  const selectedCouponForView = useMemo(() => resolveCoupon(booking.appliedCoupon, availableCoupons), [booking.appliedCoupon, availableCoupons]);
  ...
  const handleClaimWelcomeCoupon = useCallback(() => {
    ...
    try {
      ...
      setUser(prev => {
        ...
        const safePrev = prev && typeof prev === 'object'
          ? prev
          : { name: '', xp: 0, coupons: [], usedCoupons: [], hasSeenWelcome: false, ordersCount: 92, lastActivity: new Date().toISOString() };

        const usedKeys = new Set(normalizeUsedCouponKeys(safePrev.usedCoupons));

        const currentCoupons = normalizeCouponList(safePrev.coupons);

        const couponKey = makeCouponKey(coupon);

        const nextCoupons = usedKeys.has(couponKey)

          ? currentCoupons

          : hasCouponInList(currentCoupons, coupon)
        ...
        return {
          ...
          usedCoupons: Array.from(usedKeys),
      ...
      // O resgate não toca no booking. O usuário aplica o cupom depois, na etapa de resumo.
