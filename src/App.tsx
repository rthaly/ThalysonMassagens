...
import {
  ...
  Crown, LayoutList, Package
} from 'lucide-react';

// ==================================================================================
// 1. DADOS E CONFIGURAÇÕES (RESTAURADOS E EXPANDIDOS)
// ==================================================================================

const CONFIG = {
  PHONE: "5517991360413", 
  INSTAGRAM_URL: "https://instagram.com/thalyson.massagens", 
  STORAGE_KEY: '@thaly_app_original_v6', 
};
...
const getData = (lang) => {
    ...
    return {
        // NÍVEIS DE FIDELIDADE (MANTIDO O ORIGINAL QUE FUNCIONAVA BEM)
        levels: [
        ...
        // === ABA 1: SERVIÇOS AVULSOS (RESTAURADO TEXTO ORIGINAL) ===
        services: [
            { 
              id: 'relaxante', 
              min: 60, 
...
4. FINAL: Você goza no final, sem pressa.` : "Complete experience: Relaxing + Body to Body + Lingam."
        ...
        // === ABA 2: PLANOS E PACOTES (NOVIDADE SOLICITADA) ===
        plans: [
            {
                id: 'pack_relax',
                type: 'pack',
                title: isPT ? "Pack Relax (4 Sessões)" : "Relax Pack (4x)",
                price: 320, // 90*4 = 360. Desconto real de 40.
                fullPrice: 360,
                savings: 40,
            ...
            },
            {
                ...
                price: 540, // 200*3 = 600. Desconto real de 60.
                fullPrice: 600,
                savings: 60,
            ...
            },
            {
                id: 'vip_club',
                type: 'subscription',
                title: isPT ? "Assinatura VIP Mensal" : "VIP Monthly",
                price: 350, // Preço fixo mensal
                fullPrice: 450, // Valor ancorado
                savings: 100,
        ...
        // AVALIAÇÕES REAIS (LISTA COMPLETA RESTAURADA)
        reviews: [
        ...
        text: {
            ...
            subtitle: isPT ? "Escolha como quer relaxar:" : "Choose service:",
            
            // ABAS
            tab_single: isPT ? "Sessão Avulsa" : "Single",
            tab_packs: isPT ? "Pacotes & Planos" : "Packs",
            ...
            zap: {
              ...
              section_plan: isPT ? "🏆 *QUERO O PACOTE:*" : "🏆 *I WANT PACK:*",
...
// ==================================================================================
// 2. COMPONENTES VISUAIS (DESIGN "ACONCHEGANTE" / COZY)
// ==================================================================================
...
const LoadingScreen = ({ isDark, text }) => (
    <div className={`fixed inset-0 z-[200] flex flex-col items-center justify-center ${isDark ? 'bg-zinc-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
        <div className="relative">
            <div className="w-20 h-20 rounded-full bg-amber-500/80 flex items-center justify-center animate-pulse shadow-2xl shadow-amber-500/30">
                <span className="text-2xl font-black text-black">TM</span>
            </div>
        </div>
        ...
        <div className={`relative overflow-hidden rounded-3xl p-5 mb-6 border shadow-lg transition-all ${isDark ? 'bg-gradient-to-br from-zinc-900 to-black border-zinc-800' : 'bg-white border-slate-100'}`}>
            ...
            {nextLevel ? (
                <>
                    ...
                    <div className={`h-3 w-full rounded-full overflow-hidden ${isDark ? 'bg-zinc-800' : 'bg-slate-100'}`}>
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState('single'); // 'single' ou 'packs'
  ...
  const [booking, setBooking] = useState({
    type: 'single', // 'single' ou 'pack'
    item: null, // O serviço ou plano selecionado
    extras: {}, date: null, time: null, locationType: 'home', 
  ...
  useEffect(() => {
    ...
    try {
        ...
        if (s) {
            ...
            setUser(p => ({...p, coupons: [{ id: 'welcome', val: 10, title: '🎁 Boas Vindas' }]}));
        }
    } catch (e) {
  ...
  const finishBooking = () => {
    ...
    const oldXP = user.xp;
    // Pacotes dão mais XP
    const xpGain = booking.type === 'pack' ? getFinancials.total * 1.5 : getFinancials.total;
  ...
  const openZap = () => {
    ...
    // MENSAGEM DIFERENTE PARA PACOTE OU AVULSO
...
📅 Início: ${dateStr} às ${booking.time}
  ...
  return (
    <div className={`h-[100dvh] w-full font-sans flex flex-col overflow-hidden transition-colors duration-500 ${isDark ? 'bg-zinc-950 text-zinc-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <header className={`h-16 px-6 flex items-center justify-between z-20 shrink-0 ${isDark ? 'bg-zinc-950 border-b border-zinc-800' : 'bg-white border-b border-slate-200'}`}>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-amber-500 flex items-center justify-center font-bold text-black text-xs shadow-lg shadow-amber-500/30">TM</div>
      ...
      <main ref={scrollRef} className="flex-1 overflow-y-auto overflow-x-hidden p-6 pb-32 scroll-smooth relative">
        ...
        <div className="max-w-md mx-auto space-y-8 pt-2">
          ...
          {/* STEP 0: SERVICES & PLANS */}
          {step === 0 && (
            <div className="animate-fade-in">
              <div className="mb-6">
                <h1 className="text-2xl font-bold mb-1">{T.welcome} <span className="text-amber-500">{user.name ? user.name.split(' ')[0] : 'Visitante'}</span></h1>
                ...
                <button onClick={() => setReviewsOpen(true)} className="w-full mt-2 inline-flex items-center justify-center gap-2 px-3 py-3 rounded-xl bg-zinc-800/50 text-zinc-300 text-sm font-bold cursor-pointer hover:bg-zinc-800 active:scale-95 transition-transform border border-zinc-800">
                   <Star size={14} fill="currentColor" className="text-amber-400"/> {T.reviews_count}
                </button>
              </div>

              {/* TABS (NOVA FUNCIONALIDADE, DESIGN ORIGINAL) */}
              <div className={`flex p-1 rounded-2xl mb-6 ${isDark ? 'bg-zinc-900' : 'bg-slate-200'}`}>
                  <button onClick={()=>setActiveTab('single')} className={`flex-1 py-3 rounded-xl text-xs font-bold transition-all ${activeTab==='single' ? (isDark?'bg-zinc-800 text-white shadow':'bg-white text-black shadow') : 'opacity-50'}`}>
                      <LayoutList size={16} className="inline mb-0.5 mr-2"/> {T.tab_single}
                  </button>
                  <button onClick={()=>setActiveTab('packs')} className={`flex-1 py-3 rounded-xl text-xs font-bold transition-all ${activeTab==='packs' ? (isDark?'bg-zinc-800 text-white shadow':'bg-white text-black shadow') : 'opacity-50'}`}>
                      <Package size={16} className="inline mb-0.5 mr-2"/> {T.tab_packs}
                  </button>
              </div>

              {/* LISTA: SESSÕES AVULSAS */}
              {activeTab === 'single' && (
                  <div className="space-y-4 animate-slide-in">
                    {DATA.services.map(s => (
                      <div key={s.id} onClick={() => { setBooking(b => ({ ...b, type: 'single', item: s })); }} 
                        className={`p-5 rounded-3xl border-2 cursor-pointer transition-all active:scale-[0.98] ${booking.item?.id === s.id ? 'border-amber-500 bg-amber-500/5' : (isDark ? 'border-zinc-800 bg-zinc-900' : 'border-slate-200 bg-white')}`}
                      >
                          <div className="flex justify-between items-center mb-3">
                            <div className={`p-3 rounded-2xl ${booking.item?.id === s.id ? 'bg-amber-500 text-black' : (isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500')}`}><s.icon size={24}/></div>
                            <div className="text-right">
                          ...
                          {booking.item?.id === s.id && (
                              <div className={`mt-3 p-3 rounded-xl text-xs leading-relaxed animate-fade-in ${isDark ? 'bg-black/20 text-zinc-300' : 'bg-slate-100 text-slate-700'}`}>
                                 <div className="flex items-center gap-2 font-bold mb-1 text-amber-500"><Info size={12}/> {T.details_label}</div>
              ...
              {/* LISTA: PACOTES E PLANOS (NOVO) */}
              {activeTab === 'packs' && (
                  <div className="space-y-4 animate-slide-in">
                      {DATA.plans.map(plan => (
                          <div key={plan.id} onClick={() => { setBooking(b => ({ ...b, type: plan.type, item: plan })); }}
                              className={`relative p-5 rounded-3xl border-2 cursor-pointer transition-all active:scale-[0.98] 
                              ${booking.item?.id === plan.id ? 'border-amber-500 bg-amber-500/5' : (isDark ? 'border-zinc-800 bg-zinc-900' : 'border-slate-200 bg-white')}`}
                          >
                              {plan.tag && (
                                  <span className="absolute top-4 right-4 text-[10px] font-bold bg-amber-500 text-black px-2 py-1 rounded-md">
                                      {plan.tag}
                                  </span>
                              )}
                              <div className="flex items-center gap-3 mb-3">
                                  <div className={`p-3 rounded-2xl ${booking.item?.id === plan.id ? 'bg-amber-500 text-black' : (isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500')}`}>
                                      <plan.icon size={24}/>
                                  </div>
                                  <div>
                                      <h3 className="font-bold text-lg leading-none">{plan.title}</h3>
                                      <p className="text-[10px] opacity-60 mt-1 uppercase tracking-wider">{plan.type === 'pack' ? 'Pacote Fechado' : 'Assinatura'}</p>
                                  </div>
                              </div>
                              
                              <p className={`text-sm mb-4 ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>{plan.details}</p>

                              <div className="flex items-end gap-2">
                                  <span className="text-2xl font-bold">{T.currency} {plan.price}</span>
                                  <span className="text-sm line-through opacity-40 mb-1">{T.currency} {plan.fullPrice}</span>
                                  <span className="text-xs text-green-500 font-bold mb-1 ml-auto">Economize {T.currency}{plan.savings}</span>
                              </div>
                          </div>
                      ))}
                  </div>
                ...
                {[...Array(14)].map((_, i) => { 
                  ...
                  return (
                    <button key={i} onClick={() => setBooking(b => ({ ...b, date: d, time: null }))} 
                      className={`min-w-[70px] h-20 rounded-2xl flex flex-col items-center justify-center gap-1 border-2 transition-all flex-shrink-0 active:scale-95 ${isSel ? 'bg-amber-500 border-amber-500 text-black shadow-lg shadow-amber-500/20' : (isDark ? 'bg-zinc-900 border-zinc-800 text-zinc-500' : 'bg-white border-slate-200 text-slate-400')}`}
                    >
                 ...
                 {availableTimes.map(t => (
                      <div key={t} className="relative group">
                         <button onClick={() => { setBooking(b => ({...b, time: t})); triggerScarcity(); }}
                             className={`w-full py-3 rounded-xl text-xs font-bold border transition-all active:scale-95 ${booking.time === t ? 'bg-amber-500 text-black border-amber-500' : (isDark ? 'bg-zinc-900 border-zinc-800 hover:border-zinc-700' : 'bg-white border-slate-200 hover:border-slate-300')}`}
                         >
                             {t}
                         </button>
                      </div>
            ...
            <div className="animate-slide-in">
              ...
              <div className="space-y-4">
                 <div>
                    ...
                    <input value={user.name} onChange={e=>setUser(u=>({...u, name: e.target.value}))} className={`w-full p-4 rounded-2xl border outline-none text-base transition-colors focus:border-amber-500 ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`} />
                 ...
                 {booking.locationType === 'home' && (
                     <div className="space-y-4 animate-fade-in">
                        <div className="grid grid-cols-[1fr_80px] gap-3">
                           <input value={booking.address.street} onChange={e=>setBooking(b=>({...b, address: {...b.address, street: e.target.value}}))} placeholder={T.input_addr} className={`p-4 rounded-2xl border outline-none focus:border-amber-500 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}/>
                           <input type="tel" value={booking.address.number} onChange={e=>setBooking(b=>({...b, address: {...b.address, number: e.target.value}}))} placeholder={T.input_num} className={`p-4 rounded-2xl border outline-none focus:border-amber-500 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}/>
                        </div>
                        <input value={booking.address.district} onChange={e=>setBooking(b=>({...b, address: {...b.address, district: e.target.value}}))} placeholder={T.input_bairro} className={`w-full p-4 rounded-2xl border outline-none focus:border-amber-500 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}/>
                        <input value={booking.address.city} onChange={e=>setBooking(b=>({...b, address: {...b.address, city: e.target.value}}))} placeholder={T.input_city} className={`w-full p-4 rounded-2xl border outline-none focus:border-amber-500 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}/>
                        <input value={booking.address.comp} onChange={e=>setBooking(b=>({...b, address: {...b.address, comp: e.target.value}}))} placeholder={T.input_comp} className={`w-full p-4 rounded-2xl border outline-none focus:border-amber-500 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}/>
                     </div>
                 )}
                 
                 {booking.locationType === 'hotel' && (
                    <div className="space-y-4 animate-fade-in">
                        <input value={booking.address.placeName} onChange={e=>setBooking(b=>({...b, address: {...b.address, placeName: e.target.value}}))} placeholder={T.input_hotel} className={`w-full p-4 rounded-2xl border outline-none focus:border-amber-500 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}/>
                        <input value={booking.address.city} onChange={e=>setBooking(b=>({...b, address: {...b.address, city: e.target.value}}))} placeholder={T.input_city} className={`w-full p-4 rounded-2xl border outline-none focus:border-amber-500 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}/>
                        <input value={booking.address.comp} onChange={e=>setBooking(b=>({...b, address: {...b.address, comp: e.target.value}}))} placeholder={T.input_room} className={`w-full p-4 rounded-2xl border outline-none focus:border-amber-500 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}/>
                    </div>
                 )}
              ...
              {booking.type === 'single' && (
                  <div className="pt-6 border-t border-dashed border-zinc-700/50 mt-6">
                     ...
                     <div className="space-y-3">
                        {DATA.extras.map(ex => (
                           <div key={ex.id} onClick={()=>setBooking(b=>({...b, extras:{...b.extras, [ex.id]: !b.extras[ex.id]}}))} className={`flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all active:scale-[0.99] ${booking.extras[ex.id] ? 'bg-amber-500/10 border-amber-500' : (isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200')}`}>
                              <div className="flex items-center gap-3">
                                 <div className={`p-2 rounded-xl ${booking.extras[ex.id] ? 'bg-amber-500 text-black' : (isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500')}`}><ex.icon size={18}/></div>
                                 <div>
                                     <p className="text-sm font-bold">{ex.label}</p>
                                     <p className="text-[10px] opacity-60">{ex.desc}</p>
                                 </div>
                              </div>
                              <span className={`text-xs font-bold ${booking.extras[ex.id] ? 'text-amber-500' : 'opacity-50'}`}>+ {T.currency} {ex.price}</span>
                           </div>
                        ))}
                     </div>
                  </div>
            ...
            <div className="animate-slide-in pb-10">
               <div className={`p-6 rounded-3xl border shadow-xl ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}>
                  <div className="flex justify-between items-center mb-4 pb-4 border-b border-dashed border-gray-500/20">
                      <div>
                          <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded mb-1 inline-block ${isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500'}`}>
                              {booking.type === 'pack' ? 'Pacote' : (booking.type === 'subscription' ? 'Assinatura' : 'Sessão')}
                          </span>
                          <span className="font-bold text-lg block">{booking.item.title}</span>
                      </div>
                      <span className="font-bold text-lg">{T.currency} {booking.item.price}</span>
                  </div>
                        ...
                        <div className="grid gap-2">
                            {user.coupons.map(c => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-lg ${isApplied ? 'bg-green-500 text-white' : 'bg-gray-500/10 text-gray-500'}`}>
                                                <Ticket size={16}/>
                                            </div>
                                            <div>
                                                <p className="font-bold text-sm">{c.title}</p>
                                                <p className={`text-[10px] ${isApplied ? 'text-green-500 font-bold' : 'opacity-60'}`}>{isApplied ? T.coupon_applied : T.coupon_select}</p>
                                            </div>
                                        </div>
                                        <span className={`block font-black text-sm ${isApplied ? 'text-green-500' : 'opacity-50'}`}>-{T.currency}{c.val}</span>
                                    </div>
                                )
                            })}
                        </div>
               ...
               <div className="mt-6">
                   ...
                   <div className="grid grid-cols-3 gap-3">
                       {[{id:'pix', l:T.pay_pix, i:QrCode}, {id:'card', l:T.pay_card, i:CreditCard}, {id:'money', l:T.pay_cash, i:Banknote}].map(p => (
                           <button key={p.id} onClick={()=>setBooking(b=>({...b, payment: p.id}))} className={`py-4 rounded-2xl border flex flex-col items-center justify-center gap-2 transition-all active:scale-95 ${booking.payment === p.id ? 'bg-amber-500 text-black border-amber-500' : (isDark ? 'bg-zinc-900 border-zinc-800 hover:border-zinc-700' : 'bg-white border-slate-200 hover:border-slate-300')}`}>
               ...
               <div className={`mt-8 p-4 rounded-2xl border flex flex-col gap-3 ${isDark ? 'bg-zinc-900/50 border-zinc-800' : 'bg-amber-50 border-amber-200'}`}>
                    ...
                    <label className="flex items-center gap-3 p-3 rounded-xl bg-black/5 cursor-pointer">
                        <input type="checkbox" checked={booking.termsAccepted} onChange={e=>setBooking(b=>({...b, termsAccepted: e.target.checked}))} className="w-5 h-5 accent-amber-500 rounded"/>
          ...
          {step === 4 && (
             <div className="flex flex-col items-center justify-center pt-10 text-center animate-scale-in">
                 <div className="w-24 h-24 bg-green-500 rounded-full flex items-center justify-center shadow-2xl shadow-green-500/30 mb-6 animate-bounce-slow">
                 ...
                 <button onClick={openZap} className="w-full py-5 bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold rounded-2xl text-lg shadow-xl flex items-center justify-center gap-3 transition-transform active:scale-95">
                     <MessageCircle size={24} fill="white"/> {T.whatsapp_btn}
                 </button>
                 <button onClick={()=>{setStep(0); setBooking({...booking, item: null, type:'single', payment: '', appliedCoupon: null, termsAccepted: false});}} className="mt-8 text-xs font-bold uppercase opacity-50 tracking-widest hover:opacity-100">{T.back_home}</button>
             </div>
          )}
         ...
         <div className="fixed bottom-6 left-6 right-6 z-50">
            <div className={`p-2 rounded-[2rem] shadow-2xl flex items-center gap-4 pr-3 backdrop-blur-xl border ${isDark ? 'bg-zinc-900/90 border-zinc-700' : 'bg-white/90 border-zinc-200'}`}>
                ...
                <button 
                    ...
                    className={`h-12 px-6 rounded-full font-bold flex items-center justify-center gap-2 transition-all ${step < 3 ? 'ml-auto' : 'w-full'} ${!canProceed() ? 'bg-zinc-500 opacity-50 cursor-not-allowed text-white' : 'bg-amber-500 text-black shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 active:scale-95'}`}
                >
         ...
         <div className="space-y-3">
            {DATA.reviews.map((r,i)=>(
               <div key={i} className={`p-4 rounded-xl border ${isDark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                   <div className="flex justify-between mb-1"><span className="font-bold text-sm text-amber-500">{r.n}</span><div className="flex text-amber-400 gap-0.5">{[...Array(r.s)].map((_,k)=><Star key={k} size={10} fill="currentColor"/>)}</div></div>
      ...
      <Modal isOpen={termsOpen} onClose={()=>setTermsOpen(false)} title={T.terms_title} isDark={isDark}>
         <div className="space-y-4">
            {T.terms_body.map((t,i)=><p key={i} className="text-sm opacity-80 border-l-2 border-amber-500 pl-3">{t}</p>)}
            <button onClick={()=>setTermsOpen(false)} className="w-full py-4 bg-amber-500 text-black rounded-xl font-bold mt-4">{T.terms_btn}</button>
         </div>
      </Modal>
      ...
      {levelUpPopup && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-6">
            ...
            <div className={`relative p-8 rounded-[2.5rem] text-center max-w-sm w-full animate-scale-in shadow-2xl border ${isDark ? 'bg-zinc-900 border-zinc-700 text-white' : 'bg-white text-zinc-900'}`}>
                ...
                <button onClick={()=>setLevelUpPopup(false)} className="w-full py-4 bg-amber-500 text-black font-black rounded-2xl text-lg hover:bg-amber-600 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20">
