...
const SecureStorage = {
  ...
  get: (key) => {
    ...
    // Recuperação de dados antigos para não perder clientes
    if (!cipher) {
...
// 🌟 SMART DATE SELECTOR (Com lógica de 40 minutos)
      ...
      {selectedDate && (
        <div className="animate-slide-up mt-2">
           ...
           <div className="mt-4 flex items-center gap-2 text-gray-500 text-[11px] bg-[#1C1C1E] p-3 rounded-xl border border-white/5">
                ...
                <span>Mínimo 40 min de antecedência.</span>
           </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  useEffect(() => { const hr = new Date().getHours(); }, []);
  ...
  const handleLogoClick = () => {
    ...
    if(adminClicks + 1 === 5) { setIsAdmin(true); addToast("Modo Admin Ativado", "success"); setAdminClicks(0); }
  };
  ...
  const handleWhatsApp = (acceptedUpsell = false) => {
    ...
    let addressTxt = "";
    if(sel.location.requiresAddress) {
        ...
        <div className="w-full max-w-[440px] bg-[#000] sm:my-4 sm:rounded-[40px] sm:border border-white/10 shadow-2xl relative overflow-hidden h-screen sm:h-[90vh] flex flex-col aurora-bg">
          ...
          <div className="relative z-20 px-6 pt-10 pb-4 flex justify-between items-center bg-gradient-to-b from-black/90 to-transparent">
             ...
             <div className="flex gap-3">
               <button onClick={() => setShowNotifications(true)} className="w-10 h-10 rounded-full bg-[#1C1C1E] flex items-center justify-center border border-white/10 relative"><Bell className="w-5 h-5 text-gray-400"/>{loyalty.notifications.some(n=>!n.read)&&<div className="absolute top-0 right-0 w-3 h-3 bg-red-500 rounded-full"/>}</button>
          ...
          {showMenu && (
             <div className="absolute top-20 right-6 w-48 bg-[#1C1C1E] border border-white/10 rounded-2xl z-50 animate-slide-up overflow-hidden shadow-2xl">
                <button onClick={() => setShowFaq(true)} className="w-full p-4 text-left text-sm hover:bg-white/5 flex items-center gap-2"><HelpCircle size={16}/> Ajuda</button>
                <button onClick={() => { SecureStorage.clear(); window.location.reload(); }} className="w-full p-4 text-left text-red-500 text-sm hover:bg-white/5 flex items-center gap-2"><LogOut size={16}/> Sair</button>
             </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-32 relative z-10 scrollbar-hide">
            ...
            {step === 'config' && selection.service && (
               <div className="animate-slide-up pt-4 space-y-8">
                  ...
                  <div ref={locationRef} className="space-y-3">
                      ...
                      {locations.map(l => (
                          <div key={l.id}>
                              ...
                              {/* CAMPOS DE ENDEREÇO OBRIGATÓRIOS PARA SANTA FÉ */}
                              {selection.location?.id === l.id && l.requiresAddress && (
                                  <div className="mt-3 animate-fade-in p-4 bg-[#2C2C2E] rounded-xl border border-white/5 space-y-3">
                                      <h5 className="text-[10px] font-bold uppercase text-blue-400 flex items-center gap-1"><MapPin size={10}/> Endereço Obrigatório</h5>
                                      <input value={selection.address.street} onChange={e => setSelection(p => ({...p, address: {...p.address, street: e.target.value}}))} placeholder="Rua" className="w-full custom-input p-3 rounded-lg text-sm"/>
                                      <div className="flex gap-2">
                                          <input value={selection.address.number} onChange={e => setSelection(p => ({...p, address: {...p.address, number: e.target.value}}))} placeholder="Número" className="w-1/3 custom-input p-3 rounded-lg text-sm"/>
                                          <input value={selection.address.district} onChange={e => setSelection(p => ({...p, address: {...p.address, district: e.target.value}}))} placeholder="Bairro" className="w-2/3 custom-input p-3 rounded-lg text-sm"/>
                                      </div>
                                      <input value={selection.address.ref} onChange={e => setSelection(p => ({...p, address: {...p.address, ref: e.target.value}}))} placeholder="Ponto de Referência" className="w-full custom-input p-3 rounded-lg text-sm"/>
                                  </div>
                              )}
                  ...
                  {/* VIBE */}
                  <div ref={vibeRef}>
                    <h4 className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-3">Vibe Sonora</h4>
                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                       {musicVibes.map(v => (
                          <button key={v} onClick={() => { setSelection({...selection, music: v}); scrollTo(extrasRef); }} className={`px-5 py-2 rounded-xl border text-xs font-bold whitespace-nowrap ${selection.music === v ? 'bg-white text-black' : 'bg-[#1C1C1E] border-white/10'}`}>{v}</button>
                       ))}
                    </div>
                  </div>
              ...
              <div className="bg-[#1C1C1E] w-full max-w-sm rounded-[32px] p-8 border border-white/10 shadow-2xl animate-scale">
                ...
                <div className="space-y-5 text-[15px] text-gray-300 leading-relaxed">
                  ...
                  <div className="pt-6 border-t border-white/10">
                      <p className="text-xs text-gray-500 mb-3">⚠️Atenção: Limpar dados apagará seu progresso e nível.</p>
                      <button onClick={() => { if(window.confirm("Tem certeza?")) { SecureStorage.clear(); window.location.reload(); }}} className="w-full py-3 rounded-xl bg-red-500/10 text-red-500 text-sm font-bold flex items-center justify-center gap-2 hover:bg-red-500/20 transition-colors"><Trash2 className="w-4 h-4"/> Limpar Dados do App</button>
                  </div>
                </div>
