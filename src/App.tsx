...
const ToastContainer = memo(({ toasts, isDark }: { toasts: any[]; isDark: boolean }) => (
  <div className="fixed top-5 left-1/2 -translate-x-1/2 z-[200] flex flex-col gap-2 pointer-events-none w-full max-w-sm px-4">
    {toasts.map(t => (
      <div key={t.id} role="alert" className={`animate-toast-in pointer-events-auto flex items-center gap-3 px-5 py-4 rounded-2xl border shadow-2xl ${t.type === 'error' ? 'bg-red-950/95 border-red-800 text-red-200' : isDark ? 'bg-zinc-900/98 border-white/10 text-white' : 'bg-white/98 border-black/8 text-slate-900'}`}>
      ...
      <div className={`relative w-full max-w-md max-h-[90vh] flex flex-col rounded-[2.5rem] border shadow-2xl animate-scale-in overflow-hidden ${isDark ? 'bg-[#11141a] border-white/10' : 'bg-[#ffffff] border-slate-200'}`}>
        ...
        <div className={`relative p-8 pb-6 flex-shrink-0 ${isPremium ? (isDark ? 'bg-amber-950/20' : 'bg-amber-50/50') : (isDark ? 'bg-blue-950/20' : 'bg-blue-50/50')}`}>
          ...
          <div className="flex items-baseline gap-2 mt-4">
            ...
            <span className={`font-display text-3xl ${isDark ? 'text-white' : 'text-slate-900'}`}>{formatMoney(service.price, 'pt')}</span>
          </div>
        </div>
          ...
          {step === 1 && (
            <section className="animate-fade-up max-w-2xl mx-auto space-y-10">
              <div className="text-center">
                ...
                <p className={`text-base ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>{lang === 'en' ? 'I come to you' : 'Eu vou até o seu local'}</p>
              </div>
                    ...
                    <div className={`border-t pt-6 space-y-3 ${isDark ? 'border-white/8' : 'border-slate-200'}`}>
                      ...
                      {booking.appliedCoupon && (
                        <div className={`flex justify-between text-base font-medium ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>
                          <span className="flex items-center gap-2"><Icon name="gift" size={16} />{booking.appliedCoupon.code}</span>
                      ...
                      {financials.rushFee > 0 && (
                        <div className={`flex justify-between text-base font-medium ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>
                          <span className="flex items-center gap-2"><Icon name="car" size={16} />{T.msg_rush_fee}</span>
                      ...
                      <div className={`flex justify-between items-end pt-5 mt-2 border-t ${isDark ? 'border-white/8' : 'border-slate-100'}`}>
                        ...
                        <div className="text-right">
                          <p className="font-display text-4xl text-gradient-blue">{formatMoney(financials.total, lang)}</p>
                ...
                <div className="space-y-6">
                  ...
                  <div className={`p-6 rounded-[2rem] border ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                      ...
                      <div className="flex flex-wrap gap-2.5 mb-5">
                        {user.coupons.map(c => (
                          <button key={c.id} onClick={() => { setBooking(b => ({ ...b, appliedCoupon: b.appliedCoupon?.id === c.id ? null : c })); vibrate(30); }}
                            className={`px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wider border transition-all flex items-center gap-2 ${booking.appliedCoupon?.id === c.id ? 'bg-emerald-600 text-white border-emerald-500 shadow-lg shadow-emerald-900/20' : isDark ? 'bg-white/6 border-white/10 text-zinc-300 hover:bg-white/10' : 'bg-slate-100 border-slate-200 text-slate-700 hover:border-slate-300'}`}>
                    ...
                    <div className="flex gap-3">
                      <input
                        ...
                        className={`input-field flex-1 h-12 px-4 rounded-xl text-sm font-semibold border outline-none tracking-widest uppercase ${isDark ? 'bg-white/5 border-white/10 text-white placeholder:text-zinc-600 focus:border-blue-500/60' : 'bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus:border-blue-500'}`}
                      />
                      <button onClick={handleApplyCoupon}
                        className={`h-12 px-6 rounded-xl text-sm font-bold uppercase tracking-wide transition-all border ${isDark ? 'bg-white/8 border-white/12 text-zinc-200 hover:bg-white/14 hover:text-white' : 'bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200'}`}>
                        {T.coupon_apply}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className={`p-6 rounded-[2rem] border ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                    ...
                    <button onClick={() => { setBooking(b => ({ ...b, mediaAllowed: !b.mediaAllowed })); vibrate(30); }}
                      ...
                      <span className={`px-3 py-1 rounded-lg ${booking.mediaAllowed ? 'bg-blue-600 text-white' : isDark ? 'bg-white/8' : 'bg-slate-200'}`}>{T.media_bonus}</span>
                    </button>
                  </div>
                      ...
                      {[
                        ...
                        <button key={p.id} onClick={() => {
                          ...
                          {p.id === 'pix' && booking.payment === 'pix' && (
                            <span className="text-[10px] font-bold bg-white/20 px-2.5 py-1 rounded-lg">{p.note}</span>
                          )}
            ...
            <section className="min-h-[80vh] flex flex-col items-center justify-center text-center animate-scale-in max-w-md mx-auto px-5 pt-12">
              ...
              <div className={`w-full p-6 rounded-[2rem] border mb-10 text-left space-y-3 ${isDark ? 'bg-white/4 border-white/8' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <div className={`flex items-center justify-between text-base pt-3 border-t ${isDark ? 'border-white/6 text-white' : 'border-slate-100 text-slate-900'}`}>
                  ...
                  <span className="font-display text-2xl text-gradient-blue">{formatMoney(financials.total, lang)}</span>
                </div>
              </div>
          ...
          <div className={`max-w-5xl mx-auto backdrop-blur-xl pointer-events-auto rounded-[2rem] overflow-hidden border shadow-2xl ${isDark ? 'bg-black/60 shadow-black/80 border-white/10' : 'bg-white/80 shadow-slate-300/80 border-black/8'}`}>
            <div className="flex items-center gap-4 px-5 py-4">
              ...
              <div className={`flex-1 min-w-0 pl-${step === 0 ? '3' : '1'}`}>
                <p className={`text-[10px] uppercase font-bold tracking-widest mb-1 ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
