...
const Header = ({ title, subtitle, onBack, rightAction }) => (
  <div className="flex items-center justify-between pt-12 pb-6 px-6 bg-gradient-to-b from-black/90 to-transparent sticky top-0 z-40 backdrop-blur-sm">
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
       ...
       <div className="relative bg-[#1C1C1E] w-full max-w-[440px] rounded-t-[32px] border-t border-white/10 shadow-2xl animate-slide-up h-[85vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-6 pt-4 pb-32">
             ...
             <p className="text-gray-300 text-sm leading-relaxed mb-6 bg-white/5 p-4 rounded-2xl border border-white/5">
                {service.description}
             </p>
             ...
             <ul className="space-y-3 mb-8">
                {service.benefits.map((b, i) => (
                   <li key={i} className="flex items-start gap-3 text-sm text-gray-300">
                      <Check className="w-5 h-5 text-blue-500 flex-shrink-0"/> {b}
                   </li>
                ))}
             </ul>
          </div>
  ...
  return (
    <div className="animate-enter pb-32">
          ...
          <section>
             ...
             <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide -mx-2 px-2">
                {days.map((d, i) => {
                   ...
                   return (
                      <button key={i} onClick={() => dispatch({type: 'UPDATE', payload: {date: d, time: ''}})}
             ...
             <section className="animate-slide-up">
                ...
                <div className="grid grid-cols-4 gap-2">
                   {['09:00','10:00','11:00','13:00','14:00','15:00','16:00','17:00','18:00','19:00','20:00'].map(t => {
                      ...
                      return (
                         <button key={t} disabled={blocked} onClick={() => dispatch({type: 'UPDATE', payload: {time: t}})}
          ...
          <section>
             ...
             <div className="space-y-3">
                {LOCATIONS_DB.map(loc => (
                   <div key={loc.id} onClick={() => dispatch({type: 'UPDATE', payload: {location: loc, address: '', city: ''}})}
       ...
       <div className="fixed bottom-0 w-full max-w-[440px] p-6 bg-[#000] border-t border-white/10">
          <Button onClick={onNext} disabled={!state.date || !state.time || !state.location || (state.location.askAddress && state.address.length < 5)}>
     ...
     <div className="animate-enter pb-40">
        ...
        <div className="fixed bottom-0 w-full max-w-[440px] bg-[#1C1C1E]/90 backdrop-blur-xl border-t border-white/10 p-5 rounded-t-[32px] z-50">
           <div className="flex justify-between items-end mb-4">
              <div>
                 <p className="text-[10px] text-gray-500 uppercase font-bold">Total Final</p>
           ...
           <Button onClick={onFinalize} loading={isProcessing} disabled={!user.name || !user.isAdult || !user.isMassagemOk || !state.paymentMethod}>
       ...
       <div className="flex justify-center min-h-screen bg-black">
          ...
          <div className="w-full max-w-[440px] aurora-bg relative shadow-2xl overflow-hidden flex flex-col">
             ...
             {state.step === 'home' && (
                <HomeScreen 
                   loyalty={loyalty} 
                   onSelectService={(s) => dispatch({type: 'SELECT_SERVICE', payload: s})}
                   hasNotifs={loyalty.notifications.some(n => !n.read)}
             ...
             {state.step === 'service_detail' && (
                <div className="animate-enter h-full relative">
                   ...
                   <ServiceDetailModal 
                      ...
                      onContinue={() => dispatch({type: 'SET_STEP', payload: 'logistics'})}
                   />
                </div>
             )}

             {state.step === 'logistics' && (
                <BookingLogisticsScreen 
                   state={state} 
                   dispatch={dispatch} 
                   onBack={() => dispatch({type: 'SET_STEP', payload: 'home'})}
                   onNext={() => dispatch({type: 'SET_STEP', payload: 'checkout'})}
                />
             )}
             ...
             {state.step === 'checkout' && (
                <CheckoutScreen 
                   ...
                   onBack={() => dispatch({type: 'SET_STEP', payload: 'logistics'})}
             ...
             {state.step === 'success' && (
                <div className="h-full flex flex-col items-center justify-center p-8 animate-enter text-center">
                   <div className="w-24 h-24 rounded-full bg-green-500 flex items-center justify-center shadow-[0_0_50px_rgba(34,197,94,0.5)] mb-6">
