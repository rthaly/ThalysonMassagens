...
// ==================================================================================
// 0. CONFIGURAÇÃO DE CORES E TEMA (MUDE AQUI!)
// ==================================================================================

const THEME = {
  // Coloque a SUA cor principal aqui (ex: #0A84FF para Azul, #8B5CF6 para Roxo)
  primary: '#D4AF37', // Dourado Premium
  primaryGlow: 'rgba(212, 175, 55, 0.4)', // Brilho da cor principal
  secondary: '#32D74B', // Verde (Sucesso/Dinheiro)
  bg: '#000000', // Fundo
  card: '#1C1C1E', // Cor dos cartões
  text: '#FFFFFF',
  textDim: '#8E8E93'
};

// ==================================================================================
// 1. ESTILOS GLOBAIS (CSS)
// ==================================================================================

const globalStyles = `
/* Reset & Base */
* { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
html { font-size: 16px; background-color: ${THEME.bg}; }
body { 
...
.aurora-bg {
  background: 
    ...
    radial-gradient(100% 100% at 50% 100%, ${THEME.primary}08, transparent 50%);
...
.ios-btn-primary {
  ...
  color: ${THEME.bg}; /* Texto preto/escuro no botão colorido para contraste */
  box-shadow: 0 8px 20px ${THEME.primaryGlow};
  border: none;
}
...
.custom-input:focus { border-color: ${THEME.primary}; box-shadow: 0 0 0 2px ${THEME.primary}33; }
...
const IconBack = () => <ChevronLeft className="w-6 h-6" style={{color: THEME.primary}} />;
...
const REVIEWS_DB = [
  ...
  { t: "Mão leve e firme ao mesmo tempo. Recomendo.", a: "Anônimo", r: 5 },
  ...
  { t: "Sou hétero, foi minha primeira vez. Respeitou e focou só no meu relaxamento.", a: "Anônimo", r: 5 },
  { t: "O melhor da região. Técnica apurada.", a: "Cliente Vip", r: 5 }
];
  ...
  return (
    <div>
        ...
        <div className="relative h-2 bg-white/10 rounded-full mb-2 overflow-hidden z-10">
            <div className="absolute top-0 left-0 h-full rounded-full transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(255,255,255,0.3)]" style={{ width: `${progress}%`, background: `linear-gradient(90deg, ${THEME.primary}, ${THEME.secondary})` }} />
        </div>
        
        <div className="flex justify-between text-[9px] text-gray-500 font-medium tracking-wide">
            <span>Benefício: <span style={{color: THEME.secondary}}>{currentLevel.perks[1]}</span></span>
            {nextLevel ? (
               <span>Faltam {formatCurrency(nextLevel.min - safeSpent)} p/ {nextLevel.name}</span>
            ) : (
               <span style={{color: THEME.primary}}>Nível Máximo</span>
            )}
        </div>
    </div>
  )
...
const LoyaltyCard = ({ data, privacyMode, onTogglePrivacy }) => {
  return (
    <div className="ios-card p-5 rounded-[28px] relative overflow-hidden mb-6 group border-t border-white/10">
      <div className="absolute top-[-50%] right-[-20%] w-64 h-64 blur-[80px] rounded-full pointer-events-none" style={{background: `${THEME.primary}1A`}}></div>
    ...
    <div className="relative h-28 flex items-center justify-center mb-8">
      <div key={idx} className="absolute inset-0 flex flex-col items-center justify-center animate-fade-in px-4 bg-[#1C1C1E] rounded-[24px] border border-white/5 shadow-xl">
        <div className="flex gap-1 mb-2">
          {[...Array(5)].map((_,k) => <Star key={k} className={`w-3.5 h-3.5 ${k < currentReview.r ? 'fill-current' : 'text-gray-800'}`} style={{color: k < currentReview.r ? THEME.primary : undefined}}/>)}
        </div>
  ...
  return (
    <div className="w-full select-none">
      <div className="flex justify-between items-end mb-4 px-1">
        ...
        <span className="text-[10px] font-medium" style={{color: THEME.primary}}>Próximos {DAYS_TO_SHOW} dias</span>
      </div>
        ...
        {days.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => { triggerHaptic(); onSelect(d, ''); }} 
              ...
              ${isSel ? 'text-black shadow-lg scale-[1.03] z-10 font-bold' : 'bg-[#2C2C2E] text-gray-400 border-white/5 active:bg-[#3A3A3C] hover:bg-[#3A3A3C]'}`}
              style={isSel ? {backgroundColor: THEME.primary, borderColor: THEME.primary} : {}}>
              
              <span className={`text-[9px] uppercase font-bold tracking-wide mb-0.5 ${label === 'HOJE' ? 'text-[#32D74B]' : isSel ? 'text-black/70' : 'opacity-60'}`}>{label}</span>
              <span className={`text-xl font-mono leading-none mb-0.5 ${isSel ? 'text-black' : 'text-gray-200'}`}>{d.getDate()}</span>
              <span className={`text-[8px] uppercase font-bold ${isSel ? 'text-black/60' : 'text-gray-600'}`}>{month}</span>
              
              {isSel && <div className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-black rounded-full shadow-sm"></div>}
            </button>
          )
        })}
      </div>
                           ...
                           {period.slots.map(t => {
                               ...
                               return (
                                  <button key={t} disabled={blocked} onClick={() => { triggerHaptic(); onSelect(selectedDate, t); }} 
                                    ...
                                    ${isSelected ? 'text-black shadow-lg scale-[1.02]' : blocked ? 'bg-white/5 text-gray-600 opacity-30 cursor-not-allowed' : 'bg-[#2C2C2E] text-gray-300 hover:bg-[#3A3A3C] border border-white/5'}`}
      ...
      {myCoupons.length > 0 ? (
        <div className="space-y-3">
          {myCoupons.map((coupon) => {
            ...
            return (
              <button key={coupon.code} onClick={() => { triggerHaptic(); isApplied ? onRemove() : onApply(coupon.code); }} className={`w-full p-4 rounded-[16px] flex justify-between items-center transition-all duration-300 ${isApplied ? 'bg-white/5 border shadow-lg' : 'bg-[#1C1C1E] border border-white/5'}`} style={isApplied ? {borderColor: THEME.primary} : {}}>
                <div className="text-left">
                  <div className="flex items-center gap-2">
                    ...
                    {isApplied && <span className="text-[10px] font-bold animate-pulse" style={{color: THEME.primary}}>APLICADO</span>}
                  </div>
...
export default function App() {
  ...
  // --- CORREÇÃO DO BOTÃO VOLTAR (MOBILE) ---
  useEffect(() => {
    const handlePopState = (event) => {
      if (step !== 'home') {
        // Impede que o navegador saia do app e volta um step
    ...
    // Adiciona um estado ao histórico quando não estiver na home
    if (step !== 'home') {
          ...
          <div className="pointer-events-auto">
             {step !== 'home' && step !== 'success' ? (
                ...
                <div className="flex flex-col items-start animate-fade-in">
                    <span className="text-[10px] font-bold uppercase tracking-widest mb-0.5" style={{color: THEME.primary}}>
      ...
      {loading ? (
        <div className="fixed inset-0 z-[200] bg-black flex flex-col items-center justify-center animate-fade-in">
          <div className="relative w-16 h-16 mb-8">
             <div className="absolute inset-0 rounded-full border-4 opacity-20" style={{borderColor: THEME.primary}}></div>
             <div className="absolute inset-0 rounded-full border-4 border-t-transparent animate-spin-slow" style={{borderColor: THEME.primary, borderTopColor: 'transparent'}}></div>
          </div>
          <span className="text-[11px] font-bold tracking-[0.3em] animate-pulse uppercase" style={{color: THEME.primary}}>Carregando</span>
        </div>
      ) : (
      <div className="w-full max-w-[440px] bg-[#000] sm:rounded-[40px] shadow-2xl flex flex-col relative overflow-hidden sm:border border-white/10 h-screen sm:h-[92vh] aurora-bg">
        ...
        {step === 'home' && (
          <div className="flex-1 p-6 overflow-y-auto pb-32 pt-32" ref={homeRef}>
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-white tracking-tight leading-tight mb-2">Massagens Relaxantes<br/><span className="text-transparent bg-clip-text text-2xl" style={{backgroundImage: `linear-gradient(to right, ${THEME.primary}, white)`}}>em Santa Fé do Sul e Região</span></h1>
            </div>
        ...
        {step === 'identity' && (
          <div className="flex-1 p-6 pt-36 animate-fade-in flex flex-col h-full pb-32">
            {selection.service && (
                <div className="mb-8 ios-card p-4 rounded-[20px] flex items-center justify-between border-l-2" style={{borderLeftColor: THEME.primary}}>
                  <div>
                  ...
                  <span className="text-[15px] font-bold" style={{color: THEME.primary}}>{formatCurrency(selection.service.basePrice)}</span>
                </div>
            )}
            ...
            <div className="space-y-6 flex-1">
              <div className="ios-card p-6 rounded-[24px]">
                <label className="text-[11px] font-bold uppercase tracking-wider block mb-2" style={{color: THEME.primary}}>Seu Nome</label>
                <input value={user.name} onChange={e => setUser({...user, name: e.target.value})} className="w-full bg-transparent text-white text-[22px] font-medium placeholder:text-gray-600 border-b border-white/10 py-2 focus:border-white transition-colors" placeholder="Digite seu nome..." />
                {user.name.length > 0 && user.name.length < 3 && <span className="text-red-500 text-[10px]">Nome muito curto.</span>}
              </div>

              <div className="space-y-3">
                <button onClick={() => { triggerHaptic(); setUser({...user, isAdult: !user.isAdult}); }} className={`w-full p-5 rounded-[22px] border flex items-center gap-4 transition-all duration-300 ${user.isAdult ? 'bg-white/10' : 'bg-[#1C1C1E] border-transparent'}`} style={user.isAdult ? {borderColor: THEME.primary} : {}}>
                  <div className={`w-6 h-6 rounded-full border-[1.5px] flex items-center justify-center transition-all ${user.isAdult ? 'border-none' : 'border-gray-600'}`} style={user.isAdult ? {backgroundColor: THEME.primary} : {}}>{user.isAdult && <Check className="w-3.5 h-3.5 text-black" />}</div>
                  <span className={`text-[16px] font-medium ${user.isAdult ? 'text-white' : 'text-gray-400'}`}>Maior de 18 anos</span>
                </button>
                <button onClick={() => { triggerHaptic(); setUser({...user, isMassagemOk: !user.isMassagemOk}); }} className={`w-full p-5 rounded-[22px] border flex items-center gap-4 transition-all duration-300 ${user.isMassagemOk ? 'bg-white/10' : 'bg-[#1C1C1E] border-transparent'}`} style={user.isMassagemOk ? {borderColor: THEME.primary} : {}}>
                  <div className={`w-6 h-6 rounded-full border-[1.5px] flex items-center justify-center transition-all ${user.isMassagemOk ? 'border-none' : 'border-gray-600'}`} style={user.isMassagemOk ? {backgroundColor: THEME.primary} : {}}>{user.isMassagemOk && <Check className="w-3.5 h-3.5 text-black" />}</div>
                  <span className={`text-[16px] font-medium ${user.isMassagemOk ? 'text-white' : 'text-gray-400'}`}>Liberado para massagem</span>
                </button>
              </div>

              <button disabled={!user.name || user.name.length < 3 || !user.isAdult || !user.isMassagemOk} onClick={() => { triggerHaptic(); setStep('services'); }} className="w-full ios-btn-primary font-bold py-4 rounded-[22px] text-[17px] disabled:opacity-50 shadow-lg mt-4">Continuar</button>
            </div>
          </div>
        )}
              ...
              {services.map(s => (
                <div key={s.id} onClick={() => { triggerHaptic(); setSelection({...selection, service: s}); setStep('configure'); }} className={`ios-card p-6 rounded-[30px] active:scale-98 transition-transform group relative overflow-hidden`}>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="font-bold text-[18px]" style={{color: THEME.primary}}>{formatCurrency(s.basePrice)}</span>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {s.details.slice(0,4).map((d, idx) => (
                        <div key={idx} className="flex items-center gap-2 bg-white/5 p-2 rounded-lg">
                            <div className="w-1 h-1 rounded-full" style={{backgroundColor: THEME.primary}}></div> 
        ...
        {step === 'configure' && selection.service && (
          <div className="flex-1 p-6 pt-36 overflow-y-auto pb-64 animate-fade-in scrollbar-hide"> 
            <div className="ios-card p-5 rounded-[22px] mb-8 flex items-center justify-between border-l-4" style={{borderLeftColor: THEME.primary}}>
              <div>
            ...
            <div className="space-y-10">
                ...
                <div className="space-y-3">
                  {locations.map(l => {
                    ...
                    <div key={l.id} className="animate-fade-in">
                        <button onClick={() => { triggerHaptic(); setSelection({...selection, location: l, useTable: null}); scrollTo(vibeRef); }} className={`w-full p-5 rounded-[22px] border text-left transition-all duration-300 ${selection.location?.id === l.id ? 'bg-white/10' : 'bg-[#1C1C1E] border-transparent'}`} style={selection.location?.id === l.id ? {borderColor: THEME.primary} : {}}>
                        ...
                        {selection.location?.id === l.id && l.id === 'santa-fe' && l.allowsTableChoice && (
                          <div ref={surfaceRef} className="mt-3 grid grid-cols-2 gap-3 animate-fade-in">
                            <button onClick={() => { setSelection({...selection, useTable: false}); scrollTo(vibeRef); }} className={`p-4 rounded-[18px] border text-[13px] font-bold transition-all ${selection.useTable === false ? 'text-black' : 'bg-[#1C1C1E] border-transparent text-gray-400'}`} style={selection.useTable === false ? {backgroundColor: THEME.primary, borderColor: THEME.primary} : {}}>🛏 Na Cama</button>
                            <button onClick={() => { setSelection({...selection, useTable: true}); scrollTo(vibeRef); }} className={`p-4 rounded-[18px] border text-[13px] font-bold transition-all ${selection.useTable === true ? 'text-black' : 'bg-[#1C1C1E] border-transparent text-gray-400'}`} style={selection.useTable === true ? {backgroundColor: THEME.primary, borderColor: THEME.primary} : {}}>💆‍♂️ Maca (+{formatCurrency(CONFIG.PRICES.MACA)})</button>
                          </div>
                        )}
                        {selection.location?.id === l.id && l.id === 'outras-cidades' && (
                            <input value={selection.city} onChange={e => setSelection({...selection, city: e.target.value})} placeholder="Digite o nome da cidade..." className="mt-3 w-full bg-[#1C1C1E] p-4 rounded-[18px] border border-white/10 text-white placeholder:text-gray-600 focus:border-white transition-all animate-fade-in" />
                        )}
                        {selection.location && (
              ...
              <div className="space-y-3" ref={extrasRef}>
                ...
                <button onClick={() => { triggerHaptic(); setSelection({...selection, upgrade: !selection.upgrade}); }} className={`w-full p-4 rounded-[20px] border flex justify-between items-center transition-all ${selection.upgrade ? 'bg-white/10' : 'bg-[#1C1C1E] border-transparent'}`} style={selection.upgrade ? {borderColor: THEME.primary} : {}}>
                  ...
                  <span className="font-bold text-[15px]" style={{color: THEME.primary}}>+ {formatCurrency(selection.service.basePrice * CONFIG.PRICES.UPGRADE_PCT)}</span>
                </button>

                <button onClick={() => { triggerHaptic(); setSelection({...selection, aroma: !selection.aroma}); }} className={`w-full p-4 rounded-[20px] border flex justify-between items-center transition-all ${selection.aroma ? 'bg-white/10' : 'bg-[#1C1C1E] border-transparent'}`} style={selection.aroma ? {borderColor: THEME.primary} : {}}>
                  ...
                  <div className="text-right">
                      ...
                      ) : (<span className="font-bold text-[15px]" style={{color: THEME.primary}}>+ {formatCurrency(CONFIG.PRICES.AROMA_FULL)}</span>)}
                  </div>
                </button>
              </div>
              ...
              <div className="mt-6" ref={paymentRef}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setSelection({...selection, paymentMethod: 'pix'})} className={`h-24 rounded-[18px] border flex flex-col items-center justify-center gap-2 transition-all ${selection.paymentMethod === 'pix' ? 'bg-white/10' : 'bg-[#1C1C1E] border-transparent'}`} style={selection.paymentMethod === 'pix' ? {borderColor: THEME.primary} : {}}>
                    <QrCode className="w-6 h-6" style={{color: THEME.primary}} /><span className="text-[13px] font-bold text-white">Pix</span>
                  </button>
                  <button onClick={() => setSelection({...selection, paymentMethod: 'cash'})} className={`h-24 rounded-[18px] border flex flex-col items-center justify-center gap-2 transition-all ${selection.paymentMethod === 'cash' ? 'bg-white/10' : 'bg-[#1C1C1E] border-transparent'}`} style={selection.paymentMethod === 'cash' ? {borderColor: THEME.primary} : {}}>
                    <Banknote className="w-6 h-6 text-[#30D158]" /><span className="text-[13px] font-bold text-white">Dinheiro</span>
                  </button>
                  <button onClick={() => setSelection({...selection, paymentMethod: 'debit_card'})} className={`h-24 rounded-[18px] border flex flex-col items-center justify-center gap-2 transition-all ${selection.paymentMethod === 'debit_card' ? 'bg-white/10' : 'bg-[#1C1C1E] border-transparent'}`} style={selection.paymentMethod === 'debit_card' ? {borderColor: THEME.primary} : {}}>
                    <CreditCard className="w-6 h-6" style={{color: THEME.primary}} /><span className="text-[13px] font-bold text-white">Débito</span>
                  </button>
                  <button onClick={() => setSelection({...selection, paymentMethod: 'credit_card'})} className={`h-24 rounded-[18px] border flex flex-col items-center justify-center gap-2 transition-all ${selection.paymentMethod === 'credit_card' ? 'bg-white/10' : 'bg-[#1C1C1E] border-transparent'}`} style={selection.paymentMethod === 'credit_card' ? {borderColor: THEME.primary} : {}}>
                ...
                {selection.paymentMethod === 'credit_card' && (
                  <div className="mt-3 ios-card p-4 rounded-[16px] animate-fade-in">
                    ...
                    <select value={selection.installments} onChange={(e) => setSelection({...selection, installments: parseInt(e.target.value)})} className="w-full bg-[#000] border border-white/10 text-white text-[15px] rounded-xl p-3 focus:border-white">
        ...
        {step === 'configure' && selection.location && (
          <div className="absolute bottom-0 w-full p-0 z-40">
            ...
            <div className="bg-[#1C1C1E] rounded-t-[32px] p-5 border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.8)]">
              ...
              <button disabled={!canFinalize} onClick={handleWhatsApp} className="w-full ios-btn-primary font-bold py-4 rounded-[18px] shadow-[0_4px_20px_rgba(0,0,0,0.4)] flex justify-center items-center gap-2 text-[16px] disabled:opacity-50 disabled:shadow-none">CONFIRMAR NO WHATSAPP</button>
            </div>
          </div>
        )}
        ...
        {step === 'success' && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-fade-in pb-32">
            ...
            <button onClick={() => window.open(lastOrderLink, '_blank')} className="mb-4 w-full flex items-center justify-center gap-2 text-[15px] font-bold bg-white/10 py-3.5 rounded-xl border border-white/20 hover:bg-white/20 transition-colors" style={{color: THEME.primary}}><Send className="w-4 h-4"/> Reenviar Mensagem</button>
        ...
        {showFaq && (
          <div className="absolute inset-0 z-[200] bg-black/60 backdrop-blur-xl flex items-center justify-center p-5">
            <div className="bg-[#1C1C1E] w-full max-w-sm rounded-[32px] p-8 border border-white/10 shadow-2xl animate-scale">
              <h3 className="text-2xl font-bold text-white mb-6 flex items-center gap-2"><HelpCircle className="w-6 h-6" style={{color: THEME.primary}}/> Ajuda & Informações</h3>
                ...
                {loyalty.notifications.length === 0 ? (
                    ...
                    loyalty.notifications.map(n => (
                        <div key={n.id} className="p-4 rounded-[20px] bg-[#2C2C2E] border border-white/5 flex items-start gap-4">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${n.icon === 'level' ? 'bg-[#FFD60A]/20 text-[#FFD60A]' : n.icon === 'coupon' ? 'bg-[#FF375F]/20 text-[#FF375F]' : 'bg-white/20 text-white'}`} style={n.icon !== 'level' && n.icon !== 'coupon' ? {backgroundColor: `${THEME.primary}33`, color: THEME.primary} : {}}>
