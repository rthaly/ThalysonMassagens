...
import {
  ...
  HelpCircle, Instagram, Calendar as CalendarIcon, Clock, User, AlertTriangle, Car, Sparkles
} from 'lucide-react';
...
// LOCAIS (TAXAS IDA+VOLTA JÁ CALCULADAS)
const LOCATIONS = [
...
const SERVICES = [
  { 
    ...
    // SEM ÓLEO QUENTE NO TEXTO
    desc: 'O protocolo exclusivo. Massagem profunda, uso de cremes e óleos de alta qualidade, toque pele na pele e finalização manual intensa.', 
    duration: 60, 
    price: 155, 
    badge: 'RECOMENDADO 🔥',
    xp: 100
  ...
  { 
    ...
    desc: 'Foco 100% terapêutico. Ideal para remover dores lombares e cansaço. Toques firmes para zerar o stress, sem toques íntimos.', 
    duration: 60, 
    price: 125, 
    badge: null,
    xp: 50
...
const Utils = {
  ...
  isBlocked: (d, t) => {
    ...
    return slot < new Date(now.getTime() + 30 * 60000); // Bloqueia próximos 30min
  }
};
...
export default function BookingApp() {
  ...
  const [data, setData] = useState(() => {
     try {
       const s = localStorage.getItem('thaly_v_remaster_1');
       if(s) { 
           const p = JSON.parse(s); 
           if(p.date) p.date = new Date(p.date);
           if(!p.location) throw new Error("Reset"); // Validação básica
           return p; 
       }
     } catch(e) { localStorage.removeItem('thaly_v_remaster_1'); }
     return { 
  ...
  const isFirstTime = !localStorage.getItem('thaly_history_remaster');

  // Rola suave
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  useEffect(() => { localStorage.setItem('thaly_v_remaster_1', JSON.stringify(data)); }, [data]);
  ...
  const finalizeOrder = () => {
      if(couponActive || isFirstTime) localStorage.setItem('thaly_history_remaster', 'true');
      setSuccess(true);
      scrollToTop();
  };
  ...
  const generateMessage = () => {
    ...
    if(couponActive) t += `🎟️ Desconto: -${Utils.fmt(financials.desc)}\n`;
  ...
  if (success) return (
    <div className="min-h-screen bg-[#09090b] pt-20 px-6 flex flex-col items-center text-center animate-fade">
       ...
       <button onClick={() => { setSuccess(false); setStage(0); setCouponActive(false); scrollToTop(); }} className="text-gray-500 font-bold text-xs uppercase py-4">Voltar ao Início</button>
    </div>
  );
    ...
    <div className="min-h-screen pb-40 relative">
      ...
      {helpOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 animate-fade">
              ...
              <div className="relative bg-[#18181b] w-full max-w-sm rounded-3xl border border-[#333] p-6 shadow-2xl">
                  ...
                  <div className="space-y-4">
                      <div className="bg-[#27272a] p-4 rounded-2xl">
                          ...
                          <p className="text-xs text-gray-400">Atendimento estritamente profissional. Seus dados são usados apenas para o agendamento.</p>
                      </div>
                      <div className="bg-[#27272a] p-4 rounded-2xl">
                          <h4 className="font-bold text-white text-sm mb-1 flex gap-2"><Car size={14}/> Logística</h4>
                          <p className="text-xs text-gray-400">Taxa de deslocamento calculada para ida e volta. Atendo em hotéis e residências.</p>
                      </div>
                  </div>
              </div>
          </div>
      ...
      <main className="max-w-md mx-auto pt-28 px-5">
        ...
        <section className={`transition-all duration-500 ${stage === 0 ? 'opacity-100' : 'section-disabled'}`}>
            <div className="mb-8">
                <h1 className="text-4xl font-extrabold mb-2 tracking-tight">Massagem &<br/><span className="text-[#0A84FF]">Experiência.</span></h1>
                <p className="text-gray-400 text-sm">Atendimento masculino exclusivo. Técnica apurada e total discrição.</p>
            </div>

            <div className="card-base p-6 space-y-5 shadow-xl">
                <div className="space-y-3">
                    <input value={data.name} onChange={e => setData({...data, name: e.target.value})} placeholder="Seu Nome" className="input-clean"/>
                    <input type="tel" maxLength={2} value={data.age} onChange={e => setData({...data, age: e.target.value.replace(/\D/g,'')})} placeholder="Idade (Ex: 30)" className="input-clean"/>
                </div>
                <div onClick={() => { Utils.vibrate(); setData({...data, medical: !data.medical}) }} 
                    className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${data.medical ? 'bg-[#0A84FF]/10 border-[#0A84FF]' : 'bg-[#27272a] border-[#3f3f46]'}`}>
                    <div className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${data.medical ? 'bg-[#0A84FF] border-[#0A84FF]' : 'border-[#666]'}`}>{data.medical && <Check size={14} className="text-white"/>}</div>
                    <p className="text-sm font-bold text-white">Sou maior de idade e saudável</p>
                </div>
                {data.name.length > 2 && data.age && data.medical && stage === 0 && (
                    <button onClick={() => { setStage(1); window.scrollTo(0, 400); }} className="btn-primary w-full py-4 flex items-center justify-center gap-2 mt-2 animate-fade">Começar <ArrowRight size={20}/></button>
                )}
            </div>
        </section>
        ...
        <section className={`mt-12 transition-opacity duration-500 ${stage >= 1 ? 'opacity-100' : 'hidden'}`}>
            ...
            <div className="space-y-5">
                {SERVICES.map(s => (
                    <div key={s.id} onClick={() => { if(stage === 1) { setData({...data, service: s}); setStage(2); window.scrollTo(0, 800); }}} className={`card-base p-6 cursor-pointer relative ${data.service?.id === s.id ? 'card-active' : ''}`}>
                        {s.badge && <div className="absolute top-0 right-0 bg-[#FFD60A] text-black text-[9px] font-black px-3 py-1.5 rounded-bl-2xl">{s.badge}</div>}
        ...
        <section className={`mt-12 transition-opacity duration-500 ${stage >= 3 ? 'opacity-100' : 'hidden'}`}>
            ...
            <div className="card-base divide-y divide-[#27272a]">
                {[
                   { id: 'upgrade', label: '+30 Minutos', desc: 'Mais tempo de sessão.', icon: Clock, price: data.service?.price * CONFIG.PRICES.UPGRADE_PCT },
                   { id: 'touch', label: 'Interação / Toque', desc: 'Você no controle. Toques recíprocos.', icon: Flame, price: CONFIG.PRICES.TOUCH },
                   { id: 'aroma', label: 'Aromaterapia', desc: 'Essências relaxantes no ambiente.', icon: Wind, price: CONFIG.PRICES.AROMA }
                ].map((item) => (
        ...
        <section className={`mt-12 transition-opacity duration-500 ${stage >= 4 ? 'opacity-100' : 'hidden'}`}>
            ...
            <div className="mb-6 -mx-5 px-5">
                <div className="flex gap-3 overflow-x-auto pb-4 hide-scroll snap-x px-1">
                    {LOCATIONS.map(loc => (
                        <div key={loc.id} onClick={() => setData({...data, location: {...data.location, id: loc.id}})}
                            className={`snap-center flex-shrink-0 w-40 p-4 rounded-2xl border transition-all cursor-pointer ${data.location.id === loc.id ? 'border-[#0A84FF] bg-[#0A84FF]/10' : 'border-[#3f3f46] bg-[#18181b]'}`}>
                            <p className="text-[10px] uppercase font-bold text-gray-500 mb-1">{loc.zone}</p>
                            <p className="text-sm font-bold text-white mb-2 h-10">{loc.name}</p>
                            <span className={`text-xs font-bold ${data.location.id === loc.id ? 'text-[#0A84FF]' : 'text-white'}`}>{loc.fee === 0 ? 'Grátis' : Utils.fmt(loc.fee)}</span>
                        </div>
                    ))}
                </div>
            </div>
