...
const CONFIG = {
  ...
  STORAGE_KEY: '@thaly_app_v32_vip_club',
  PIX_KEY: "62.922.530/0001-14",
  BILLING_NAME: "T.S. Serviços Digitais", // Nome discreto para a fatura do cartão
  LOCALE_PT: 'pt-BR',
  LOCALE_EN: 'en-US',
  EXCHANGE_RATE: 5.0,
...
const ICON_PATHS: Record<string, string> = {
  ...
  'crown': 'M2 22h20 M12 2l-4 6-4-4 2 8h12l2-8-4 4-4-6z', // Crown icon for VIP
};
...
const GlobalStyles = memo(({ isDark }: { isDark: boolean }) => (
  <style dangerouslySetInnerHTML={{ __html: `
    ...
    *:focus-visible { outline: 2px solid var(--c-blue); outline-offset: 4px; }
    ...
    .service-card-selected-vip { box-shadow: 0 0 0 2px var(--c-vip), 0 8px 24px rgba(168,85,247,0.2); }
...
// TYPES
// ==================================================================================
interface ServiceItem { id: string; min: number; price: number; icon: string; tag: string; title: string; desc: string; details: string; fullPrice?: number; savings?: number; type?: string; popular?: boolean; category?: 'relax' | 'express' | 'final' | 'care'; recurring?: boolean; }
...
interface BookingData { type: 'single' | 'pack' | 'vip'; cart: ServiceItem[]; extras: Record<string, boolean>; date: string | null; time: string | null; locationType: 'home' | 'motel' | 'hotel'; address: Address; payment: string; appliedCoupon: Coupon | null; termsAccepted: boolean; bookingId: string; customExtraText: string; }
...
const getData = (lang: 'pt' | 'en') => {
  ...
  const p = {
    ...
    vip_gold: { v: 590, full: 700, save: 110 }, // 2x Nuru/Fusion
    vip_black: { v: 1100, full: 1400, save: 300 }, // 4x Sessions + Perks
  ...
  return {
    ...
    vips: [
      { id: 'vip_gold', type: 'vip', recurring: true, title: isEn ? "Gold Club (2x/mo)" : "Clube Ouro (2x/mês)", price: p.vip_gold.v, fullPrice: p.vip_gold.full, savings: p.vip_gold.save, desc: isEn ? "Priority booking and 2 premium sessions." : "Prioridade de agenda e 2 encontros intensos por mês para renovar a energia.", details: isEn ? "2x Any Premium Session\nPriority Booking\nDiscreet Billing" : "2x Massagens à sua escolha (inclui Nuru/Fusion)\nPrioridade nos agendamentos\nFatura discreta no cartão ou Pix manual", tag: "QUINZENAL", icon: "award" },
      { id: 'vip_black', type: 'vip', recurring: true, title: isEn ? "Black Club (4x/mo)" : "Clube Black (4x/mês)", price: p.vip_black.v, fullPrice: p.vip_black.full, savings: p.vip_black.save, desc: isEn ? "The ultimate weekly relief. Full priority." : "O alívio semanal definitivo. Tratamento de rei com mimos exclusivos.", details: isEn ? "4x Any Premium Session\n1x Free Trim\nTop Priority" : "4x Massagens à sua escolha\n1x Aparo de Pelos Grátis/mês\nHorários bloqueados só para você\nCobrança 100% sigilosa", tag: "SEMANAL", icon: "crown" }
    ] as ServiceItem[],
    ...
    faq: [
      ...
      { q: isEn ? "Where is the meeting?" : "Onde nós vamos nos encontrar?", a: isEn ? "I come to you or a hotel." : "Eu vou até você (residência ou hotel) ou você pode vir na minha Suíte Privada na Bela Vista. Eu cuido de todo o ambiente (óleos, gel, toalhas)." },
      { q: isEn ? "How does the VIP Club work?" : "Como funciona o Clube VIP e a cobrança?", a: isEn ? "It's a discreet monthly recurrence." : "O Clube VIP garante que você cuide de si todo mês pagando menos e tendo prioridade na agenda. Pensando no seu sigilo, a cobrança no cartão vem em nome de 'T.S. Serviços Digitais' (sem relação com massagem), ou você pode optar por um lembrete de Pix mensal manual." },
    ...
    text: {
      ...
      tab_packs: isEn ? "Plans" : "Combos (Únicos)",
      tab_single: isEn ? "Single" : "Avulsas",
      tab_vip: isEn ? "VIP Club" : "Clube VIP",
      ...
      toast_select_item: isEn ? "Add a service." : "Escolha ao menos um serviço/plano.",
      toast_select_date: isEn ? "Choose date & time." : "Escolha o dia e horário do 1º encontro.",
      ...
      payment_title: isEn ? "Payment (In person)" : "Forma de pagamento",
      terms_title: isEn ? "Rules & Discretion" : "Regras e Sigilo",
      success_title: isEn ? "Almost there!" : "Tudo Certo, falta pouco!",
      success_sub: isEn ? "Send the summary on WhatsApp to confirm." : "Para garantir seu horário (e sua vaga no clube), me envie o resumo no WhatsApp para confirmarmos tudo.",
      ...
      timer_text: isEn ? "Cart saved for" : "Sua reserva expira em",
      ...
      step_when: isEn ? "When" : "Quando (1º Encontro)",
      ...
      summary_title: isEn ? "Summary" : "Resumo da Seleção",
      summary_items: isEn ? "SERVICES" : "O QUE VAMOS FAZER",
      summary_extras: isEn ? "EXTRAS" : "ADICIONAIS",
      summary_info: isEn ? "DETAILS" : "INFORMAÇÕES DO 1º ENCONTRO",
      ...
      pay_pix: isEn ? "Pix" : "Pix",
      pay_card: isEn ? "Card" : "Cartão (Crédito)",
      pay_cash: isEn ? "Cash" : "Dinheiro (Apenas Avulsas)",
      ...
      toast_cart_toggle: isEn ? "Updated." : "Carrinho atualizado.",
...
const Button = memo(({ children, onClick, variant = 'primary', size = 'md', disabled = false, full = false, icon, className = '', loading = false }: any) => {
  ...
  const variants: Record<string, string> = {
    ...
    vip: "bg-purple-600 text-white hover:bg-purple-500 shadow-md shadow-purple-900/20",
  };
...
// ==================================================================================
const PRIZES = [
  { val: 5, color: '#f59e0b' }, { val: 10, color: '#2563eb' }, { val: 5, color: '#f59e0b' }, { val: 15, color: '#10b981' },
  { val: 5, color: '#f59e0b' }, { val: 10, color: '#2563eb' }, { val: 5, color: '#f59e0b' }, { val: 20, color: '#e11d48' }
];
...
const TigrinhoRoulette = memo(({ isOpen, isDark, lang, onWin, onClose }: any) => {
  ...
  const spinWheel = () => {
    ...
    const r = Math.random();
    let targetVal;
    if (r < 0.6) targetVal = 10; else if (r < 0.85) targetVal = 5; else if (r < 0.95) targetVal = 15; else targetVal = 20;
    ...
    setTimeout(() => { setWinValue(targetVal); setPhase('won'); vibrate([100, 50, 200]); }, 5000); 
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div role="dialog" className={`relative w-full max-w-sm rounded-[2rem] p-6 text-center border shadow-2xl animate-scale-in flex flex-col items-center ${isDark ? 'bg-[#181c25] border-amber-900/50 shadow-[0_0_50px_rgba(245,158,11,0.15)]' : 'bg-white border-amber-200'}`}>
        <button onClick={onClose} className={`absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center z-50 ${isDark ? 'bg-zinc-800 text-zinc-400 hover:text-white' : 'bg-slate-100 text-slate-500 hover:text-slate-800'}`}><Icon name="x" size={18} /></button>
        ...
        <div className="relative w-64 h-64 mb-8">
          ...
          <div className="w-full h-full rounded-full overflow-hidden border-[8px] border-[#11141a] shadow-inner relative"
            style={{ transition: 'transform 5s cubic-bezier(0.2, 0.8, 0.2, 1)', transform: `rotate(${rotation}deg)`,
...
const ServiceModal = memo(({ service, isOpen, onClose, onSelect, isInCart, isDark, T, lang, isPack, isVip }: any) => {
  if (!isOpen || !service) return null;
  const isVipTheme = isVip;
  const isPackTheme = isPack && !isVip;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div role="dialog" className={`relative w-full max-w-md max-h-[85vh] flex flex-col rounded-3xl border shadow-2xl animate-scale-in overflow-hidden ${isDark ? 'bg-[#181c25] border-zinc-700' : 'bg-white border-slate-200'}`}>
        <div className={`relative p-5 pb-4 shrink-0 border-b ${isDark ? 'border-zinc-800' : 'border-slate-100'} ${isVipTheme ? (isDark ? 'bg-purple-950/20' : 'bg-purple-50/50') : isPackTheme ? (isDark ? 'bg-amber-950/20' : 'bg-amber-50/50') : ''}`}>
          ...
          <div className="flex items-center gap-3 mb-3 pr-10">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center border shrink-0 ${isVipTheme ? isDark ? 'bg-purple-900/30 border-purple-800 text-purple-400' : 'bg-purple-100 border-purple-200 text-purple-700' : isPackTheme ? isDark ? 'bg-amber-900/30 border-amber-800 text-amber-500' : 'bg-amber-100 border-amber-200 text-amber-700' : isDark ? 'bg-zinc-800 border-zinc-700 text-zinc-300' : 'bg-slate-100 border-slate-200 text-slate-700'}`}>
              <Icon name={service.icon} size={20} />
            </div>
            <div className="flex-1 min-w-0">
               <div className="flex items-center gap-2 flex-wrap mb-0.5">
                 <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md border ${isVipTheme ? isDark ? 'border-purple-800/50 text-purple-400' : 'border-purple-300 text-purple-800' : isPackTheme ? isDark ? 'border-amber-800/50 text-amber-500' : 'border-amber-300 text-amber-800' : isDark ? 'border-zinc-700 text-zinc-400' : 'border-slate-300 text-slate-600'}`}>{service.tag}</span>
            ...
            <div className="space-y-3">
              {service.details.split('\n').map((line: string, i: number) => (
                <div key={i} className="flex items-start gap-3 text-sm font-medium">
                  <div className={`mt-0.5 shrink-0 w-4 h-4 rounded-full flex items-center justify-center ${isVipTheme ? isDark ? 'bg-purple-500/20 text-purple-400' : 'bg-purple-100 text-purple-700' : isPackTheme ? isDark ? 'bg-amber-500/20 text-amber-500' : 'bg-amber-100 text-amber-700' : isDark ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-700'}`}><Icon name="check" size={10} /></div>
        ...
        <div className={`p-4 border-t shrink-0 ${isDark ? 'border-zinc-800 bg-[#181c25]' : 'border-slate-100 bg-white'}`}>
          <Button full size="lg" variant={isInCart ? 'outline' : isVipTheme ? 'vip' : isPackTheme ? 'amber' : 'primary'} onClick={() => { onSelect(service); onClose(); }}>
...
const ServiceCard = memo(({ service, isInCart, onToggle, isDark, T, lang, isPack = false, isVip = false, onOpenModal }: any) => {
  const isVipTheme = isVip;
  const isPackTheme = isPack && !isVip;

  return (
    <button className={`relative w-full text-left rounded-2xl border transition-all card-hover flex flex-col h-auto min-h-[120px] ${isInCart ? isVipTheme ? 'service-card-selected-vip border-purple-500/50 bg-purple-500/10' : isPackTheme ? 'service-card-selected-amber border-amber-500/50 bg-amber-500/10' : 'service-card-selected border-blue-500/50 bg-blue-500/5' : isDark ? 'bg-[#181c25] border-zinc-700 hover:border-zinc-600' : 'bg-white border-slate-200 shadow-sm hover:border-slate-300'}`} onClick={() => onOpenModal(service)}>
      {isInCart && <div className={`absolute top-3 right-3 z-10 w-6 h-6 rounded-full flex items-center justify-center animate-scale-in shrink-0 ${isVipTheme ? 'bg-purple-600 text-white' : isPackTheme ? 'bg-amber-500 text-amber-950' : 'bg-blue-600 text-white'}`}><Icon name="check" size={14} /></div>}
      <div className="p-4 flex-1 flex flex-col">
        <div className="flex items-start gap-3 mb-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${isVipTheme ? isDark ? 'bg-purple-900/30 border-purple-800/50 text-purple-400' : 'bg-purple-50 border-purple-200 text-purple-600' : isPackTheme ? isDark ? 'bg-amber-900/30 border-amber-800/50 text-amber-500' : 'bg-amber-50 border-amber-200 text-amber-600' : isDark ? 'bg-zinc-800 border-zinc-700 text-zinc-300' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
            <Icon name={service.icon} size={20} />
          </div>
        ...
        <div className="flex items-end justify-between mt-auto gap-2">
          <div className={`inline-block text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md border truncate max-w-[50%] ${isVipTheme ? isDark ? 'border-purple-800/50 text-purple-400' : 'border-purple-300 text-purple-700' : isPackTheme ? isDark ? 'border-amber-800/50 text-amber-500' : 'border-amber-300 text-amber-700' : isDark ? 'border-zinc-700 text-zinc-400' : 'border-slate-300 text-slate-700'}`}>{service.tag}</div>
...
export default function App() {
  ...
  const downloadICS = () => {
    ...
    const endD = new Date(d.getTime() + (120 * 60000)); 
  ...
  const handleToggleCartItem = useCallback((item: ServiceItem) => {
    vibrate(50);
    setBooking(prev => {
      // Limpa itens de outras categorias (se for plano, não pode ter avulsa e vice versa)
      let currentCart = prev.cart;
      if (item.type === 'vip') {
        currentCart = currentCart.filter(c => c.type === 'vip'); // Só VIP
      } else if (item.type === 'pack') {
        currentCart = currentCart.filter(c => c.type === 'pack'); // Só Packs
      } else {
        currentCart = currentCart.filter(c => !c.type); // Só Avulsas
      }

      const exists = currentCart.find(c => c.id === item.id);
      return { ...prev, cart: exists ? currentCart.filter(c => c.id !== item.id) : [...currentCart, item], payment: '', termsAccepted: false };
    });
  ...
  const financials = useMemo(() => {
    ...
    const isPack = booking.cart.some(i => i.type === 'pack' || i.type === 'vip');
    booking.cart.forEach(item => { sub += item.price; if (!isPack) baseDuration += (item.min || 60); });
    if (isPack) baseDuration = 60; // 1st session placeholder
    let addedTime = 0;
  ...
  const estimatedXP = useMemo(() => Math.floor(financials.total * (booking.cart.some(i => i.type === 'pack' || i.type === 'vip') ? 0.30 : 0.15)), [financials.total, booking.cart]);
  ...
  const generateWhatsAppMsg = () => {
    const f = financials;
    const isVipSub = booking.cart.some(i => i.type === 'vip');
          ...
          {step === 0 && (
            <section className="animate-fade-up space-y-8">
              <div className="flex flex-col gap-6">
                ...
                {/* HORÁRIOS DE ATENDIMENTO - NO TOPO */}
              ...
              {/* NAV DE ABAS (AVULSO, PACOTE, VIP) */}
              <nav className={`flex p-1.5 rounded-2xl border w-full mx-auto ${isDark ? 'bg-[#181c25] border-zinc-800' : 'bg-slate-50 border-slate-200'}`}>
                {[{ id: 'single', label: T.tab_single, icon: 'user' }, { id: 'packs', label: T.tab_packs, icon: 'package' }, { id: 'vip', label: T.tab_vip, icon: 'crown' }].map(tab => (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 flex flex-col items-center justify-center gap-1.5 px-2 py-3 rounded-xl text-[10px] font-bold uppercase transition-all min-h-[50px] ${activeTab === tab.id ? tab.id === 'vip' ? 'bg-purple-600 text-white' : tab.id === 'packs' ? 'bg-amber-500 text-amber-950' : 'bg-blue-600 text-white' : isDark ? 'text-zinc-400' : 'text-slate-600'}`}>
              ...
              <div className="tab-content">
                ...
                {activeTab === 'vip' && (
                  <div className="space-y-6">
                    <div className={`p-4 rounded-2xl border flex items-start gap-3 ${isDark ? 'bg-purple-900/10 border-purple-800/50' : 'bg-purple-50 border-purple-200'}`}>
                      <Icon name="shield" size={20} className={isDark ? 'text-purple-400' : 'text-purple-600'} />
                      <div>
                        <h4 className={`text-sm font-bold ${isDark ? 'text-purple-300' : 'text-purple-800'}`}>{lang === 'en' ? 'Discreet Billing' : 'Cobrança 100% Sigilosa'}</h4>
                        <p className={`text-xs mt-1 ${isDark ? 'text-purple-200/70' : 'text-purple-700/80'}`}>{lang === 'en' ? `Charges appear as '${CONFIG.BILLING_NAME}' on your card, or choose manual Pix.` : `A fatura do cartão constará como '${CONFIG.BILLING_NAME}'. Se preferir, faça no Pix manual todo mês.`}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {DATA.vips.map((s: ServiceItem) => <ServiceCard key={s.id} service={s} isInCart={booking.cart.some(c => c.id === s.id)} onToggle={handleToggleCartItem} isDark={isDark} T={T} lang={lang} isVip={true} onOpenModal={setSelectedServiceForModal} />)}
                    </div>
                  </div>
                )}
              </div>
          ...
          {step === 3 && (
            <section className="animate-fade-up space-y-6 max-w-2xl mx-auto">
              ...
              <article className={`p-5 sm:p-6 rounded-3xl border ${isDark ? 'bg-[#181c25] border-zinc-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                  {DATA.extras.map((ex: any) => {
                    const price = booking.cart.some(i => i.type === 'pack' || i.type === 'vip') ? Math.floor(ex.price * 0.8) : ex.price;
              ...
              <article className={`p-5 sm:p-6 rounded-3xl border ${hasErrorGlobal && !booking.payment ? 'animate-shake' : ''} ${isDark ? 'bg-[#181c25] border-zinc-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <div className="space-y-3">
                  {[{ id: 'pix', label: T.pay_pix, icon: 'smartphone' }, { id: 'card', label: T.pay_card, icon: 'credit-card' }, ...(booking.cart.some(i => i.type === 'vip') ? [] : [{ id: 'money', label: T.pay_cash, icon: 'banknote' }])].map(p => (
