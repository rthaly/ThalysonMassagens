...
import {
  ...
  FileSpreadsheet, ShieldCheck, User, Siren, CheckCircle2
} from 'lucide-react';

// --- ESTILOS GLOBAIS (DESIGN SENIOR - FOCO EM LEGIBILIDADE E CONTRASTE) ---
const globalStyles = `
...
const moodOptions = [
  { id: 'stressed', icon: '🤯', label: 'Estressado', desc: 'Cabeça cheia, preciso desligar' },
  { id: 'needy', icon: '🥺', label: 'Carente', desc: 'Preciso de atenção e toque' },
  { id: 'tired', icon: '🔋', label: 'Cansado', desc: 'Corpo moído, preciso renovar' },
  { id: 'horny', icon: '🔥', label: 'Com Vontade', desc: 'Preciso resolver o tesão' }
];
...
const MasseurProfile = () => (
  <div className="flex items-center gap-4 bg-[#151517] p-4 rounded-2xl border border-white/5 mb-6">
    <div className="relative">
      <div className="w-14 h-14 rounded-full bg-gray-800 flex items-center justify-center text-2xl border border-white/10">👨🏻</div>
      <div className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-green-500 rounded-full border-2 border-[#151517]"></div>
    </div>
    <div>
      <h3 className="font-bold text-white text-[16px]">Thaly <span className="text-[10px] text-[#0A84FF] bg-[#0A84FF]/10 px-2 py-0.5 rounded ml-1 uppercase">Profissional</span></h3>
      <p className="text-gray-400 text-[13px] leading-tight mt-1">"Faço o serviço direito. Sigilo absoluto e respeito."</p>
    </div>
  </div>
);
...
const CouponInventory = ({ inventory, appliedCoupon, onApply, onRemove, onAddManual }) => {
  ...
  const handleManualAdd = () => {
      ...
      if(codeUpper && SYSTEM_COUPONS[codeUpper]) {
          if (inventory.includes(codeUpper)) { alert('Você já tem esse!'); } 
  ...
  return (
    <div className="space-y-4 mt-8">
      ...
      <div className="flex gap-2 mb-3">
          <input value={manualCode} onChange={(e) => setManualCode(e.target.value)} placeholder="Digite aqui..." className="w-full bg-[#1C1C1E] border border-white/10 text-white text-[16px] rounded-[14px] p-3 placeholder:text-gray-600" />
...
export default function App() {
  ...
  const handleWhatsApp = () => {
    ...
    let surfaceText = "";
    if (selection.location.allowsTableChoice) surfaceText = selection.useTable ? "Levar Maca (+R$20)" : "Leito Próprio";
    ...
    let msg = `*PEDIDO NOVO #${bookingId}*
--------------------------------
👤 *Nome:* ${user.name}
📅 *Data:* ${dateStr}
⏰ *Hora:* ${selection.time}
📍 *Onde:* ${selection.location.label}
💆‍♂️ *Serviço:* ${selection.service.name}
⏳ *Tempo:* ${finalDuration}
🎵 *Vibe:* ${selection.music} / ${moodLabel}
${surfaceText}
${selection.aroma ? '🌸 Com Aromaterapia' : ''}

💰 *VALOR:* ${selection.location.id === 'outras-cidades' ? 'A combinar' : formatCurrency(finalPrice)}
💳 *Forma:* ${paymentText}
--------------------------------
            ...
            <div className="flex-1 p-6 pt-32 overflow-y-auto pb-48 animate-fade-in">
              ...
              <div className="space-y-10">
                ...
                <div className="mt-4" ref={vibeRef}>
                  <h4 className="text-[13px] font-bold text-gray-400 uppercase mb-3 tracking-widest flex items-center gap-2"><Zap className="w-4 h-4"/> Como você está se sentindo?</h4>
