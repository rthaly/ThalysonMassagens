...
// ==================================================================================
// GLOBAL STYLES (MELHORADOS PARA ALTO CONTRASTE)
// ==================================================================================
  ...
  <style dangerouslySetInnerHTML={{ __html: `
    ...
    *, *::before, *::after { 
    ...
    :root {
      ...
      /* Fundo mais profundo no dark mode para os cards destacarem */
      --c-bg: ${isDark ? '#09090b' : '#f8fafc'};
      /* Cards ligeiramente mais claros/contrastantes */
      --c-surface: ${isDark ? '#18181b' : '#ffffff'};
      --c-border: ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'};
      --c-text: ${isDark ? '#fafafa' : '#0f172a'};
      --c-text-muted: ${isDark ? '#a1a1aa' : '#64748b'};
      --c-blue: #3b82f6;
      --c-amber: #f59e0b;
    }
...
const InputField = memo(({ label, value, onChange, placeholder, icon, type = 'text', isDark = true, hasError = false, disabled = false, maxLength, id }: any) => {
  ...
  return (
    <div className={`space-y-1.5 w-full ${hasError ? 'animate-shake' : ''}`}>
      {label && (
        <label htmlFor={inputId} className={`block text-[10px] sm:text-xs font-bold uppercase tracking-widest pl-1 ${hasError ? 'text-red-400' : isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
          {label}
        </label>
      )}
      <div className="relative group">
        ...
        <input
          ...
          disabled={disabled}
          maxLength={maxLength}
          className={`input-field w-full min-h-[52px] rounded-xl text-sm font-medium transition-all border outline-none disabled:opacity-50 disabled:cursor-not-allowed ${icon ? 'pl-12 pr-4' : 'px-4'} ${hasError
            ? 'border-red-500/50 bg-red-950/10 text-red-400 placeholder:text-red-500/40'
            : isDark
              ? 'border-zinc-800 bg-zinc-900/50 text-white placeholder:text-zinc-600 focus:border-blue-500 focus:bg-zinc-800/80'
...
// ==================================================================================
// FLOATING WHATSAPP COMPONENT (ANIMAÇÃO ABRE E FECHA)
// ==================================================================================
const FloatingWhatsApp = memo(({ isDark, lang, openExternal }: any) => {
  const [isOpen, setIsOpen] = useState(false);
  const [hasInteracted, setHasInteracted] = useState(false);
  const floatBtnTexts = useMemo(() => lang === 'en' ? ["Need help?", "Have questions?", "Chat with me!"] : ["Precisa de ajuda?", "Ficou com dúvidas?", "Agende agora!"], [lang]);

  useEffect(() => {
    if (hasInteracted) return;
    
    // Animação de abrir e fechar o balão repetidamente
    const intervalId = setInterval(() => {
      setIsOpen(true);
      setTimeout(() => setIsOpen(false), 6000); // Fica aberto por 6s
    }, 15000); // Repete a cada 15s

    // Dispara a primeira vez rapidinho após carregar a página
    const initialTimeout = setTimeout(() => setIsOpen(true), 3000);
    const initialClose = setTimeout(() => setIsOpen(false), 10000);

    return () => { clearInterval(intervalId); clearTimeout(initialTimeout); clearTimeout(initialClose); };
  }, [hasInteracted]);

  return (
    <div className="fixed bottom-[88px] sm:bottom-24 right-4 z-50 flex items-end flex-col gap-2"
         onMouseEnter={() => { setIsOpen(true); setHasInteracted(true); }}
         onMouseLeave={() => setIsOpen(false)}>
      
      {/* O Balão de Chat que expande/recolhe */}
      <div 
        className={`transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] origin-bottom-right 
                    ${isOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-50 opacity-0 translate-y-4 pointer-events-none'}`}>
        
        <div 
          onClick={() => openExternal('whatsapp', 'Olá, estava no site relaxarhoje.com e gostaria de tirar uma dúvida.')}
          className={`flex items-center gap-3 p-2 pr-5 rounded-full shadow-2xl border cursor-pointer 
                      ${isDark ? 'bg-zinc-800 border-zinc-700 hover:bg-zinc-700' : 'bg-white border-slate-200 hover:bg-slate-50'}`}>
          
          <div className="relative shrink-0">
            <img src="https://i.ibb.co/gZxp3Dwz/Screenshot-1.png" alt="Thalyson" className="w-10 h-10 rounded-full object-cover border-2 border-[#25D366]" />
            <span className={`absolute bottom-0 right-0 w-3 h-3 bg-[#25D366] border-2 rounded-full ${isDark ? 'border-zinc-800' : 'border-white'}`}></span>
          </div>
          
          <div className="flex flex-col justify-center min-w-[120px]">
            <span className={`text-[9px] font-bold uppercase tracking-widest leading-none mb-1 ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
              Thalyson
            </span>
            <span className={`text-xs font-bold leading-none ${isDark ? 'text-white' : 'text-slate-800'}`}>
              {isOpen && <TypewriterText texts={floatBtnTexts} />}
            </span>
          </div>
        </div>

        {/* Triângulo do balão de fala */}
        <div className={`absolute bottom-[-6px] right-6 w-4 h-4 rotate-45 border-b border-r ${isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-slate-200'}`} />
      </div>

      {/* O Botão Fixo Verde do WhatsApp */}
      <button
        onClick={() => openExternal('whatsapp', 'Olá, estava no site relaxarhoje.com e gostaria de tirar uma dúvida.')}
        className="w-14 h-14 bg-[#25D366] text-white rounded-full shadow-[0_4px_20px_rgba(37,211,102,0.4)] flex items-center justify-center hover:scale-110 transition-transform relative z-10"
...
const TigrinhoRoulette = memo(({ isOpen, isDark, lang, onWin }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div role="dialog" aria-modal="true" className={`relative w-full max-w-sm rounded-[2rem] p-6 text-center border shadow-2xl animate-scale-in flex flex-col items-center ${isDark ? 'bg-zinc-900 border-amber-900/50 shadow-[0_0_50px_rgba(245,158,11,0.15)]' : 'bg-white border-amber-200'}`}>
        <h3 className={`font-display text-3xl mb-1 mt-2 font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
          {lang === 'en' ? 'Spin & Win!' : 'Gire e Ganhe!'}
        </h3>
        <p className={`text-sm mb-8 font-medium ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
        ...
        <div className="relative w-64 h-64 mb-8">
          ...
          <div 
            className="w-full h-full rounded-full overflow-hidden border-[8px] border-[#09090b] shadow-inner relative"
            style={{ 
          ...
          >
            {PRIZES.map((p, i) => (
              <div key={i} className="absolute inset-0 origin-center" style={{ transform: `rotate(${i * 45}deg)` }}>
                <div className="absolute top-3 left-1/2 -translate-x-1/2 text-white font-bold font-display text-xl drop-shadow-md">
                  {p.val}
                </div>
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-[#09090b] border-4 border-amber-500 flex items-center justify-center shadow-lg z-10">
        ...
        {phase === 'idle' && (
          <Button full size="lg" variant="amber" onClick={spinWheel} className="animate-pulse-slow font-extrabold text-lg">
            {lang === 'en' ? 'SPIN ROULETTE' : 'GIRAR ROLETA'}
          </Button>
        )}
        
        {phase === 'spinning' && (
          <Button full size="lg" disabled variant="secondary" className="opacity-50 font-extrabold text-lg">
        ...
        {phase === 'won' && (
          <div className="animate-fade-up w-full">
            <p className={`font-display font-extrabold text-3xl mb-4 text-amber-500`}>
              {lang === 'en' ? 'You Won R$' : 'Você Ganhou R$'} {winValue}!
            </p>
            <Button full size="lg" variant="amber" onClick={() => onWin(winValue)} className="font-extrabold text-lg">
...
// ==================================================================================
// RENDER HELPERS PARA MAIS CONTRASTE
// ==================================================================================
// Função para destacar as primeiras palavras na listagem de detalhes do serviço
const highlightLine = (line: string, isDark: boolean) => {
  const cleanLine = line.replace(/^\d+\.\s*/, '');
  const words = cleanLine.split(' ');
  const limit = Math.min(2, words.length); // Destaca as primeiras 2 palavras
  const highlighted = words.slice(0, limit).join(' ');
  const rest = words.slice(limit).join(' ');
  
  return (
    <>
      <strong className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{highlighted}</strong> {rest}
    </>
  );
};
...
const ServiceModal = memo(({ service, isOpen, onClose, onSelect, isInCart, isDark, T, lang, isPack }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-modal-backdrop">
      <div role="dialog" aria-modal="true" className={`relative w-full max-w-md max-h-[85vh] flex flex-col rounded-3xl border shadow-2xl animate-scale-in overflow-hidden ${isDark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-slate-200'}`}>
        <div className={`relative p-5 pb-4 shrink-0 border-b ${isDark ? 'border-zinc-800' : 'border-slate-100'} ${isPack ? (isDark ? 'bg-amber-950/20' : 'bg-amber-50/50') : ''}`}>
          ...
          <div className="flex items-center gap-3 mb-3 pr-10">
            ...
            <div className="flex-1 min-w-0">
               ...
               <h2 className={`font-display text-xl sm:text-2xl font-bold truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{service.title}</h2>
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className={`font-display font-extrabold text-3xl ${isDark ? 'text-blue-400' : 'text-blue-600'} ${isPack ? isDark ? 'text-amber-400' : 'text-amber-600' : ''}`}>{formatMoney(service.price, lang)}</span>
            {service.fullPrice && <span className={`text-sm font-bold line-through ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>{formatMoney(service.fullPrice, lang)}</span>}
          </div>
        </div>
        <div className={`flex-1 overflow-y-auto p-5 space-y-6 scrollbar-hide ${isDark ? 'text-zinc-300' : 'text-slate-700'}`}>
          <p className={`text-sm leading-relaxed font-medium ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>{service.desc}</p>
          <div>
            ...
            <div className="space-y-4">
              {service.details.split('\n').map((line: string, i: number) => {
                const stepNum = line.match(/^\d+/)?.[0] || (i + 1).toString();
                return (
                  <div key={i} className="flex items-start gap-3 text-sm">
                    <div className={`mt-0.5 shrink-0 w-6 h-6 rounded-full flex items-center justify-center font-bold text-[11px] ${isPack ? isDark ? 'bg-amber-500/20 text-amber-500' : 'bg-amber-100 text-amber-700' : isDark ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-700'}`}>
                      {stepNum}
                    </div>
                    <span className={`leading-relaxed flex-1 font-medium ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>
        ...
        <div className={`p-4 border-t shrink-0 ${isDark ? 'border-zinc-800 bg-zinc-900' : 'border-slate-100 bg-white'}`}>
          <Button full size="xl" variant={isInCart ? 'outline' : isPack ? 'amber' : 'primary'} onClick={() => { onSelect(service); onClose(); }}>
            {isInCart ? (lang === 'en' ? 'Remove from Selection' : 'Remover da Seleção') : (lang === 'en' ? 'Select this Service' : 'Selecionar Opção')}
          </Button>
        </div>
      </div>
    </div>
...
const ServiceCard = memo(({ service, isInCart, onToggle, isDark, T, lang, isPack = false, onOpenModal }: any) => {
  return (
    <button className={`relative w-full text-left rounded-2xl border transition-all duration-200 card-hover flex flex-col h-auto min-h-[120px] ${isInCart ? isPack ? 'service-card-selected-amber border-amber-500/50 bg-amber-500/10' : 'service-card-selected border-blue-500/50 bg-blue-500/5' : isDark ? 'bg-zinc-900 border-zinc-800 hover:border-zinc-700' : 'bg-white border-slate-200 shadow-sm hover:border-slate-300'}`} onClick={() => onOpenModal(service)}>
      {isInCart && (
      ...
      <div className="p-4 flex-1 flex flex-col">
        <div className="flex items-start gap-3 mb-3">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border ${isPack ? isDark ? 'bg-amber-900/30 border-amber-800/50 text-amber-500' : 'bg-amber-50 border-amber-200 text-amber-600' : isDark ? 'bg-zinc-800 border-zinc-700 text-zinc-300' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
            <Icon name={service.icon} size={24} />
          </div>
          <div className="flex-1 min-w-0 pr-6 pt-0.5">
            <h3 className={`text-lg font-extrabold leading-tight truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{service.title}</h3>
            <p className={`text-xs mt-1.5 leading-relaxed line-clamp-2 font-medium ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>{service.desc}</p>
          </div>
        </div>
        <div className="flex items-end justify-between mt-auto gap-2">
          <div className={`inline-block text-[10px] font-bold uppercase tracking-widest px-2.5 py-1.5 rounded-md border truncate max-w-[50%] ${isPack ? isDark ? 'border-amber-800/50 text-amber-500' : 'border-amber-300 text-amber-700' : isDark ? 'border-zinc-700 text-zinc-300' : 'border-slate-300 text-slate-600'}`}>
            {service.tag}
          </div>
          <div className="text-right shrink-0">
            ...
            <p className={`font-display font-bold text-xl sm:text-2xl leading-none ${isDark ? 'text-blue-400' : 'text-blue-600'} ${isPack ? isDark ? 'text-amber-400' : 'text-amber-600' : ''}`}>{formatMoney(service.price, lang)}</p>
          </div>
        </div>
      </div>
    </button>
...
export default function App() {
  ...
  if (!isClient) return <div className="min-h-screen w-full bg-[#09090b]" />;

  if (loading) {
    return (
      <div className={`fixed inset-0 flex flex-col items-center justify-center z-[100] ${isDark ? 'bg-[#09090b]' : 'bg-slate-50'}`}>
        <div className="flex flex-col items-center w-[200px] text-center">
          <div className="relative mb-8">
            ...
            <div className="relative w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center shadow-2xl border border-blue-400/20">
              <span className="font-display font-extrabold text-4xl text-white">T</span>
            </div>
          </div>
          <p className={`text-xs uppercase font-extrabold tracking-widest ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>{T.loading}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <FloatingWhatsApp isDark={isDark} lang={lang} openExternal={openExternal} />
          ...
          <header className="pt-6 pb-6 sm:pt-10 sm:pb-8">
            <div className="flex items-center justify-between gap-4">
              <button onClick={() => setStep(0)} className="group text-left" aria-label="Voltar para o início">
                <h1 className={`font-display font-extrabold text-3xl sm:text-4xl leading-tight mb-1 transition-opacity group-hover:opacity-80 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  Thalyson Massagens
                </h1>
                <div className={`flex items-center gap-2 text-[10px] uppercase tracking-widest font-extrabold ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
              ...
              <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                <button onClick={() => setLang(l => l === 'pt' ? 'en' : 'pt')} className={`relative h-12 w-12 flex items-center justify-center rounded-xl border transition-all ${isDark ? 'border-zinc-800 bg-zinc-900 text-zinc-400 hover:text-white hover:border-zinc-600' : 'border-slate-200 bg-white text-slate-500 hover:text-slate-800'}`}>
                  <Icon name="globe" size={20} />
                </button>
                <button onClick={() => openExternal('instagram')} className={`h-12 w-12 flex items-center justify-center rounded-xl border transition-all ${isDark ? 'border-zinc-800 bg-zinc-900 text-pink-500 hover:text-pink-400 hover:border-pink-900' : 'border-slate-200 bg-white text-pink-600 hover:text-pink-500'}`}>
                  <Icon name="instagram" size={20} />
                </button>
                <button onClick={() => setMenuOpen(true)} className={`h-12 w-12 flex items-center justify-center rounded-xl border transition-all ${isDark ? 'border-zinc-800 bg-zinc-900 text-zinc-400 hover:text-white hover:border-zinc-600' : 'border-slate-200 bg-white text-slate-500 hover:text-slate-800'}`}>
              ...
              <nav aria-label="Progresso do agendamento" className="mt-6 sm:mt-8 flex items-center gap-2">
                {[1, 2, 3].map(i => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1.5 cursor-pointer" onClick={() => { if (i < step) setStep(i); }}>
                    ...
                    <span className={`text-[10px] uppercase font-bold tracking-widest ${step >= i ? isDark ? 'text-white' : 'text-slate-800' : isDark ? 'text-zinc-600' : 'text-slate-400'}`}>
        ...
        <div>
          ...
          {step === 0 && (
            <section className="animate-fade-up space-y-8 sm:space-y-12">
              ...
              <div className="flex flex-col gap-6">
                ...
                <article className={`p-5 rounded-[2rem] border flex flex-col sm:flex-row items-start sm:items-center gap-4 ${isDark ? 'bg-zinc-900 border-zinc-800 shadow-xl shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
                  <div className="relative shrink-0">
                    <div className={`w-24 h-24 rounded-[1.5rem] overflow-hidden border-2 p-0.5 ${isDark ? 'border-blue-500/50' : 'border-blue-200'}`}>
                      <img src="https://i.ibb.co/gZxp3Dwz/Screenshot-1.png" alt="Thalyson Santos, São Paulo - Bela Vista" className="w-full h-full object-cover rounded-[1.2rem]" />
                    </div>
                  </div>
                  <div className="min-w-0 flex-1">
                    <h3 className={`font-display font-extrabold text-2xl mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>Thalyson</h3>
                    <p className={`text-[11px] font-extrabold uppercase tracking-widest mb-3 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>{T.specialist}</p>
                    <p className={`text-sm leading-relaxed font-medium ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>{T.choose_sub}</p>
                  </div>
                </article>

                <article className={`p-5 rounded-[2rem] border ${isDark ? 'bg-zinc-900 border-zinc-800 shadow-xl shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
                  <div className="flex items-center justify-between mb-5 gap-4">
                    <div className="flex items-center gap-4 min-w-0">
                      <div className={`w-12 h-12 rounded-[1rem] flex items-center justify-center shrink-0 ${isDark ? 'bg-amber-900/30 text-amber-500 border border-amber-900/50' : 'bg-amber-50 text-amber-600 border border-amber-200'}`}>
                        <Icon name="award" size={24} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <h3 className={`text-[10px] uppercase font-bold tracking-widest ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>{T.level_label}</h3>
                        <p className={`text-base font-bold truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{getCurrentLevelTitle()}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <span className="font-display text-3xl font-extrabold text-blue-500">{user.xp}</span>
              ...
              <nav aria-label="Tipos de agendamento" className={`flex p-1.5 rounded-2xl border w-full sm:w-fit mx-auto ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-slate-50 border-slate-200'}`}>
                {[ { id: 'single', label: T.tab_single, icon: 'user' }, { id: 'packs', label: T.tab_packs, icon: 'package' } ].map(tab => (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)} aria-pressed={activeTab === tab.id}
                    className={`flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-[14px] text-xs font-bold uppercase tracking-wider transition-all duration-300 min-h-[48px] ${activeTab === tab.id
                      ? tab.id === 'packs' ? 'bg-amber-500 text-amber-950 shadow-lg' : 'bg-blue-600 text-white shadow-lg'
                      : isDark ? 'text-zinc-400 hover:text-white hover:bg-white/5' : 'text-slate-500 hover:text-slate-900 hover:bg-black/5'}`}>
                ...
                {activeTab === 'single' ? (
                  <div className="space-y-8">
                    {categoryConfig.map(cat => {
                      ...
                      return (
                        <section key={cat.id} className="rounded-[2rem] border overflow-hidden shadow-sm" style={{ borderColor: cfg.borderColor, background: isDark ? cfg.bg : cfg.lightBg }}>
                          <div className="px-5 py-5 flex items-center gap-4 border-b" style={{ borderColor: cfg.borderColor }}>
                            <div className="w-12 h-12 rounded-[1rem] flex items-center justify-center shrink-0" style={{ background: `${cfg.color}15`, border: `1px solid ${cfg.color}30` }}>
                              <Icon name={cat.icon} size={24} style={{ color: cfg.color }} />
                            </div>
                            <div className="min-w-0 flex-1">
                              <h2 className={`font-display text-xl font-bold mb-0.5 ${isDark ? 'text-white' : 'text-slate-900'}`}>{cat.title}</h2>
                              <p className={`text-xs font-medium ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>{cat.desc}</p>
                            </div>
                          </div>
              ...
              <section className={`py-10 border-t border-b ${isDark ? 'border-zinc-800' : 'border-slate-200'}`}>
                <div className="flex items-center justify-between mb-6">
                  <h2 className={`font-display text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.reviews_title}</h2>
                  <div className="flex gap-2 shrink-0">
                    {['chevron-left', 'chevron-right'].map((dir, i) => (
                      <button key={dir} onClick={() => reviewScrollRef.current?.scrollBy({ left: i === 0 ? -260 : 260, behavior: 'smooth' })} className={`w-12 h-12 rounded-[1rem] flex items-center justify-center border transition-all ${isDark ? 'border-zinc-700 bg-zinc-800 text-zinc-400 hover:text-white hover:border-zinc-600' : 'border-slate-200 bg-white text-slate-500 hover:text-slate-800 shadow-sm'}`}>
              ...
              <section className="pb-6">
                <h2 className={`font-display text-2xl font-bold text-center mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.faq_title}</h2>
                <div className={`rounded-3xl border overflow-hidden ${isDark ? 'bg-zinc-900 border-zinc-800 shadow-lg shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
          ...
          {step === 1 && (
            <section className="animate-fade-up max-w-xl mx-auto space-y-8">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[ { id: 'home', label: T.loc_home, icon: 'home' }, { id: 'motel', label: T.loc_motel, icon: 'bed' }, { id: 'hotel', label: T.loc_hotel, icon: 'building' } ].map(x => (
                  <button key={x.id} onClick={() => setBooking(b => ({ ...b, locationType: x.id as any }))} aria-pressed={booking.locationType === x.id}
                    className={`p-4 rounded-[1.5rem] flex flex-col items-center justify-center gap-3 transition-all border text-center min-h-[100px] ${booking.locationType === x.id ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-900/20' : isDark ? 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-white hover:border-zinc-700' : 'bg-white border-slate-200 text-slate-500 hover:text-slate-800 shadow-sm'}`}>
              ...
              <article className={`p-5 sm:p-8 rounded-[2rem] border space-y-6 ${isDark ? 'bg-zinc-900 border-zinc-800 shadow-xl shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                {booking.locationType === 'motel' && (
                  <div className={`p-5 rounded-2xl border flex items-start gap-4 animate-fade-up ${isDark ? 'bg-pink-900/10 border-pink-900/30' : 'bg-pink-50 border-pink-100'}`}>
                    <div className={`shrink-0 w-12 h-12 rounded-xl flex items-center justify-center ${isDark ? 'bg-pink-500/20 text-pink-400' : 'bg-pink-100 text-pink-600'}`}>
                      <Icon name="heart" size={24} />
                    </div>
                    <p className={`text-sm leading-relaxed font-medium ${isDark ? 'text-zinc-300' : 'text-slate-700'}`}>{T.motel_note}</p>
                  </div>
                )}
              </article>
            </section>
          ...
          {step === 2 && (
            <section className="animate-fade-up max-w-2xl mx-auto space-y-8">
              <header className="text-center">
                <h2 className={`font-display text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.select_time_title}</h2>
              </header>

              <div className="relative w-full">
                <button onClick={() => scrollDates('left')} className={`hidden md:flex absolute -left-12 top-1/2 -translate-y-1/2 z-20 w-12 h-12 items-center justify-center rounded-xl border transition-all ${isDark ? 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:text-white hover:bg-zinc-800' : 'bg-white border-slate-200 text-slate-500 hover:text-slate-800 shadow-sm'} shadow-md`}><Icon name="chevron-left" size={24} /></button>
                <div ref={dateScrollRef} className="flex gap-4 overflow-x-auto snap-x py-2 scrollbar-hide w-full">
                  {daysArray.map((d, idx) => {
                    ...
                    return (
                      <button key={idx} onClick={() => { setBooking(b => ({ ...b, date: d.toISOString(), time: null })); vibrate(30); }} aria-pressed={isSel}
                        className={`snap-center shrink-0 w-[88px] min-h-[100px] py-3 rounded-[1.2rem] flex flex-col items-center justify-center gap-1.5 transition-all border ${isSel ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-900/20' : isDark ? 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-700' : 'bg-white border-slate-200 text-slate-500 hover:text-slate-800 shadow-sm'}`}>
                        <span className={`text-[10px] uppercase font-bold tracking-wider ${isSel ? 'text-blue-200' : ''}`}>{mo}</span>
                        <span className={`font-display text-3xl font-bold leading-none ${isSel ? 'text-white' : isDark ? 'text-zinc-200' : 'text-slate-800'}`}>{d.getDate()}</span>
              ...
              {booking.date && generateTimeSlots.length > 0 && (
                <div className={`space-y-6 animate-fade-up ${hasErrorGlobal && !booking.time ? 'animate-shake' : ''}`}>
                  {[
                    ...
                    <div key={group.key} className={`p-5 sm:p-6 rounded-[2rem] border ${isDark ? 'bg-zinc-900 border-zinc-800 shadow-lg shadow-black/10' : 'bg-white border-slate-100 shadow-sm'}`}>
                      <div className={`flex items-center gap-2 mb-5 text-[11px] uppercase font-bold tracking-widest ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
                        ...
                        {group.slots.map(t => {
                          ...
                          return (
                            <button key={t} onClick={() => { setBooking(b => ({ ...b, time: t })); vibrate(30); }} aria-pressed={isSel}
                              className={`relative flex flex-col items-center justify-center py-3.5 px-2 rounded-xl border text-[15px] font-bold transition-all min-h-[60px] w-full ${isSel
                                ? isRush ? 'bg-amber-500 border-amber-500 text-amber-950 shadow-md' : 'bg-blue-600 border-blue-500 text-white shadow-md'
                                : isDark
                                  ? isRush ? 'bg-amber-900/20 border-amber-800/50 text-amber-500 hover:bg-amber-900/40' : 'bg-zinc-800 border-zinc-700 text-zinc-300 hover:text-white hover:border-zinc-600'
                                  : isRush ? 'bg-amber-50 border-amber-200 text-amber-700 hover:bg-amber-100' : 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-white hover:border-slate-300 shadow-sm'}`}>
                              {t}
                              {isRush && <span className={`text-[9px] uppercase font-extrabold tracking-widest mt-1 ${isSel ? 'text-amber-900' : isDark ? 'text-amber-500' : 'text-amber-600'}`}>+{formatMoney(RUSH_FEE, lang).replace('R$ ', 'R$')}</span>}
                            </button>
                          );
                        })}
                      </div>
                  ...
                  {Object.values(groupedTimeSlots).flat().some(t => RUSH_HOURS.includes(t)) && booking.locationType !== 'motel' && (
                    <div className={`flex items-start gap-3 p-4 rounded-2xl border text-sm font-semibold ${isDark ? 'bg-amber-900/10 border-amber-800/40 text-amber-400' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>
          ...
          {step === 3 && (
            <section className="animate-fade-up space-y-6 max-w-2xl mx-auto">
              ...
              <article className={`p-5 sm:p-8 rounded-[2rem] border ${isDark ? 'bg-zinc-900 border-zinc-800 shadow-xl shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
                <h3 className={`font-display text-2xl font-bold mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.extras_title}</h3>
                <p className={`text-sm font-medium mb-6 ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>{lang === 'en' ? 'Optional add-ons.' : 'Deseja adicionar algo extra?'}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {DATA.extras.map((ex: any) => {
                    ...
                    return (
                      <button key={ex.id} onClick={() => { setBooking(b => ({ ...b, extras: { ...b.extras, [ex.id]: !b.extras[ex.id] } })); vibrate(30); }}
                        className={`flex items-start p-4 rounded-2xl border text-left cursor-pointer transition-all gap-4 ${isActive ? isDark ? 'bg-blue-900/20 border-blue-800' : 'bg-blue-50 border-blue-200' : isDark ? 'bg-zinc-800 border-zinc-700 hover:border-zinc-600' : 'bg-slate-50 border-slate-200 hover:border-slate-300 shadow-sm'}`}>
                        <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center mt-0.5 ${isActive ? 'bg-blue-500 text-white' : isDark ? 'bg-zinc-900 text-zinc-400 border border-zinc-700' : 'bg-white border border-slate-200 text-slate-500'}`}>
                          <Icon name={ex.icon} size={20} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className={`text-base font-bold truncate ${isActive ? isDark ? 'text-blue-400' : 'text-blue-700' : isDark ? 'text-zinc-200' : 'text-slate-800'}`}>{ex.label}</p>
                          <span className={`text-[11px] font-extrabold mt-1 inline-block ${isActive ? isDark ? 'text-blue-300' : 'text-blue-600' : isDark ? 'text-zinc-500' : 'text-slate-500'}`}>+{formatMoney(price, lang)}</span>
                        </div>
                      </button>
                    );
                  })}
              ...
              <article className={`p-5 sm:p-8 rounded-[2rem] border ${isDark ? 'bg-zinc-900 border-zinc-800 shadow-xl shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
                <h3 className={`font-display text-2xl font-bold mb-5 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.coupon_section}</h3>
                ...
                <div className="flex items-center gap-3 mb-6">
                  <input
                    ...
                    className={`flex-1 rounded-xl px-4 h-[52px] font-bold text-sm outline-none border transition-colors ${isDark ? 'bg-zinc-800 border-zinc-700 text-white focus:border-blue-500' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-500'}`}
                  />
                  <Button onClick={() => {
                  ...
                  }} size="lg" variant="primary" className="!w-auto !px-6 !flex-none">
                ...
                {user.coupons.length > 0 ? (
                  <div className="space-y-4">
                    {user.coupons.map(c => (
                      <button key={c.id} onClick={() => { setBooking(b => ({ ...b, appliedCoupon: b.appliedCoupon?.id === c.id ? null : c })); vibrate(30); }}
                        className={`w-full p-4 rounded-2xl border transition-all flex items-center justify-between gap-4 ${booking.appliedCoupon?.id === c.id ? isDark ? 'bg-emerald-900/20 border-emerald-800 text-emerald-400' : 'bg-emerald-50 border-emerald-200 text-emerald-700' : isDark ? 'bg-zinc-800 border-zinc-700 text-zinc-300 hover:border-zinc-600' : 'bg-slate-50 border-slate-200 text-slate-700 hover:border-slate-300'}`}>
                        <div className="flex items-center gap-4">
                          <Icon name="gift" size={24} className={booking.appliedCoupon?.id === c.id ? 'text-emerald-500' : isDark ? 'text-zinc-500' : 'text-slate-400'} />
                          <span className="text-base font-bold truncate">{c.title}</span>
                        </div>
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${booking.appliedCoupon?.id === c.id ? 'bg-emerald-500 border-emerald-500 text-white' : isDark ? 'border-zinc-600' : 'border-slate-300'}`}>
                  ...
                  <div className={`p-5 rounded-2xl border border-dashed text-center text-sm font-semibold ${isDark ? 'border-zinc-700 text-zinc-500' : 'border-slate-300 text-slate-400'}`}>
                    {T.coupon_empty}
                  </div>
                )}
              </article>

              <article className={`p-5 sm:p-8 rounded-[2rem] border ${hasErrorGlobal && !booking.payment ? 'animate-shake' : ''} ${isDark ? 'bg-zinc-900 border-zinc-800 shadow-xl shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
                <h4 className={`font-display text-2xl font-bold mb-5 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.payment_title}</h4>
                <div className="space-y-4">
                  {[ { id: 'pix', label: T.pay_pix, icon: 'smartphone' }, { id: 'card', label: T.pay_card, icon: 'credit-card' }, { id: 'money', label: T.pay_cash, icon: 'banknote' } ].map(p => (
                    <button key={p.id} onClick={() => { setBooking(b => ({ ...b, payment: p.id })); vibrate(30); if (p.id === 'pix') { navigator.clipboard.writeText(CONFIG.PIX_KEY); addToast(T.toast_pix_copied); } }}
                      className={`w-full flex items-center gap-4 p-5 rounded-2xl border transition-all ${booking.payment === p.id ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-900/20' : isDark ? 'bg-zinc-800 border-zinc-700 text-zinc-300 hover:text-white hover:border-zinc-600' : 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-white shadow-sm'}`}>
                      <Icon name={p.icon} size={24} className="shrink-0" />
                      <span className="flex-1 text-left text-base font-bold">{p.label}</span>
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${booking.payment === p.id ? 'border-white' : isDark ? 'border-zinc-600' : 'border-slate-300'}`}>
              ...
              <article className={`p-5 sm:p-8 rounded-[2rem] border ${isDark ? 'bg-zinc-900 border-zinc-800 shadow-xl shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
                <h3 className={`font-display text-2xl font-bold mb-6 flex items-center gap-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  <Icon name="file-text" size={24} className={isDark ? 'text-zinc-500' : 'text-slate-400'} /> {T.summary_title}
                </h3>
                <div className="space-y-6">
                  <div>
                    <p className={`text-[11px] uppercase font-extrabold tracking-widest mb-4 ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>{T.summary_items}</p>
                    <div className="space-y-3">
                      {booking.cart.map((item, i) => (
                        <div key={i} className={`flex justify-between items-center text-base font-bold ${isDark ? 'text-zinc-200' : 'text-slate-800'}`}>
                  ...
                  {(Object.keys(booking.extras || {}).filter(k => booking.extras[k]).length > 0 || (booking.customExtraText || '').trim().length > 0) && (
                    <div className={`border-t pt-5 ${isDark ? 'border-zinc-800' : 'border-slate-100'}`}>
                      <p className={`text-[11px] uppercase font-extrabold tracking-widest mb-4 ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>{T.summary_extras}</p>
                      <div className="space-y-3">
                        {Object.keys(booking.extras || {}).filter(k => booking.extras[k]).map(k => {
                          ...
                          const price = booking.cart.some(i => i.type === 'pack') ? Math.floor(ex.price * 0.8) : ex.price;
                          return (
                            <div key={k} className={`flex justify-between items-center text-sm font-semibold ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>
                  ...
                  <div className={`border-t pt-5 ${isDark ? 'border-zinc-800' : 'border-slate-100'}`}>
                    <p className={`text-[11px] uppercase font-extrabold tracking-widest mb-4 ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>{T.summary_info}</p>
                    <div className={`space-y-3 text-sm font-medium ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>
                      <div className="flex items-center gap-3"><Icon name="calendar" size={18} className="text-blue-500" /> {booking.date ? new Date(booking.date).toLocaleDateString(lang === 'en' ? CONFIG.LOCALE_EN : CONFIG.LOCALE_PT) : ''} {lang === 'en' ? 'at' : 'às'} {booking.time}</div>
                      <div className="flex items-center gap-3"><Icon name="map-pin" size={18} className="text-blue-500" /> {booking.locationType === 'home' ? T.summary_loc_home : booking.locationType === 'motel' ? T.summary_loc_motel : T.summary_loc_hotel}</div>
                      <div className="flex items-center gap-3"><Icon name="clock" size={18} className="text-blue-500" /> ~{financials.duration} min</div>
                    </div>
                  </div>

                  <div className={`border-t pt-5 space-y-3 ${isDark ? 'border-zinc-800' : 'border-slate-100'}`}>
                    <div className={`flex justify-between items-center text-base font-bold ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>
                      <span>{T.subtotal}</span>
                      <span>{formatMoney(financials.sub, lang)}</span>
                    </div>
                    {booking.appliedCoupon && (
                      <div className="flex justify-between items-center text-base font-bold text-emerald-500">
                        <span>{booking.appliedCoupon.title}</span>
                        <span>-{formatMoney(financials.disc, lang)}</span>
                      </div>
                    )}
                    {financials.pixDisc > 0 && (
                      <div className={`flex justify-between items-center text-base font-bold ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>
                        <span>{T.pix_discount}</span>
                        <span>-{formatMoney(financials.pixDisc, lang)}</span>
                      </div>
                    )}
                    {financials.rushFee > 0 && (
                      <div className="flex justify-between items-center text-base font-bold text-amber-500">
                        <span>{T.msg_rush_fee}</span>
                        <span>+{formatMoney(financials.rushFee, lang)}</span>
                      </div>
                    )}
                    <div className={`flex justify-between items-end pt-5 mt-3 border-t ${isDark ? 'border-zinc-800' : 'border-slate-200'}`}>
                      <span className={`text-sm uppercase font-extrabold tracking-widest ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>{T.total_label}</span>
                      <div className="text-right">
                        <p className="font-display font-extrabold text-4xl text-blue-500 leading-none">{formatMoney(financials.total, lang)}</p>
                        <p className="text-[11px] uppercase font-extrabold tracking-widest text-blue-400 mt-2">+{estimatedXP} XP</p>
                      </div>
                    </div>
                  </div>
                  
                  {booking.locationType !== 'motel' && (
                    <div className={`flex items-start gap-3 p-4 rounded-xl text-sm font-semibold border ${isDark ? 'bg-zinc-800/50 border-zinc-700 text-zinc-400' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
                      <Icon name="car" size={20} className="shrink-0" />
              ...
              <article className={hasErrorGlobal && !booking.termsAccepted ? 'animate-shake' : ''}>
                <button onClick={() => setTermsOpen(true)} className={`w-full flex items-center justify-between p-6 rounded-[2rem] border transition-all gap-4 ${booking.termsAccepted ? isDark ? 'bg-emerald-900/20 border-emerald-800 shadow-lg shadow-emerald-900/10' : 'bg-emerald-50 border-emerald-200' : isDark ? 'bg-zinc-900 border-zinc-800 shadow-xl shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
                  <div className="flex items-center gap-5">
                    <div className={`w-14 h-14 rounded-[1.2rem] flex items-center justify-center shrink-0 ${booking.termsAccepted ? isDark ? 'bg-emerald-900/40 text-emerald-500' : 'bg-emerald-100 text-emerald-600' : isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500'}`}>
                      <Icon name="shield" size={28} />
                    </div>
                    <div className="text-left">
                      <p className={`text-base font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.terms_title}</p>
                      <p className={`text-[11px] font-extrabold uppercase tracking-widest mt-1 ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>{T.terms_read}</p>
                    </div>
                  </div>
          ...
          {step === 4 && (
            <section className="min-h-[70vh] flex flex-col items-center justify-center text-center animate-scale-in max-w-sm mx-auto px-4">
              <div className="relative mb-8">
                <div className={`w-28 h-28 rounded-full flex items-center justify-center border-[6px] ${isDark ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-emerald-200 bg-emerald-50'}`}>
                  <Icon name="check" size={48} className="text-emerald-500" />
                </div>
              </div>
              <h2 className={`font-display font-extrabold text-4xl mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.success_title}</h2>
              <p className={`text-base font-medium mb-10 ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>{T.success_sub}</p>
              
              <div className="w-full space-y-5">
                <Button variant="whatsapp" size="xl" full icon="message" onClick={() => openExternal('whatsapp', generateWhatsAppMsg())}>
                  {T.whatsapp_btn}
                </Button>
                <button onClick={() => { setStep(0); setBooking({ ...booking, cart: [], termsAccepted: false, appliedCoupon: null, bookingId: `BOOK_${Date.now()}`, customExtraText: '' }); }} className={`w-full text-xs font-extrabold uppercase tracking-widest py-4 ${isDark ? 'text-zinc-500 hover:text-white' : 'text-slate-500 hover:text-slate-900'} transition-colors`}>
                  {T.back_home}
                </button>
              </div>
            </section>
      ...
      {step >= 0 && step < 4 && booking.cart.length > 0 && (
        <nav className="fixed bottom-0 inset-x-0 p-3 sm:p-5 z-40 animate-slide-up pointer-events-none">
          <div className={`max-w-3xl mx-auto pointer-events-auto rounded-[2rem] overflow-hidden border shadow-2xl ${isDark ? 'bg-zinc-900/95 backdrop-blur-xl border-zinc-800 shadow-black/50' : 'bg-white/95 backdrop-blur-xl border-slate-200'}`}>
            <div className="flex items-center justify-between gap-4 px-5 py-4 sm:px-6 sm:py-5">
              {step > 0 && (
                <button onClick={() => { setStep(s => s - 1); vibrate(30); }} className={`w-14 h-14 flex items-center justify-center rounded-[1.2rem] border shrink-0 ${isDark ? 'border-zinc-700 bg-zinc-800 text-zinc-300 hover:text-white hover:bg-zinc-700' : 'border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100 shadow-sm'}`}>
                  <Icon name="chevron-left" size={24} />
                </button>
              )}
              <div className="flex-1 min-w-0 pl-1">
                <p className={`text-[10px] sm:text-xs uppercase font-extrabold tracking-widest truncate ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
                  {step === 0 ? `${booking.cart.length} ${T.items_selected}` : step === 3 ? T.total_label : T.subtotal}
                </p>
                <p className={`font-display font-extrabold text-2xl sm:text-3xl leading-none truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  {step === 3 ? formatMoney(financials.total, lang) : formatMoney(financials.sub, lang)}
                </p>
              </div>
              <button onClick={handleNextStep} className={`h-14 flex items-center gap-3 px-6 sm:px-8 rounded-[1.2rem] font-bold text-sm sm:text-base uppercase tracking-wider shrink-0 transition-all ${isStepValid() ? step === 3 ? 'bg-[#25D366] hover:bg-[#22c55e] text-white shadow-lg shadow-[#25D366]/20' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-900/20' : isDark ? 'bg-zinc-800 text-zinc-600' : 'bg-slate-100 text-slate-400'}`}>
                {step === 3 ? (
                  ...
                  <><span className="hidden sm:inline">{T.next_btn}</span><span className="sm:hidden">{T.btn_next_short}</span> <Icon name="chevron-right" size={20} /></>
                )}
              </button>
            </div>
          </div>
      ...
      {termsOpen && (
        <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div role="dialog" aria-modal="true" className={`relative w-full max-w-lg max-h-[85vh] rounded-[2rem] flex flex-col border shadow-2xl animate-slide-up ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}>
            <div className={`flex items-center justify-between p-6 border-b shrink-0 ${isDark ? 'border-zinc-800' : 'border-slate-100'}`}>
              <h3 className={`font-display text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.rules_complete}</h3>
              <button onClick={() => setTermsOpen(false)} className={`w-10 h-10 rounded-full flex items-center justify-center ${isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500'}`}>
                <Icon name="x" size={20} />
              </button>
            </div>
            ...
            <div className={`p-6 border-t shrink-0 ${isDark ? 'border-zinc-800' : 'border-slate-100'}`}>
      ...
      {levelUpPopup && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div role="dialog" aria-modal="true" className={`relative w-full max-w-sm rounded-[2rem] p-8 text-center border shadow-2xl animate-scale-in ${isDark ? 'bg-zinc-900 border-amber-900/50' : 'bg-white border-amber-200'}`}>
            <div className={`w-24 h-24 mx-auto rounded-[1.5rem] flex items-center justify-center mb-6 bg-gradient-to-br from-amber-400 to-amber-600 text-amber-950 shadow-lg shadow-amber-900/20`}>
              <Icon name="trophy" size={40} />
            </div>
            <h3 className={`font-display font-extrabold text-4xl mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>{T.levelup_popup_title}</h3>
            <p className={`text-base font-medium leading-relaxed mb-8 ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>{T.levelup_popup_msg}</p>
            <Button full size="xl" variant="amber" onClick={() => { setLevelUpPopup(false); vibrate(50); }}>
              {T.level_redeem}
            </Button>
          </div>
        </div>
