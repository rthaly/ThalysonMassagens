...
const Card = ({ children, isDark, className = '', onClick, active = false }) => (
  <div onClick={onClick} className={`
    ...
    ${isDark 
      ? `bg-zinc-900/80 backdrop-blur-md ${active ? 'border border-amber-500 bg-amber-500/5 ring-1 ring-amber-500/50' : 'border border-zinc-800'}` 
      : `bg-white ${active ? 'border border-amber-500 bg-amber-50 ring-1 ring-amber-500/50' : 'border border-slate-200 shadow-sm'}`}
    ${className}
  `}>
    {children}
  </div>
...
const Confetti = ({ active }) => {
  ...
  useEffect(() => {
    ...
    const particles = Array.from({ length: 100 }, () => ({
      ...
      w: Math.random() * 10 + 5,
      h: Math.random() * 5 + 5,
      color: ['#f59e0b', '#fbbf24', '#ffffff', '#3b82f6'][Math.floor(Math.random() * 4)],
...
⚠️ Obs: Massagem focada em tirar dor e cansaço.` : "Wood therapy focused on relaxation."
            ...
            { 
              id: 'sensitiva', min: 60, price: 160, icon: Flame, tag: "INTENSA",
...
• FINALIZAÇÃO: Manual inclusa (com bastante óleo).` : "Starts with relaxing, moves to sensory touches and includes Lingam Massage."
            ...
            { 
              id: 'mista', min: 60, price: 200, icon: Zap, tag: "PREMIUM",
        ...
        <div className="max-w-md mx-auto space-y-8 pt-2">
          ...
          {step === 1 && (
            <div className="animate-slide-in">
              ...
              {booking.date && generateTimeSlots.length === 0 && (<div className="text-center py-10 opacity-50 bg-zinc-900/50 rounded-2xl border border-zinc-800"><p className="text-sm font-bold">{T.empty_slots}</p><p className="text-xs mt-1">{T.try_tomorrow}</p></div>)}
            </div>
          )}
               ...
               <div className="relative">
                   <div className={`p-6 rounded-t-[2rem] rounded-b-xl border-t border-x shadow-2xl relative overflow-hidden ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'}`}>
                      ...
                      <div className="flex justify-between items-start mb-6 mt-2">
                          <div>
                              <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded mb-2 inline-block ${isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-100 text-slate-500'}`}>{booking.type === 'pack' ? 'Pacote' : (booking.type === 'subscription' ? 'Assinatura' : 'Sessão Individual')}</span>
                      ...
                      <div className="space-y-3 border-b border-dashed border-zinc-700/50 pb-6 mb-6">
                          <div className="flex justify-between text-sm"><span>Valor Base</span><span>{T.currency} {booking.item.price}</span></div>
